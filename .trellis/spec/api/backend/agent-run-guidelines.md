...
- 终态顺序固定：等待 executor result -> 写 `starter.run` -> 条件更新主库（含 fencing 校验）-> 发布唯一 terminal event -> 清续租定时器 -> db lease release -> registry release。`runToTerminal` 与 `finalizeRun` 函数体最外层有兜底 catch：调用方以 void fire-and-forget，收尾路径任何异常只记 error 日志（含 runId），不产生 unhandled rejection。兜底触发时 Run 可能已落终态，也可能停留在非终态且两层 lease 未释放（该 lane 阻塞到进程重启的恢复扫描）——进程存活优先于单 lane 自愈。
...
- RunEvent publisher 成功持久化事件时同时推入 start queue 和已有恢复 subscribers。终态提交成功、终态未提交或终态事务抛错后都必须关闭 start queue；SSE iterator 提前 `return()` 只关闭当前 queue，不 abort Run。`startRunTransport` 的 JSON 分支在返回 runId 前调用 start iterable 的 `return()` 结束 start queue（幂等命中返回的回放流同样结束），事件不积累到 Run 终态；`AsyncEventQueue.push` 对已关闭 queue 是 no-op，publisher 与 Run 行为不受影响。
...
- webhook 事件交付的测试清单见 `webhook-guidelines.md`。
