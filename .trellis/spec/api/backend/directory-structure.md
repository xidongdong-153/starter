...
`routes/index.ts` 仍只挂载 `createAiRoute(runtime)`。AI 根路由显式组合 agent、configuration、run、session、usage-audit、conversation、prompt 和 skill 子路由；tool 目录只放工具注册、编排和测试工具。
