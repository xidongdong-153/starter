...
- SDK partial message、thinking、Provider payload 和原始错误不会进入公开协议。
...
SSE 断开不会 abort Run。Route 只停止向当前连接写数据；Agent 继续运行、写 Pi transcript、写主库终态。Admin 在连接异常后重新读取 Run 和 transcript，用持久结果替换临时流式视图。
