...
const envSchema = z.object({
  ...
  AI_REQUEST_TIMEOUT_MS: z.coerce.number().int().min(1_000).max(3_600_000).default(60_000),
  AI_RUN_MAX_MS: z.coerce.number().int().min(1_000).max(3_600_000).default(120_000),
  ...
  CORS_ORIGINS: z.string().default('http://localhost:2333,http://localhost:4399'),
...
export function parseEnv(input: NodeJS.ProcessEnv = process.env): AppEnv {
  ...
  if (backoffParts.length === 0 || aiWebhookBackoffMs.some((ms) => !Number.isInteger(ms) || ms < 0)) {
