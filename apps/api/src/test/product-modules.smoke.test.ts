...
it("chat agents：未登录 401，登录后与 /api/ai/agents 同构", async () => {
  ...
  try {
    ...
    const chatBody = await readSuccess<unknown>(
      await app.request("/api/chat/agents", { headers: { cookie } }),
    );
    const aiBody = await readSuccess<unknown>(
      await app.request("/api/ai/agents", { headers: { cookie } }),
    );
...
it("chat sessions：创建、改名、列表、归档全链路", async () => {
  ...
  try {
    ...
    expect(
      (await requestJson(app, "POST", "/api/chat/sessions", "", {})).status,
    ).toBe(401);
    ...
    const renamed = await requestJson(
      app,
      "PATCH",
      `/api/chat/sessions/${createdBody.data.id}`,
      cookie,
      { title: "改名" },
    );
    expect(renamed.status).toBe(200);
    expect((await readSuccess<{ title: string }>(renamed)).data.title).toBe(
      "改名",
    );
    ...
    expect(
      list.data.items.some((item) => item.id === createdBody.data.id),
    ).toBe(true);
    ...
    expect(
      listAfter.data.items.some((item) => item.id === createdBody.data.id),
    ).toBe(false);
...
it("flow agents：与 /api/ai/agents 同构", async () => {
  ...
  try {
    ...
    const flowBody = await readSuccess<unknown>(
      await app.request("/api/flow/agents", { headers: { cookie } }),
    );
    const aiBody = await readSuccess<unknown>(
      await app.request("/api/ai/agents", { headers: { cookie } }),
    );
...
it("flow sessions：创建后读取 lane=main transcript；未登录 401", async () => {
  ...
  try {
    expect(
      (await requestJson(app, "POST", "/api/flow/sessions", "", {})).status,
    ).toBe(401);
    ...
    const transcript = await app.request(
      `/api/flow/sessions/${createdBody.data.id}/transcript?lane=main`,
      { headers: { cookie } },
    );
