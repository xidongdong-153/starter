...
it('隔离 lane，并保留 starter.run 的原始 data', async () => {
  ...
  try {
    ...
    expect(await session.findRunTerminalEntries({ lane: 'review', runId: 'run-1' })).toEqual([terminal, duplicate])
    expect(
...
it('两个 Session DB 相互隔离，不向 app.db 写入 Pi 表', async () => {
  ...
  try {
    ...
    const tables = appDb.prepare("SELECT name FROM sqlite_master WHERE type = 'table' AND name LIKE 'pi_%'").all() as {
      name: string
    }[]
    appDb.close()
...
it('删除 Session 后不可打开，close 可重复调用且临时目录可清理', async () => {
  ...
  try {
    ...
    await expect(fixture.store.openSession('session-1')).rejects.toThrow('Pi Session 不存在')
  } finally {
    await fixture.store.close()
    await fixture.store.close()
    await rm(fixture.directory, { recursive: true, force: true })
...
it('支持调用方预置 message ID，并写入 Pi compaction entry', async () => {
  ...
  try {
    const session = await fixture.store.createSession({ id: 'session-1' })
    const messageEntry = await session.appendMessage('main', message, 'message-entry-1')
