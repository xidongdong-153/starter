...
describe("ai output contract registry", () => {
  it("校验定义并按 name/version 精确解析，不自动选择其他版本", () => {
    ...
    expect(registry.resolve({ name: first.name, version: first.version })).toBe(
      first,
    );
    expect(
      registry.resolve({ name: second.name, version: second.version }),
    ).toBe(second);
    expect(() =>
      registry.resolve({ name: first.name, version: "3.0.0" }),
    ).toThrow("未注册");
  ...
  it("拒绝非法 name、semver、空描述和非 object schema", () => {
    expect(() =>
      defineAiOutputContract({ ...contractInput, name: "Bad Name" }),
    ).toThrow("name");
    expect(() =>
      defineAiOutputContract({ ...contractInput, version: "latest" }),
    ).toThrow("version");
    expect(() =>
      defineAiOutputContract({ ...contractInput, description: " " }),
    ).toThrow("description");
    expect(() =>
      defineAiOutputContract({
        ...contractInput,
