import type { AccountProfile, FileItem, PublicProfile } from '@starter/contracts'
...
it('登录用户可以读取、更新和公开读取个人资料，并设置和清空头像', async () => {
  ...
  try {
    ...
    expect((await readSuccess<AccountProfile>(initial)).data.name).toBe('Test User')
    ...
    expect((await readSuccess<{ fileId: string }>(avatar)).data.fileId).toBe(file.id)
    ...
    expect(avatarContent.headers.get('cache-control')).toBe('public, max-age=300')
    expect(new Uint8Array(await avatarContent.arrayBuffer())).toEqual(new Uint8Array([137, 80, 78, 71]))
    ...
    expect((await app.request(`/api/profiles/${user.id}/avatar`)).status).toBe(404)
  } finally {
    cleanup()
  }
})
