...
import { ActiveRunRegistryError, createActiveRunRegistry } from '@api/infra/agent/active-run-registry.js'
...
describe('activeRunRegistry', () => {
  it('按 session + lane 原子 reserve，并拒绝第二个 lease', () => {
    ...
    expect(() => registry.reserve('session-1', 'main')).toThrowError(new ActiveRunRegistryError('busy'))
  ...
  it('把 abort、steer、follow-up 转发给 Pi controls，并幂等清理两个索引', () => {
    ...
    expect(registry.get('run-1')).toBeUndefined()
    expect(registry.getBySessionLane('session-1', 'main')).toBeUndefined()
    expect(() => registry.abort('run-1')).toThrowError(new ActiveRunRegistryError('not_active'))
  })
  ...
  it('拒绝不属于 registry 的 lease', () => {
    ...
    expect(() => second.attach(lease, 'run-1', controls)).toThrowError(new ActiveRunRegistryError('invalid_lease'))
  })
})
