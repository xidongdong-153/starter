...
it('destructive migration 删除旧三表、归一 legacy 审计并保留其余数据', () => {
  ...
  try {
    ...
    sqlite
      ...
      .run('message-1', 'conversation-1', 1, 'user', '{}', 'completed', now, now)
    sqlite
      .prepare(
    ...
    sqlite
      ...
      .run('call-test', 'request-2', 'user-1', 'model_test', null, null, null, 'openai', 'gpt-4o', now, 'succeeded')
    sqlite
      .prepare(
    ...
    sqlite
      ...
      .run('run-1', 'session-1', 'agent-1', 'main', 'completed', 1, '{}', 'request-3', now)
    sqlite
      .prepare(
        `INSERT INTO ai_model_calls
          (id, request_id, user_id, scenario, conversation_id, generation_id,
      ...
      .run('call-run', 'request-3', 'user-1', 'agent_run', null, null, 'run-1', 'openai', 'gpt-4o', now, 'succeeded')
    sqlite
      .prepare(
    ...
    for (const removed of ['ai_conversations', 'ai_conversation_messages', 'ai_generations']) {
    ...
    expect(
      (
        sqlite
          .prepare(`SELECT COUNT(*) AS count FROM ai_model_calls WHERE scenario = 'legacy' AND run_id IS NULL`)
          .get() as { count: number }
      ).count,
    ).toBe(1)
    expect(
      (
        sqlite.prepare(`SELECT COUNT(*) AS count FROM ai_model_calls WHERE scenario = 'model_test'`).get() as {
          count: number
        }
      ).count,
    ).toBe(1)
    expect(
      (
        sqlite
          .prepare(`SELECT COUNT(*) AS count FROM ai_model_calls WHERE scenario = 'agent_run' AND run_id = 'run-1'`)
          .get() as { count: number }
      ).count,
    ).toBe(1)

    for (const table of ['ai_agent_runs', 'ai_system_prompts', 'ai_skills', 'ai_tool_executions']) {
      expect(
        (
    ...
    const schemaSql = (
      sqlite.prepare("SELECT sql FROM sqlite_master WHERE name = 'ai_model_calls'").get() as { sql: string }
    ).sql
