...
it('registry 按 name@version 唯一并校验 scope', () => {
  ...
  expect(() => defineAiTool({ ...base, version: 'latest' })).toThrow('版本无效')

  // 精确版本查找：同名两个版本互不干扰，无版本参数已从类型中移除
  expect(registry.find({ name: 'lookup', version: '1.0.0' })?.version).toBe('1.0.0')
  expect(registry.find({ name: 'lookup', version: '2.0.0' })?.version).toBe('2.0.0')
  expect(registry.find({ name: 'lookup', version: '3.0.0' })).toBeUndefined()
  expect(registry.require({ name: 'lookup', version: '1.0.0' }).version).toBe('1.0.0')
  expect(() => registry.require({ name: 'lookup', version: '9.0.0' })).toThrow('lookup@9.0.0')
})
