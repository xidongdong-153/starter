import type { AccountProfile, CurrentPermissions, FileItem } from '@starter/contracts'
...
async function runMatrixCase(ctx: MatrixContext, tc: MatrixCase) {
  ...
  const response = await ctx.app.request(typeof tc.path === 'function' ? tc.path(ctx) : tc.path, {
    method: tc.method,
    headers,
...
async function readFileList(app: ReturnType<typeof createTestApp>['app'], cookie: string): Promise<FileItem[]> {
  const response = await app.request('/api/files', { headers: { cookie } })
  expect(response.status).toBe(200)
  return (await readSuccess<FileItem[]>(response)).data
}

async function readProfile(app: ReturnType<typeof createTestApp>['app'], cookie: string): Promise<AccountProfile> {
  const response = await app.request('/api/profile', { headers: { cookie } })
  expect(response.status).toBe(200)
  return (await readSuccess<AccountProfile>(response)).data
}

function readRoleKeys(db: ReturnType<typeof createTestApp>['runtime']['db'], userId: string) {
  return db
...
/** 被拒的删除后文件仍存在。 */
function fileStillExists(file: 'fileA' | 'fileB', owner: Exclude<ActorKey, 'anonymous'>) {
...
/** 被拒的重命名后文件名不变。 */
function fileNameUnchanged(file: 'fileA' | 'fileB', owner: Exclude<ActorKey, 'anonymous'>, name: string) {
  return async (ctx: MatrixContext) => {
    const list = await readFileList(ctx.app, ctx.cookies[owner])
...
const matrixCases: MatrixCase[] = [
  ...
  },
  {
    ...
    verifyUnchanged: async (ctx) => {
      ...
      expect((await readSuccess<FileItem>(restored)).data.name).toBe(AVATAR_FILE_NAME)
    },
  },
  {
  ...
  },
  {
    ...
    expectedStatus: 200,
    verifyUnchanged: async (ctx) => {
      const restored = await ctx.app.request(`/api/authorization/users/${ctx.userIds.ownerA}/roles`, {
        method: 'PUT',
        headers: {
  ...
  },
  {
    ...
    expectedCode: ApiErrorCodes.AUTH_FORBIDDEN,
    verifyUnchanged: (ctx) => {
      expect(ctx.db.select().from(roles).where(eq(roles.key, MATRIX_WRITE_ROLE)).get()).toBeUndefined()
    },
  },
  {
  ...
  },
  {
    ...
    expectedStatus: 200,
    verifyUnchanged: async (ctx) => {
      const restored = await ctx.app.request('/api/authorization/roles/viewer/permissions', {
        method: 'PUT',
        headers: {
          cookie: ctx.cookies.admin,
          'Content-Type': 'application/json',
...
beforeAll(async () => {
  ...
  expect(repository.replaceUserRoles(viewer.user.id, [RoleKeys.VIEWER], systemContext).kind).toBe('ok')
  expect(
  ...
  expect(
    repository.replaceUserRoles(powerUser.user.id, [RoleKeys.OPERATOR, MATRIX_CONTROL_ROLE], systemContext).kind,
  ).toBe('ok')
  expect(repository.bootstrapAdminByEmail('matrix-admin@example.com', systemContext).kind).toBe('ok')

  const fileA = await uploadFile(app, ownerA.cookie, AVATAR_FILE_NAME, AVATAR_BYTES, 'image/png')
  const fileB = await uploadFile(app, ownerB.cookie, NOTES_FILE_NAME, NOTES_BYTES, 'text/plain')
...
it('admin 特权语义：删除 admin 角色全部 role_permissions 行后权限与访问能力不变', async () => {
  const adminRole = ctx.db.select({ id: roles.id }).from(roles).where(eq(roles.key, RoleKeys.ADMIN)).get()
  expect(adminRole).toBeDefined()
  ctx.db.delete(rolePermissions).where(eq(rolePermissions.roleId, adminRole!.id)).run()
  expect(ctx.db.select().from(rolePermissions).where(eq(rolePermissions.roleId, adminRole!.id)).all()).toHaveLength(0)
  ...
  const replace = await ctx.app.request(`/api/authorization/users/${ctx.userIds.ownerA}/roles`, {
    method: 'PUT',
    headers: {
      cookie: ctx.cookies.admin,
      'Content-Type': 'application/json',
  ...
  expect(readRoleKeys(ctx.db, ctx.userIds.ownerA)).toEqual([RoleKeys.OPERATOR, RoleKeys.VIEWER])
