...
import { appendSkillDescriptions, createReadSkillTool } from '@api/modules/ai/skill/skill-tools.js'
import { createAiAgentDefinitionRepository, createAiAgentDefinitionService } from '@api/modules/ai/agent/index.js'
...
it('技能 CRUD：创建、详情、更新、删除、列表不含 content', async () => {
  ...
  try {
    ...
    const detail = await getJson(app, `/api/ai/skills/${skill.data.id}`, admin.cookie)
    expect(detail.status).toBe(200)
    const detailBody = await readSuccess<{ content: string }>(detail)
    expect(detailBody.data.content).toContain('SQL 优化专家')

    const updated = await putJson(app, `/api/ai/skills/${skill.data.id}`, admin.cookie, {
    ...
    const listBody = await readSuccess<{ name: string; content?: string }[]>(list)
    expect(listBody.data).toHaveLength(1)
    expect(listBody.data[0]).toEqual(expect.objectContaining({ name: 'sql-expert' }))
...
it('appendSkillDescriptions：注入 XML 块并转义，无技能时原样返回', () => {
  ...
  expect(appendSkillDescriptions('base', skills)).toContain('base\n\n<available_skills>')
})
...
it('agent.resolve：当 Agent 配置了启用技能时，systemPrompt 自动注入 <available_skills> 描述块', async () => {
  ...
  try {
    ...
    await patchJson(app, `/api/ai/admin/agents/${agentBody.data.id}/status`, admin.cookie, {
      status: 'enabled',
    })
    ...
    const agentService = createAiAgentDefinitionService({
      ...
      promptService: createAiPromptService(createAiPromptRepository(runtime.db)),
    ...
    expect(resolved.systemPrompt).toContain('<description>SQL 性能优化技能</description>')
    expect(resolved.systemPrompt).toContain('</available_skills>')
  } finally {
    cleanup()
  }
