...
import { authorizationAuditEvents, permissions, rolePermissions, roles } from '@api/infra/db/schema/index.js'
...
function readRoleUpdatedAt(db: TestDb, roleKey: string) {
  const row = db.select({ updatedAt: roles.updatedAt }).from(roles).where(eq(roles.key, roleKey)).get()
...
async function setupAdmin(app: TestApp['app'], db: TestDb, email: string) {
  ...
  expect(repository.bootstrapAdminByEmail(email, systemContext).kind).toBe('ok')
  return admin
}
...
it('创建自定义角色并在同一 transaction 写初始 permission 和 role.created', async () => {
  ...
  try {
    ...
    const emptyRole = (await readSuccess<AuthorizationRole>(emptyResponse)).data
    ...
    expect(role.permissionKeys).toEqual([PermissionKeys.FILE_LIST, PermissionKeys.FILE_READ])
...
it('重复 key 返回 409 且不产生角色和审计事件', async () => {
  ...
  try {
    ...
    expect((await readFailure(activeConflict)).error.code).toBe(ApiErrorCodes.AUTH_ROLE_KEY_CONFLICT)

    // 与归档角色冲突：key 不因归档释放
    expect((await create('temp-role')).status).toBe(200)
    const archive = await app.request('/api/authorization/roles/temp-role/archive', {
      method: 'POST',
      headers: jsonHeaders(admin.cookie),
    })
    ...
    const roleRows = runtime.db.select({ key: roles.key }).from(roles).where(eq(roles.key, 'temp-role')).all()
...
it('无效 permission 创建返回 400，角色和审计整体回滚', async () => {
  ...
  try {
    ...
    expect(runtime.db.select().from(roles).where(eq(roles.key, 'bad-role')).all()).toHaveLength(0)
...
it('归档 permission 创建返回 400，角色、关系和审计都不变', async () => {
  const { app, cleanup, runtime } = createTestApp()
  try {
    const admin = await setupAdmin(app, runtime.db, 'rl-archived-permission@example.com')
    ...
    runtime.db.update(permissions).set({ archivedAt: new Date() }).where(eq(permissions.id, permission!.id)).run()

    const relationRowsBefore = runtime.db.select().from(rolePermissions).all()
    const auditRowsBefore = runtime.db.select().from(authorizationAuditEvents).all()
    const response = await app.request('/api/authorization/roles', {
      method: 'POST',
      headers: jsonHeaders(admin.cookie),
      body: JSON.stringify({
    ...
    expect(runtime.db.select().from(roles).where(eq(roles.key, 'archived-permission-role')).all()).toHaveLength(0)
    expect(runtime.db.select().from(rolePermissions).all()).toHaveLength(relationRowsBefore.length)
    expect(runtime.db.select().from(authorizationAuditEvents).all()).toHaveLength(auditRowsBefore.length)
  } finally {
    cleanup()
  }
})
...
it('创建角色时 role_permissions 写入失败会回滚角色和审计', async () => {
  ...
  try {
    ...
    expect(runtime.db.select().from(roles).where(eq(roles.key, 'relation-rollback-role')).all()).toHaveLength(0)
    expect(runtime.db.select().from(rolePermissions).all()).toHaveLength(relationRowsBefore.length)
    expect(readRoleAuditRows(runtime.db, 'relation-rollback-role')).toHaveLength(0)
  } finally {
    cleanup()
  }
})
...
it('创建角色时审计写入失败会回滚角色和 permission 关系', async () => {
  ...
  try {
    ...
    const role = runtime.db.select({ id: roles.id }).from(roles).where(eq(roles.key, 'audit-rollback-role')).get()
    expect(role).toBeUndefined()
    expect(runtime.db.select().from(rolePermissions).all()).toHaveLength(relationRowsBefore.length)
    expect(readRoleAuditRows(runtime.db, 'audit-rollback-role')).toHaveLength(0)
  } finally {
    cleanup()
  }
})
...
it('持有 authorization:manage 的非 admin 对所有新增写接口仍返回 403', async () => {
  ...
  try {
    ...
    const role = runtime.db.select({ id: roles.id }).from(roles).where(eq(roles.key, RoleKeys.OPERATOR)).get()
    const permission = runtime.db
      .select({ id: permissions.id })
      .from(permissions)
    ...
    const attempts: [string, RequestInit][] = [
      ...
      ['/api/authorization/roles/guarded/archive', { method: 'POST', headers: jsonHeaders(operator.cookie) }],
      ['/api/authorization/roles/guarded/restore', { method: 'POST', headers: jsonHeaders(operator.cookie) }],
    ]
    for (const [path, init] of attempts) {
      const response = await app.request(path, init)
      expect(response.status).toBe(403)
      expect((await readFailure(response)).error.code).toBe(ApiErrorCodes.AUTH_FORBIDDEN)
    }
    // 角色未被改动
    expect(
      runtime.db
    ...
    expect(runtime.db.select().from(roles).where(eq(roles.key, 'sneaky')).all()).toHaveLength(0)
  } finally {
    cleanup()
  }
})
...
it('metadata PATCH 支持部分更新、幂等短路、系统角色和归档角色拒绝', async () => {
  ...
  try {
    ...
    runtime.db.update(roles).set({ updatedAt: idempotentUpdatedAt }).where(eq(roles.key, 'editor')).run()
    ...
    expect(readRoleUpdatedAt(runtime.db, 'editor')).toEqual(idempotentUpdatedAt)

    const auditRows = readRoleAuditRows(runtime.db, 'editor')
    const updatedRows = auditRows.filter((row) => row.action === AuditActions.ROLE_UPDATED)
...
it('归档、恢复与用户分配保护', async () => {
  ...
  try {
    ...
    // 分配用户后归档返回 409 和数量
    const assign = await app.request(`/api/authorization/users/${member.user.id}/roles`, {
      method: 'PUT',
      headers: jsonHeaders(admin.cookie),
      body: JSON.stringify({ roleKeys: ['temp-team'] }),
    })
    expect(assign.status).toBe(200)
    const secondAssign = await app.request(`/api/authorization/users/${secondMember.user.id}/roles`, {
      method: 'PUT',
      headers: jsonHeaders(admin.cookie),
      body: JSON.stringify({ roleKeys: ['temp-team'] }),
    })
    ...
    const blocked = await app.request('/api/authorization/roles/temp-team/archive', {
      method: 'POST',
      headers: jsonHeaders(admin.cookie),
    })
    ...
    // role impact 与分配一致
    const impact = await app.request('/api/authorization/roles/temp-team/impact', { headers: { cookie: admin.cookie } })
    ...
    await app.request(`/api/authorization/users/${secondMember.user.id}/roles`, {
      method: 'PUT',
      headers: jsonHeaders(admin.cookie),
      body: JSON.stringify({ roleKeys: [RoleKeys.VIEWER] }),
    })
    const archived = await app.request('/api/authorization/roles/temp-team/archive', {
      method: 'POST',
      headers: jsonHeaders(admin.cookie),
    })
    ...
    const assignArchived = await app.request(`/api/authorization/users/${member.user.id}/roles`, {
      method: 'PUT',
      headers: jsonHeaders(admin.cookie),
      body: JSON.stringify({ roleKeys: ['temp-team'] }),
    })
    expect(assignArchived.status).toBe(400)
    const replaceArchived = await app.request('/api/authorization/roles/temp-team/permissions', {
      method: 'PUT',
      headers: jsonHeaders(admin.cookie),
    ...
    // 归档目录能查到它，活动目录查不到
    const archivedCatalog = await app.request('/api/authorization/roles?status=archived', {
      headers: { cookie: admin.cookie },
    })
    const archivedData = (await readSuccess<AuthorizationRoleCatalog>(archivedCatalog)).data
    expect(archivedData.roles.map((role) => role.key)).toEqual(['temp-team'])
    expect(archivedData.roles[0]!.permissionKeys).toEqual([PermissionKeys.FILE_LIST])
    const activeCatalog = await app.request('/api/authorization/roles', {
      headers: { cookie: admin.cookie },
    })
    expect(
      (await readSuccess<AuthorizationRoleCatalog>(activeCatalog)).data.roles.map((role) => role.key),
    ).not.toContain('temp-team')

    // 重复归档幂等，不写事件，也不刷新 updatedAt
    const archivedUpdatedAt = new Date('2001-01-01T00:00:00.000Z')
    runtime.db.update(roles).set({ updatedAt: archivedUpdatedAt }).where(eq(roles.key, 'temp-team')).run()
    const rearchive = await app.request('/api/authorization/roles/temp-team/archive', {
      method: 'POST',
      headers: jsonHeaders(admin.cookie),
    })
    expect(rearchive.status).toBe(200)
    expect(readRoleUpdatedAt(runtime.db, 'temp-team')).toEqual(archivedUpdatedAt)

    // 恢复保留 permission
    const restored = await app.request('/api/authorization/roles/temp-team/restore', {
      method: 'POST',
      headers: jsonHeaders(admin.cookie),
    })
    ...
    runtime.db.update(roles).set({ updatedAt: restoredUpdatedAt }).where(eq(roles.key, 'temp-team')).run()
    const rerestore = await app.request('/api/authorization/roles/temp-team/restore', {
      method: 'POST',
      headers: jsonHeaders(admin.cookie),
    })
    expect(rerestore.status).toBe(200)
    expect(readRoleUpdatedAt(runtime.db, 'temp-team')).toEqual(restoredUpdatedAt)
    ...
    expect(auditRows.every((row) => typeof row.requestId === 'string')).toBe(true)
...
it('permission impact 合并 admin 自动权限并对用户去重', async () => {
  ...
  try {
    ...
    const assign = await app.request(`/api/authorization/users/${multi.user.id}/roles`, {
      method: 'PUT',
      headers: jsonHeaders(admin.cookie),
      body: JSON.stringify({
    ...
    const response = await app.request(`/api/authorization/permissions/${PermissionKeys.FILE_LIST}/impact`, {
      headers: { cookie: admin.cookie },
    })
    expect(response.status).toBe(200)
    const impact = (await readSuccess<AuthorizationPermissionImpact>(response)).data
    // admin 没有 file:list 的 role_permissions 行，但必须出现在有效角色里
    expect(impact.roleKeys).toEqual([RoleKeys.ADMIN, RoleKeys.OPERATOR, RoleKeys.VIEWER])
    ...
    const auditImpactData = (await readSuccess<AuthorizationPermissionImpact>(auditImpact)).data
...
it('新 action 的损坏 payload 或 targetType 返回 500', async () => {
  ...
  try {
    ...
    runtime.db
      ...
      .where(eq(authorizationAuditEvents.id, '019c3e00-0020-7000-8000-000000000001'))
      .run()
    runtime.db
      .insert(authorizationAuditEvents)
      .values({
        ...
        afterJson: '{"role":{"name":"broken","description":null,"permissionKeys":[],"archived":false}}',
        reason: null,
        requestId: null,
        createdAt: new Date(),
      })
    ...
    const targetResponse = await app.request('/api/authorization/audit-events', { headers: { cookie: admin.cookie } })
    expect(targetResponse.status).toBe(500)
    expect((await readFailure(targetResponse)).error.code).toBe(ApiErrorCodes.SYSTEM_INTERNAL_ERROR)
  } finally {
    cleanup()
  }
})
