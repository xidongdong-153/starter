...
async function listDeliveries(
  ...
  const response = await app.request(
    `/api/ai/admin/webhook-deliveries?${new URLSearchParams(query).toString()}`,
    { headers: { Cookie: cookie } },
  );
  expect(response.status).toBe(200);
  return (await readSuccess<{ items: DeliveryView[]; total: number }>(response))
    .data;
}

async function startReceiver(
  respond?: (requestCount: number) => number,
): Promise<{
  url: string;
...
async function pollTerminalRun(
  ...
  for (let attempt = 0; attempt < 100; attempt += 1) {
    ...
    expect(response.status).toBe(200);
    const run = (
      await readSuccess<{ status: string; finishedAt: string | null }>(response)
    ).data;
...
async function runOnce(
  ...
  const runId = await startProductRun(
    context.app,
    context.credential.secret,
    sessionId,
    context.agentId,
  );
  const run = await pollTerminalRun(
    context.app,
    context.credential.secret,
    sessionId,
    runId,
...
function verifySignature(
  ...
  signingSecret: string,
): { timestampSec: string; valid: boolean } {
  ...
  const expected = createHmac("sha256", signingSecret)
    .update(`${timestampSec}.${request.body}`, "utf8")
    .digest("hex");
...
it("admin CRUD：创建返回一次 secret，列表不含 secret，可改 URL/状态、rotate、删除", async () => {
  ...
  try {
    ...
    try {
      ...
      const list =
        await readSuccess<Array<Record<string, unknown>>>(listResponse);
      ...
      const patched = await context.app.request(
        `/api/ai/admin/webhook-endpoints/${created.endpointId}`,
        {
          method: "PATCH",
          headers: {
      ...
      const rotateResponse = await context.app.request(
        `/api/ai/admin/webhook-endpoints/${created.endpointId}/rotate`,
        { method: "POST", headers: { Cookie: context.admin.cookie } },
      );
      expect(rotateResponse.status).toBe(200);
      const rotated = await readSuccess<{ signingSecret: string }>(
        rotateResponse,
      );
      ...
      const deleteResponse = await context.app.request(
        `/api/ai/admin/webhook-endpoints/${created.endpointId}`,
        { method: "DELETE", headers: { Cookie: context.admin.cookie } },
      );
      expect(deleteResponse.status).toBe(200);

      const afterDelete = await context.app.request(
        `/api/ai/admin/webhook-endpoints/${created.endpointId}/rotate`,
        { method: "POST", headers: { Cookie: context.admin.cookie } },
      );
      expect(afterDelete.status).toBe(404);
      expect((await readFailure(afterDelete)).error.code).toBe(
        "AI.WEBHOOK_ENDPOINT_NOT_FOUND",
      );

      // 不存在的 appId 创建端点返回 404。
      const missingApp = await context.app.request(
        "/api/ai/admin/webhook-endpoints",
        {
          method: "POST",
          headers: {
            Cookie: context.admin.cookie,
      ...
      expect((await readFailure(missingApp)).error.code).toBe(
        "AI.APP_CREDENTIAL_NOT_FOUND",
      );
    } finally {
      context.cleanup();
    }
...
it("加密密钥不可用时创建与 rotate 返回 503 AI.CREDENTIAL_KEY_UNAVAILABLE", async () => {
  ...
  try {
    ...
    expect((await readFailure(createResponse)).error.code).toBe(
      "AI.CREDENTIAL_KEY_UNAVAILABLE",
    );
    ...
    context.runtime.db
      ...
      .run();
    const rotateResponse = await context.app.request(
      `/api/ai/admin/webhook-endpoints/${endpointId}/rotate`,
      { method: "POST", headers: { Cookie: context.admin.cookie } },
    );
    expect(rotateResponse.status).toBe(503);
    expect((await readFailure(rotateResponse)).error.code).toBe(
      "AI.CREDENTIAL_KEY_UNAVAILABLE",
    );
  } finally {
    context.cleanup();
...
it("端到端：Run 终态推送带可验证签名，payload 与 Run 终态一致，投递记录转 delivered", async () => {
  ...
  try {
    ...
    try {
      ...
      const list =
        await readSuccess<
          Array<{ endpointId: string; lastDeliveryAt: string | null }>
        >(listResponse);
  ...
  try {
    ...
    try {
      ...
      const disable = await context.app.request(
        `/api/ai/admin/webhook-endpoints/${endpointId}`,
        {
          method: "PATCH",
          headers: {
            Cookie: context.admin.cookie,
      ...
      const emptyPage = await listDeliveries(
        context.app,
        context.admin.cookie,
        {
          endpointId,
...
it("aI_WEBHOOK_ENABLED=false 时无任何投递行为，管理面 CRUD 仍可用", async () => {
  ...
  try {
    ...
    try {
      ...
      expect(
        context.runtime.db.select().from(aiWebhookDeliveries).all(),
      ).toHaveLength(0);
    } finally {
      context.cleanup();
    }
