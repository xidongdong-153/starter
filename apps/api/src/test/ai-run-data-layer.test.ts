...
function seedRun(runtime: ReturnType<typeof createTestApp>['runtime']) {
  ...
  sqlite
    ...
    .run(runId, sessionId, agentId, 'main', 'running', 1, SNAPSHOT, 'request-1', now)
  sqlite
    .prepare(
...
describe('主库 JSON 列读取', () => {
  ...
  it('被破坏的 value_json 读取时报数据损坏', () => {
    ...
    try {
      ...
      try {
        ...
        expect(error).toBeInstanceOf(StoredJsonError)
        expect((error as StoredJsonError).column).toBe('ai_structured_outputs.value_json')
        expect((error as StoredJsonError).reason).toBe('schema_mismatch')
      }
    } finally {
      test.cleanup()
  ...
  it('被破坏的 config_json 读取时报数据损坏', () => {
    expect(() => parseAgentDefinitionConfig('{not json')).toThrowError(StoredJsonError)
    expect(() => parseAgentDefinitionConfig(JSON.stringify({ schemaVersion: 2 }))).toThrowError(StoredJsonError)
    expect(parseAgentDefinitionConfig(CONFIG).maxTurns).toBe(8)
  })

  it('数据损坏错误只带列名、原因和字段路径，不带被拒绝的值', () => {
    try {
      parseAgentDefinitionConfig(JSON.stringify({ schemaVersion: 2, maxTurns: 99 }))
      throw new Error('应当抛出数据损坏错误')
    } catch (error) {
      expect(error).toBeInstanceOf(StoredJsonError)
      const stored = error as StoredJsonError
...
describe('run event sequence 并发', () => {
  it('同一 Run 并发 append 时 sequence 连续、不重复、不跳号', async () => {
    ...
    try {
      ...
      const repository = createAiRunEventRepository(test.runtime.db)
      const total = 64
      const events = await Promise.all(Array.from({ length: total }, async () => repository.append(eventDraft())))
      ...
      expect(repository.listAfter(runId, 0, total + 1).map((event) => event.sequence)).toEqual(
...
describe('内联配置 Run 行与快照兼容', () => {
  ...
  it('快照 schema 同时接受 v2 与 v3，成对约束生效', () => {
    expect(agentRunSnapshotSchema.safeParse(JSON.parse(SNAPSHOT)).success).toBe(true)
    expect(agentRunSnapshotSchema.safeParse(JSON.parse(SNAPSHOT_V3)).success).toBe(true)
    // v2 不允许空 agentId
    expect(
