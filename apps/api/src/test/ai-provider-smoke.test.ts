...
function completedEvent(text = 'hello'): Extract<AiGatewayEvent, { type: 'completed' }> {
  return {
    type: 'completed',
    turnIndex: 0,
    assistantMessage: {
      role: 'assistant',
      blocks: [{ type: 'text', text, turnIndex: 0, contentIndex: 0, blockId: '0:0' }],
    },
    stopReason: 'stop',
    usage,
    cost: null,
  ...
  try {
    ...
    expect(lines).toEqual(
      expect.arrayContaining([
        ...
        expect.stringMatching(/^provider=openai model=.+ event=text_delta count=1$/),
...
it('上游分类：auth、timeout、aborted、upstream 分别映射稳定错误码', async () => {
  ...
  try {
    ...
    for (const item of cases) {
      ...
      expect(JSON.stringify(output.log.mock.calls)).toContain(`error=${item.expected}`)
    }
...
it('缺 smoke 变量或没有终态时失败，且错误输出不含 prompt', async () => {
  ...
  try {
    const output = { log: vi.fn(), error: vi.fn() }
    const missing = await runAiProviderSmoke({ ...env, AI_SMOKE_PROMPT: '  ' }, output, {
      aiGateway: successfulGateway(),
    })
    expect(missing).toBe(1)
    expect(JSON.stringify(output.error.mock.calls)).toContain('缺少或无效的 AI smoke 环境变量')
    ...
    expect(JSON.stringify(output.log.mock.calls)).toContain('error=upstream_failed')
    expect(JSON.stringify(output.log.mock.calls)).not.toContain(promptMarker)
    expect(JSON.stringify(output.log.mock.calls)).not.toContain(responseMarker)
  } finally {
    cleanup()
...
it('显式 checkAuth 但未配置凭据时返回配置说明', async () => {
  ...
  try {
    const output = { log: vi.fn(), error: vi.fn() }
    const exitCode = await runAiProviderSmoke({ ...env, AI_SMOKE_CHECK_AUTH: 'true' }, output, {
      aiGateway: successfulGateway(),
    })
    expect(exitCode).toBe(1)
    expect(JSON.stringify(output.error.mock.calls)).toContain('认证未配置或不可用')
  } finally {
    cleanup()
  }
})
