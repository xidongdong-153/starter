...
describe('ai:auth command', () => {
  it('拒绝命令行 secret 和未知参数', async () => {
    ...
    await expect(runAiAuth(['openai-codex', '--api-key=secret'], interaction, {}, output)).resolves.toBe(1)
  ...
  it('缺少加密主密钥时不会启动 OAuth 流程', async () => {
    ...
    expect(interaction.prompt).not.toHaveBeenCalled()
    expect(output.error).toHaveBeenCalledWith(expect.stringContaining('AI_CREDENTIAL_ENCRYPTION_KEY'))
  })
})
