...
it("agentDefinition CRUD、revision、状态和公开边界可用", async () => {
  ...
  try {
    ...
    expect(
      (await readSuccess<{ items: unknown[]; total: number }>(empty)).data,
    ).toMatchObject({
      items: [],
      total: 0,
      page: 1,
    ...
    expect((await readFailure(duplicate)).error.code).toBe(
    ...
    expect(
      (await readSuccess<{ revision: number }>(renamed)).data.revision,
    ).toBe(1);
    ...
    const configured = await patchJson(
      app,
      `/api/ai/admin/agents/${createdBody.data.id}`,
      admin.cookie,
      {
    ...
    expect(
      (await readSuccess<{ revision: number }>(configured)).data.revision,
    ).toBe(2);
    ...
    expect(
      (await readSuccess<{ revision: number }>(sameConfigDifferentObjectOrder))
        .data.revision,
    ).toBe(2);
    ...
    expect(concurrentUpdates.map((response) => response.status)).toEqual([
      200, 200,
    ]);
    const afterConcurrentUpdates = await app.request(
      `/api/ai/admin/agents/${createdBody.data.id}`,
      { headers: { cookie: admin.cookie } },
    );
    expect(
      (await readSuccess<{ revision: number }>(afterConcurrentUpdates)).data
        .revision,
    ).toBe(4);
    ...
    expect(
      (await readSuccess<{ revision: number }>(enabled)).data.revision,
    ).toBe(4);
    ...
    const publicDetail = await app.request(
      `/api/ai/agents/${createdBody.data.id}`,
      { headers: { cookie: user.cookie } },
    );
    expect(publicDetail.status).toBe(200);
    expect(
      (await readSuccess<Record<string, unknown>>(publicDetail)).data,
    ...
    expect(
      (await readSuccess<{ revision: number }>(disabled)).data.revision,
    ).toBe(4);
    const publicAfterDisable = await app.request("/api/ai/agents", {
      headers: { cookie: user.cookie },
    });
    expect(
      (await readSuccess<{ items: unknown[] }>(publicAfterDisable)).data.items,
    ).toHaveLength(0);
...
it("agentDefinition 拒绝无效资源，并阻止被引用的 System Prompt 删除", async () => {
  ...
  try {
    ...
    for (const [name, config] of [
      [
        "invalid-model",
        { model: { providerId: "openai", modelId: "missing-model" } },
      ],
      [
        "invalid-prompt",
        { model, systemPromptId: "01900000-0000-7000-8000-000000000001" },
      ],
      [
        "invalid-tool",
        {
    ...
    const invalidSkill = await postJson(
      app,
      "/api/ai/admin/agents",
      admin.cookie,
      {
    ...
    expect((await readFailure(invalidSkill)).error.code).toBe(
      ApiErrorCodes.AI_AGENT_CONFIG_INVALID,
    );

    const referenced = await postJson(
      app,
      "/api/ai/admin/agents",
      admin.cookie,
      {
    ...
    expect((await readFailure(deleted)).error.code).toBe(
    ...
    const enableIncomplete = await patchJson(
      app,
      `/api/ai/admin/agents/${draftBody.data.id}/status`,
      admin.cookie,
      { status: "enabled" },
    );
