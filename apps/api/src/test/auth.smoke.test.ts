...
it('未登录、请求校验和 404 返回稳定错误', async () => {
  ...
  try {
    for (const path of ['/api/profile', '/api/files']) {
      ...
      expect((await readFailure(response)).error.code).toBe(ApiErrorCodes.AUTH_UNAUTHENTICATED)
    }

    const invalid = await app.request('/api/profiles/not-a-uuid')
    expect(invalid.status).toBe(400)
    expect((await readFailure(invalid)).error.code).toBe(ApiErrorCodes.COMMON_INVALID_REQUEST)

    const missing = await app.request('/does-not-exist')
    expect(missing.status).toBe(404)
    expect((await readFailure(missing)).error.code).toBe(ApiErrorCodes.COMMON_NOT_FOUND)
  } finally {
    cleanup()
  }
})
