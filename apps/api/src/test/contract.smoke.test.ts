...
it("health 成功响应符合共享 success envelope schema", async () => {
  ...
  try {
    ...
    const envelope = apiSuccessSchema(healthSchema).parse(
      await response.json(),
    );
...
it("用户状态更新响应包含 from 字段并通过共享 schema", async () => {
  ...
  try {
    ...
    const envelope = apiSuccessSchema(updateUserStatusResponseSchema).parse(
      await response.json(),
    );
...
it("审计列表包含 user.status_changed 事件并通过共享 schema", async () => {
  ...
  try {
    ...
    const auditResponse = await app.request(
      "/api/authorization/audit-events?page=1&pageSize=20",
      { headers: { cookie: admin.cookie } },
    );
    expect(auditResponse.status).toBe(200);

    const envelope = apiSuccessSchema(authorizationAuditEventPageSchema).parse(
      await auditResponse.json(),
    );
    const event = envelope.data.items.find(
      (item) => item.action === "user.status_changed",
    );
...
it("公开资料响应通过共享 schema（avatarUrl 接受相对路径）", async () => {
  ...
  try {
    ...
    const envelope = apiSuccessSchema(publicProfileSchema).parse(
      await response.json(),
    );
