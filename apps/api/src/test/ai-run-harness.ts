...
export function streamProviderError(): ReturnType<typeof createAssistantMessageEventStream> {
  const stream = createAssistantMessageEventStream()
...
export function modelsWith(
  ...
  return {
    ...
    streamSimple,
    ...(overrides.completeSimple ? { completeSimple: overrides.completeSimple } : {}),
  } as unknown as Models
}

export function seedEnabledModel(runtime: ReturnType<typeof createTestApp>['runtime']): void {
  const now = new Date()
  runtime.db
...
export async function createSessionId(app: ReturnType<typeof createTestApp>['app'], cookie: string): Promise<string> {
      ...
      piAgentExecutorFactory: (runtime) => {
        ...
        return createPiAgentExecutor({
          ...
          models: modelsWith(input.streamSimple, {
            model,
            ...(input.completeSimple ? { completeSimple: input.completeSimple } : {}),
          }),
          resolveModel: () => model,
