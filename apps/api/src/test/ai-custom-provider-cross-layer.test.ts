...
it.each(protocols)('%s 通过 Admin 生命周期、模型测试和 Agent Run，并写入 agent_run 审计', async (protocol) => {
  ...
  try {
    ...
    expect(
      createAuthorizationRepository(runtime.db).bootstrapAdminByEmail(`${providerId}-admin@example.com`, systemContext)
        .kind,
    ).toBe('ok')
    ...
    const checkResponse = await app.request(`/api/ai/admin/custom-providers/${providerId}/check`, {
      method: 'POST',
      headers: {
        cookie: admin.cookie,
        'Content-Type': 'application/json',
    ...
    const enableResponse = await app.request(`/api/ai/admin/custom-providers/${providerId}/state`, {
      method: 'PUT',
      headers: {
        cookie: admin.cookie,
        'Content-Type': 'application/json',
    ...
    expect((await readSuccess<{ enabled: boolean }>(enableResponse)).data.enabled).toBe(true)
    ...
    expect((await readSuccess<Array<{ providerId: string; modelId: string }>>(userModelsResponse)).data).toEqual([
    ...
    const enableAgentResponse = await app.request(`/api/ai/admin/agents/${agent.data.id}/status`, {
      method: 'PATCH',
      headers: {
        cookie: admin.cookie,
        'Content-Type': 'application/json',
    ...
    const runResponse = await app.request(`/api/ai/sessions/${session.data.id}/runs`, {
      method: 'POST',
      headers: {
        cookie: user.cookie,
        'Content-Type': 'application/json',
    ...
    const disableResponse = await app.request(`/api/ai/admin/custom-providers/${providerId}/state`, {
      method: 'PUT',
      headers: {
        cookie: admin.cookie,
        'Content-Type': 'application/json',
    ...
    expect((await readSuccess<{ enabled: boolean }>(disableResponse)).data.enabled).toBe(false)
    expect(
      (
    ...
    const blockedDelete = await app.request(`/api/ai/admin/custom-providers/${providerId}`, {
      method: 'DELETE',
      headers: {
        cookie: admin.cookie,
        'Content-Type': 'application/json',
    ...
    expect((await readFailure(blockedDelete)).error.code).toBe(ApiErrorCodes.AI_CUSTOM_PROVIDER_IN_USE)

    const disableAgentResponse = await app.request(`/api/ai/admin/agents/${agent.data.id}/status`, {
      method: 'PATCH',
      headers: {
        cookie: admin.cookie,
        'Content-Type': 'application/json',
    ...
    const clearReferenceResponse = await app.request(`/api/ai/admin/agents/${agent.data.id}`, {
      method: 'PATCH',
      headers: {
        cookie: admin.cookie,
        'Content-Type': 'application/json',
    ...
    const deleteResponse = await app.request(`/api/ai/admin/custom-providers/${providerId}`, {
      method: 'DELETE',
      headers: {
        cookie: admin.cookie,
        'Content-Type': 'application/json',
    ...
    expect(
      runtime.db.select().from(aiCustomProviders).where(eq(aiCustomProviders.providerId, providerId)).get(),
    ).toBeUndefined()
    expect(
      runtime.db.select().from(aiProviderConfigs).where(eq(aiProviderConfigs.providerId, providerId)).get(),
    ).toBeUndefined()
    expect(runtime.db.select().from(aiEnabledModels).where(eq(aiEnabledModels.providerId, providerId)).all()).toEqual(
      [],
    )
    expect(
      runtime.db
        ...
        .from(aiSettings)
        .all()
        .every((row) => row.globalProviderId !== providerId && row.globalModelId !== model.modelId),
    ).toBe(true)
    expect(
      (
        await readSuccess<Array<unknown>>(
    ...
    expect(
      runtime.db.select().from(aiAgentDefinitions).where(eq(aiAgentDefinitions.id, agent.data.id)).get(),
    ).toBeDefined()
  } finally {
    cleanup()
...
async function startFakeUpstream(protocol: Protocol): Promise<{ url: string; close: () => Promise<void> }> {
...
function writeEventSse(response: ServerResponse, event: string, data: unknown): void {
