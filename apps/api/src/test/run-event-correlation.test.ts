...
async function withRun(
  ...
  assertions: (context: { runtime: Runtime; runId: string; events: RunSseEvent[] }) => void | Promise<void>,
): Promise<void> {
  const directory = await mkdtemp(join(tmpdir(), `starter-${scenario.prefix}-`))
  ...
  const { app, cleanup, runtime } = runTestApp({
    ...
    ...(scenario.completeSimple ? { completeSimple: scenario.completeSimple } : {}),
...
function toolThenTextStream(toolCallId: string): Models['streamSimple'] {
  return ((_model: Model<Api>, context: Context, _options?: SimpleStreamOptions) => {
...
it('两轮模型调用和一次 Tool 的事件关联与 SQLite 执行记录一致', async () => {
  await withRun(
    ...
    ({ runtime, runId, events }) => {
      ...
      const turnIdByIndex = new Map(turnRows.map((row) => [row.turnIndex, row.id]))
      ...
      for (const event of events.filter((candidate) => !candidate.type.startsWith('tool.'))) {
        expect(event.toolCallId).toBeNull()
        expect(event.toolExecutionId).toBeNull()
      }

      // 两次模型调用各自一组 started / first_output / completed
      const modelEvents = events.filter((event) => event.type.startsWith('model_call.'))
      ...
      expect(new Set(modelEvents.map((event) => event.modelCallId)).size).toBe(2)
      ...
      expect(events.find((event) => event.type === 'thinking.completed')?.data).toMatchObject({
        display: true,
        summary: null,
      })
    },
...
it('model_call.first_output 每次模型调用只发一次，text、thinking 和 tool-call 首输出都能触发', async () => {
  ...
  await withRun(
    ...
    ({ events }) => {
      const started = events.filter((event) => event.type === 'model_call.started')
      const firstOutputs = events.filter((event) => event.type === 'model_call.first_output')
      ...
      const firstToolStarted = events.findIndex((event) => event.type === 'tool.started')
      const firstThinking = events.findIndex((event) => event.type === 'thinking.started')
      expect(events.indexOf(firstOutputs[0] as RunSseEvent)).toBeLessThan(firstToolStarted)
      expect(events.indexOf(firstOutputs[1] as RunSseEvent)).toBeLessThan(firstThinking)
...
it('message delta 合并后 sequence 连续，事件行数不随 token 数线性增长', async () => {
  ...
  const streamSimple = (() => {
    ...
    const done: AssistantMessage = assistantMessage([{ type: 'text', text: fullText }], 'stop')
  ...
  await withRun(
    ...
    ({ runtime, runId, events }) => {
      ...
      const maxDeltaEvents = Math.ceil(totalBytes / RUN_EVENT_MERGE_MAX_BYTES) + 1
      expect(deltaEvents.length).toBeGreaterThan(0)
      expect(deltaEvents.length).toBeLessThanOrEqual(maxDeltaEvents)
      expect(deltaEvents.length).toBeLessThan(chunkCount)
      // 合并不丢内容
      expect(deltaEvents.map((event) => event.data.delta as string).join('')).toBe(fullText)
...
it('tool 失败、tool 超时、模型失败、max turns 和 compaction 结束后都没有 running 执行记录', async () => {
  ...
  await withRun(
    ...
    ({ runtime, runId, events }) => {
      ...
      expect(events.find((event) => event.type === 'tool.completed')?.data.status).toBe('failed')
  ...
  await withRun(
    ...
    ({ runtime, runId, events }) => {
      expect(events.at(-1)?.type).toBe('run.completed')
      expect(events.find((event) => event.type === 'tool.completed')?.data.status).toBe('timed_out')
      expectNoRunningRecords(runtime, runId)
    },
  )
  ...
  await withRun(
    ...
    ({ runtime, runId, events }) => {
      ...
      expect(events.find((event) => event.type === 'model_call.failed')?.data).toMatchObject({
        error: { code: ApiErrorCodes.AI_UPSTREAM_ERROR },
      })
      expectNoRunningRecords(runtime, runId)
    },
  )
  ...
  await withRun(
    {
      ...
      streamSimple: ((_model: Model<Api>, context: Context, _options?: SimpleStreamOptions) => {
        const tools = context.tools ?? []
        if (tools.length === 0) {
          return streamAssistant(assistantMessage([{ type: 'text', text: 'wrap up' }], 'stop'), 'stop')
        }
        return streamAssistant(
          assistantMessage(
            [
...
it('事件写库失败时 Run 进入存储失败终态，客户端不会收到跳号事件和 run.completed', async () => {
  ...
  try {
    ...
    // 没有 sequence 空洞
    expect(events.map((event) => event.sequence)).toEqual(events.map((_event, index) => index + 1))
    const rows = runtime.db.select().from(aiRunEvents).where(eq(aiRunEvents.runId, runId)).all()
    expect(rows.map((row) => row.sequence).sort((a, b) => a - b)).toEqual(events.map((event) => event.sequence))
...
it('step 事件与 ai_run_steps 一一对应，assistant 和 compaction Step 都发布', async () => {
  await withRun(
    ...
    ({ runtime, runId, events }) => {
      ...
      expect(events.find((event) => event.type === 'context.compacted')?.stepId).toBe(compactionRow?.id)
      // assistant Step 的事件挂在自己那一轮，模型和 Tool 事件共用同一个 stepId
      const assistantRows = stepRows.filter((row) => row.kind === 'assistant')
      for (const row of assistantRows) {
        const modelStarted = events.find((event) => event.type === 'model_call.started' && event.stepId === row.id)
...
it('工具上报的合法 source 发布 source.available 并落库，关联字段指向当前 Tool', async () => {
  ...
  await withRun(
    ...
    ({ runtime, runId, events }) => {
      const sourceEvents = events.filter((event) => event.type === 'source.available')
      ...
      const toolRow = runtime.db.select().from(aiToolExecutions).where(eq(aiToolExecutions.runId, runId)).all()[0]
      if (!toolRow) throw new Error('缺少 Tool 执行记录')
      ...
      const startedIndex = events.findIndex((event) => event.type === 'tool.started')
      const completedIndex = events.findIndex((event) => event.type === 'tool.completed')
      for (const event of sourceEvents) {
      ...
      // 落进持久时间线，sequence 连续
      const rows = runtime.db.select().from(aiRunEvents).where(eq(aiRunEvents.runId, runId)).all()
      expect(rows.filter((row) => row.type === 'source.available')).toHaveLength(2)
      expect(events.map((event) => event.sequence)).toEqual(events.map((_event, index) => index + 1))
      // 合法上报不写安全日志，Tool 正常完成
      expect(warn).not.toHaveBeenCalled()
      expect(events.find((event) => event.type === 'tool.completed')?.data).toMatchObject({ status: 'succeeded' })
      expect(events.at(-1)?.type).toBe('run.completed')
      // source 不带 Tool 原始结果
...
it('非法 source 被丢弃并记安全日志，Tool 仍然正常完成', async () => {
  ...
  await withRun(
    ...
    ({ runtime, runId, events }) => {
      expect(events.filter((event) => event.type === 'source.available')).toHaveLength(0)
      expect(
        runtime.db
          .select()
          .from(aiRunEvents)
...
it('工具不上报 source 时没有 source.available 事件', async () => {
  await withRun(
    ...
    ({ events }) => {
      expect(events.some((event) => event.type === 'source.available')).toBe(false)
    },
  )
})
...
it('thinkingLevel 非 off 时 thinking 事件 display 为 true 且正文完整，telemetry 不带正文', async () => {
  ...
  await withRun(
    ...
    ({ runtime, runId, events }) => {
      ...
      expect(events.find((event) => event.type === 'thinking.started')?.data).toEqual({ blockIndex: 0, display: true })
      expect(events.find((event) => event.type === 'thinking.completed')?.data).toEqual({
        blockIndex: 0,
        display: true,
        summary: null,
      })
      ...
      const completedMessage = events.find((event) => event.type === 'message.completed')
      expect(completedMessage?.data.content).toBe('answer')
      // telemetry span 不带 reasoning 正文
      expect(JSON.stringify(recorder.getSpans())).not.toContain('REASONING-BODY')
      // SQLite 审计表不带 reasoning 正文
      const auditRows = runtime.db.select().from(aiModelCalls).where(eq(aiModelCalls.runId, runId)).all()
...
it('thinkingLevel 为 off 时不产生 thinking 事件', async () => {
  await withRun(
    ...
    ({ runtime, runId, events }) => {
      expect(events.filter((event) => event.type.startsWith('thinking.'))).toHaveLength(0)
      expect(
        runtime.db
          .select()
          .from(aiRunEvents)
