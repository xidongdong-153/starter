...
import { aiAppCredentials, permissions, rolePermissions, roles, userRoles } from '@api/infra/db/schema/index.js'
...
it('product app Session 按 app scope 和 externalUserId 隔离', async () => {
  ...
  try {
    ...
    const record = runtime.db.select().from(aiAppCredentials).where(eq(aiAppCredentials.projectId, 'project-a')).get()!
    ...
    const otherProject = await app.request(`/api/ai/sessions/${created.data.id}`, {
