...
import type { RecordedTelemetrySpan, TelemetryContext, TelemetrySpan } from '@earendil-works/pi-telemetry'
...
import { aiAgentRuns, aiModelCalls, aiRunSteps, aiRunTurns, aiToolExecutions } from '@api/infra/db/schema/index.js'
import { createAiTelemetryContext, openAiSpanScope, startAiSpan } from '@api/infra/telemetry/index.js'
...
function spanByName(spans: readonly RecordedTelemetrySpan[], name: string): RecordedTelemetrySpan {
...
it('startAiSpan 与 openAiSpanScope 组成 Run -> Turn -> Step -> Model Call/Tool 的父子树', async () => {
  ...
  await vi.waitFor(() => expect(recorder.getSpans().every((span) => span.settled !== false)).toBe(true))
  ...
  expect(toolSpan.attributes['starter.ai.tool.execution_id']).toBe('tool-execution-1')
})
...
it('telemetry adapter 抛错时业务 callback 仍然只执行一次并返回原结果', async () => {
  ...
  const telemetry = createAiTelemetryContext(
    ...
    {
      onFailure: (failure) => failures.push(`${failure.span}:${failure.operation}`),
    },
  )
...
it('业务 callback 抛错时保留原始异常，不被 telemetry 改写', async () => {
  ...
  expect(spanByName(recorder.getSpans(), 'starter.ai.turn').status).toMatchObject({ status: 'error' })
...
it('run 的 span 树与 SQLite 审计、Turn/Step 记录和 RunEvent 关联字段一致', async () => {
  ...
  const recorder = new InMemoryTelemetryContext()
  let calls = 0
  const streamSimple = ((_model: Model<Api>, context: Context, _options?: SimpleStreamOptions) => {
    calls += 1
  ...
  try {
    ...
    const modelSpans = spans.filter((span) => span.name === 'starter.ai.model_call')
    const toolSpans = spans.filter((span) => span.name === 'starter.ai.tool_execution')
    ...
    expect(runSpan.attributes['starter.ai.run.outcome']).toBe('completed')
    expect(runSpan.attributes['starter.ai.run.completion_reason']).toBe('model_finished')
    expect(runSpan.attributes['starter.ai.principal.kind']).toBe('starter_user')
    ...
    for (const modelSpan of modelSpans) {
      ...
      expect(modelSpan.attributes['starter.ai.model_call.result']).toBe('succeeded')
      expect(modelSpan.attributes['starter.ai.provider']).toBe(streamModel.provider)
      ...
      expect(modelSpan.attributes['starter.ai.response.id']).toBe('provider-response-1')
    }
    ...
    expect(toolCompleted?.toolExecutionId).toBe(toolSpan.attributes['starter.ai.tool.execution_id'])
    expect(toolCompleted?.toolCallId).toBe(toolSpan.attributes['starter.ai.tool.call_id'])
    expect(toolCompleted?.stepId).toBe(
      stepSpans.find((span) => span.id === toolSpan.parentId)?.attributes['starter.ai.step.id'],
    )
    expect(events.filter((event) => event.type === 'turn.started').map((event) => event.turnIndex)).toEqual(
...
it('模型上游失败时 Run、Turn、Step 和 Model Call span 都记录失败终态', async () => {
  ...
  const { app, cleanup, runtime } = runTestApp({
    store,
    telemetry: recorder,
    streamSimple: (() => streamProviderError()) as unknown as Models['streamSimple'],
  ...
  try {
    ...
    expect(runSpan.attributes['starter.ai.error.code']).toBe(ApiErrorCodes.AI_UPSTREAM_ERROR)
    ...
    expect(stepSpan.attributes['starter.ai.error.code']).toBe(ApiErrorCodes.AI_UPSTREAM_ERROR)
    const turnSpan = spanByName(spans, 'starter.ai.turn')
    expect(turnSpan.attributes['starter.ai.turn.outcome']).toBe('failed')
    const modelSpan = spanByName(spans, 'starter.ai.model_call')
    expect(modelSpan.attributes['starter.ai.model_call.result']).toBe('upstream_failed')
...
it('telemetry context 全面故障时 Run 终态、审计和 Tool 结果不变', async () => {
  ...
  try {
    ...
    expect(calls).toBe(2)
    expect(events.at(-1)?.type).toBe('run.completed')
    const runRow = runtime.db.select().from(aiAgentRuns).where(eq(aiAgentRuns.id, runId)).get()
    ...
    expect(events.find((event) => event.type === 'tool.completed')?.data).toMatchObject({
      status: 'succeeded',
      summary: 'looked up',
    })
    expect(
...
it('abort 的 Run、Turn、Step 和 Model Call span 记录取消终态', async () => {
  ...
  try {
    ...
    expect(runSpan.attributes['starter.ai.error.code']).toBe(ApiErrorCodes.AI_REQUEST_ABORTED)
    expect(runSpan.attributes['starter.ai.error.category']).toBe('cancelled')
    expect(runSpan.status).toMatchObject({ status: 'error' })
    expect(spanByName(spans, 'starter.ai.turn').attributes['starter.ai.turn.outcome']).toBe('aborted')
...
it('tool 失败但模型继续并最终成功时 Turn 和 Step 都记 succeeded', async () => {
  const directory = await mkdtemp(join(tmpdir(), 'starter-telemetry-toolfail-'))
  const store = createPiSessionStore({
    cwd: directory,
    databasePath: join(directory, 'agent-sessions.db'),
  })
  ...
  const streamSimple = ((_model: Model<Api>, context: Context, _options?: SimpleStreamOptions) => {
    const last = context.messages.at(-1)
    if (last?.role === 'toolResult') {
      return streamAssistant(assistantMessage([{ type: 'text', text: 'recovered' }], 'stop'), 'stop')
    }
    return streamAssistant(
      assistantMessage(
        [
  ...
  try {
    ...
    expect(events.filter((event) => event.type === 'turn.completed').map((event) => event.data.outcome)).toEqual([
      'succeeded',
      'succeeded',
    ])
    expect(JSON.stringify(recorder.getSpans())).not.toContain('SECRET-TOOL-FAILURE')
  } finally {
    cleanup()
    await rm(directory, { recursive: true, force: true })
  }
...
it('compaction 写入 kind=compaction 的 Step，id 与 span 和 context.compacted 事件一致', async () => {
  const directory = await mkdtemp(join(tmpdir(), 'starter-telemetry-compaction-'))
  const store = createPiSessionStore({
    cwd: directory,
    databasePath: join(directory, 'agent-sessions.db'),
  })
  ...
  try {
    ...
    const compactionSpan = recorder
      .getSpans()
      .find((span) => span.name === 'starter.ai.step' && span.attributes['starter.ai.step.kind'] === 'compaction')
    ...
    expect(compactionStep.id).toBe(compactionSpan?.attributes['starter.ai.step.id'])
    ...
    expect(JSON.stringify(recorder.getSpans())).not.toContain('SECRET-COMPACTION-SUMMARY')
  } finally {
    cleanup()
    await rm(directory, { recursive: true, force: true })
  }
...
it('compaction 摘要请求没有 audit 时仍产生 model_call span，parent 是 compaction step', async () => {
  const directory = await mkdtemp(join(tmpdir(), 'starter-telemetry-compaction-no-audit-'))
  const store = createPiSessionStore({
    cwd: directory,
    databasePath: join(directory, 'agent-sessions.db'),
  })
  ...
  try {
    const user = await register(app, 'telemetry-compaction-no-audit@example.com')
    seedEnabledModel(runtime)
    const agentId = seedAgent(runtime, [])
    const sessionId = await createSessionId(app, user.cookie)
    const session = await store.openSession(sessionId)
    ...
    const compactionStep = spans.find(
      (span) => span.name === 'starter.ai.step' && span.attributes['starter.ai.step.kind'] === 'compaction',
    )
    expect(compactionStep).toBeDefined()
    const summaryCall = spans.find(
      (span) => span.name === 'starter.ai.model_call' && span.attributes['starter.ai.streaming'] === false,
    )
    expect(summaryCall).toBeDefined()
    expect(summaryCall?.parentId).toBe(compactionStep?.id)
    // 没有 audit 也要有 modelCallId，并且记录终态
    expect(typeof summaryCall?.attributes['starter.ai.model_call.id']).toBe('string')
    expect(summaryCall?.attributes['starter.ai.model_call.result']).toBe('succeeded')
