import type { UserManagementUserDetail, UserManagementUserPage } from '@starter/contracts'
...
it('用户目录接口在未登录时返回 401，无权限时返回 403', async () => {
  ...
  try {
    ...
    expect((await readFailure(unauthenticatedList)).error.code).toBe(ApiErrorCodes.AUTH_UNAUTHENTICATED)

    const unauthenticatedDetail = await app.request('/api/users/019c3e00-0010-7000-8000-000000000099')
    ...
    expect((await readFailure(viewerList)).error.code).toBe(ApiErrorCodes.AUTH_FORBIDDEN)

    const viewerDetail = await app.request('/api/users/019c3e00-0010-7000-8000-000000000099', {
      headers: { cookie: viewerCookie },
    })
...
it('admin 可读取默认分页、搜索、角色筛选和稳定排序', async () => {
  ...
  try {
    ...
    const defaultData = (await readSuccess<UserManagementUserPage>(defaultPage)).data
    ...
    const customData = (await readSuccess<UserManagementUserPage>(customPage)).data
    ...
    const searchNameData = (await readSuccess<UserManagementUserPage>(searchByName)).data
    ...
    const searchEmailData = (await readSuccess<UserManagementUserPage>(searchByEmail)).data
    ...
    const caseData = (await readSuccess<UserManagementUserPage>(caseInsensitive)).data
    ...
    const wildcardData = (await readSuccess<UserManagementUserPage>(wildcardSearch)).data
    ...
    const roleData = (await readSuccess<UserManagementUserPage>(roleFilter)).data
    ...
    const viewerFilterData = (await readSuccess<UserManagementUserPage>(viewerFilter)).data
...
it('用户详情返回聚合的 provider、profile 和头像 URL，缺失时返回空值', async () => {
  ...
  try {
    ...
    const detailData = await readSuccess<UserManagementUserDetail>(detailResponse)
...
it('用户详情在用户不存在时返回 404，资料缺失时返回基础信息', async () => {
  ...
  try {
    ...
    // Non-existent user
    const notFound = await app.request('/api/users/019c3e00-0010-7000-8000-000000000099', {
      headers: { cookie: admin.cookie },
    })
    expect(notFound.status).toBe(404)
    expect((await readFailure(notFound)).error.code).toBe(ApiErrorCodes.COMMON_NOT_FOUND)
    ...
    runtime.db.delete(profiles).where(eq(profiles.userId, noProfileUserId)).run()

    const noProfileDetail = await app.request(`/api/users/${noProfileUserId}`, {
      headers: { cookie: admin.cookie },
    })
    expect(noProfileDetail.status).toBe(200)
    const noProfileData = (await readSuccess<UserManagementUserDetail>(noProfileDetail)).data
