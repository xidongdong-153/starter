  ...
  try {
    ...
    expect(
      runtime.db
        ...
        .where(eq(aiAppCredentialAuditEvents.appId, record.id))
        .all(),
    ).toHaveLength(1);
...
it("应用凭据轮换和撤销状态生效", async () => {
  ...
  try {
    ...
    const rotate = await app.request(
      `/api/ai/admin/applications/${created.data.application.appId}/rotate`,
    ...
    const revoke = await app.request(
      `/api/ai/admin/applications/${created.data.application.appId}/revoke`,
      { method: "POST", headers: { Cookie: admin.cookie } },
    );
    ...
    expect((await readFailure(repeated)).error.code).toBe(
      "AI.APP_CREDENTIAL_REVOKED",
    );
    expect(runtime.db.select().from(aiAppCredentials).all()[0]?.status).toBe(
      "revoked",
    );
  } finally {
    cleanup();
