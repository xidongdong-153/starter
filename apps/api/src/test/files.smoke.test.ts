...
it('文件上传、列表、下载、重命名、删除和用户所有权隔离可用', async () => {
  ...
  try {
    ...
    expect(content.headers.get('content-disposition')).toBe('inline; filename="avatar.png"')
    expect(new Uint8Array(await content.arrayBuffer())).toEqual(new Uint8Array([137, 80, 78, 71]))
    ...
    for (const response of [
      ...
      expect((await readFailure(response)).error.code).toBe(ApiErrorCodes.COMMON_NOT_FOUND)
    }
    ...
    expect((await app.request(`/api/profiles/${first.user.id}/avatar`)).status).toBe(404)
