...
function parseCompletionStream(
  body: string,
): Array<{ id?: string; event: CompletionStreamEvent }> {
  return body
    .trim()
    .split(/\r?\n\r?\n/)
    ...
    .map((frame) => {
      ...
      const event = completionStreamEventSchema.parse(
...
it("sse 模式返回 text_delta 序列和 done 事件，缺省 Accept 同样走流", async () => {
  ...
  try {
    ...
    const content = frames
      .map((frame) =>
        frame.event.type === "text_delta" ? frame.event.text : "",
      )
      .join("");
    ...
    expect(defaultResponse.headers.get("content-type")).toContain(
      "text/event-stream",
    );
    const defaultFrames = parseCompletionStream(await defaultResponse.text());
    expect(defaultFrames.map((frame) => frame.event.type)).toEqual([
      "text_delta",
      "text_delta",
      "done",
    ]);
...
it("schema 边界：input 超长、systemPrompt 超长、缺 model 返回 400", async () => {
  ...
  try {
    ...
    expect((await readFailure(longInput)).error.code).toBe(
