...
import { AiCredentialStore, AiCredentialConflictError } from '@api/infra/ai/ai-credential-store.js'
import { AiCredentialDecryptError, AiCredentialKeyUnavailableError, createAiCrypto } from '@api/infra/ai/ai-crypto.js'
import { AiModelsStore } from '@api/infra/ai/ai-models-store.js'
import { aiModelCatalogs, aiProviderConfigs } from '@api/infra/db/schema/index.js'
...
describe('ai encrypted stores', () => {
  it('aes-256-GCM 每次使用随机 IV，并拒绝缺失、错误和损坏密钥', () => {
    ...
    expect(() => createAiCrypto(undefined).encrypt(payload)).toThrow(AiCredentialKeyUnavailableError)
    expect(() => createAiCrypto(otherKey).decrypt(first)).toThrow(AiCredentialDecryptError)
    expect(() =>
      crypto.decrypt({
        ...first,
  ...
  it('credentialStore refresh 只增加 row version，undefined 不写库，delete 保留 runtime settings', async () => {
    ...
    try {
      ...
      await expect(store.modify('radius', async () => undefined)).resolves.toMatchObject({ access: 'old-access' })
  ...
  it('modelsStore 完整恢复 models、checkedAt、lastModified 和 etag，并拒绝损坏 JSON', async () => {
    ...
    try {
      ...
      runtime.db
        ...
        .run()
      await expect(store.read('radius')).rejects.toThrow('AI model catalog is invalid')
    } finally {
      cleanup()
    }
  })
...
function readProvider(runtime: ReturnType<typeof createTestApp>['runtime'], providerId: string) {
  return runtime.db.select().from(aiProviderConfigs).where(eq(aiProviderConfigs.providerId, providerId)).get()
}
