...
it("移除 ai_call_id 的 replacement migration 保留旧 Tool 行并修复外键", () => {
  ...
  try {
    ...
    sqlite.exec(
      readFileSync(resolve(migrationsFolder, "0020_amusing_plazm.sql"), "utf8"),
    );

    const columns = sqlite
      .prepare(`SELECT name FROM pragma_table_info('ai_tool_executions')`)
      .all() as Array<{ name: string }>;
    expect(columns.map((column) => column.name)).not.toContain("ai_call_id");
    expect(
      sqlite
        .prepare(
          `SELECT model_call_id, tool_name FROM ai_tool_executions WHERE id = ?`,
        )
        .get("legacy-tool-1"),
    ...
    const modelCallForeignKey = (
      sqlite
        .prepare(
          `SELECT "from", "on_delete" FROM pragma_foreign_key_list('ai_tool_executions')`,
        )
    ...
    sqlite
      .prepare(`DELETE FROM ai_model_calls WHERE id = ?`)
      .run("legacy-call-1");
    expect(
      sqlite
        .prepare(`SELECT id FROM ai_tool_executions WHERE id = ?`)
        .get("legacy-tool-1"),
    ).toBeUndefined();
...
it("ai_agent_runs 已经等于目标形态，不需要 replacement migration", () => {
  ...
  try {
    ...
    const columns = sqlite
      .prepare(`SELECT name, "notnull" FROM pragma_table_info('ai_agent_runs')`)
    ...
    const ddl = (
      sqlite
        .prepare(
          `SELECT sql FROM sqlite_master WHERE type = 'table' AND name = 'ai_agent_runs'`,
        )
        .get() as { sql: string }
    ).sql;
    ...
    const foreignKeys = sqlite
      .prepare(
        `SELECT "table", "on_delete" FROM pragma_foreign_key_list('ai_agent_runs')`,
      )
      .all() as Array<{ table: string; on_delete: string }>;
    expect(
      foreignKeys.map((key) => `${key.table}:${key.on_delete}`).sort(),
    ).toEqual(["ai_agent_definitions:RESTRICT", "ai_agent_sessions:CASCADE"]);
