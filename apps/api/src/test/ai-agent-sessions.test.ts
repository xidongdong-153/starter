...
import { createAiAgentSessionRepository, createAiAgentSessionService } from '@api/modules/ai/session/index.js'
...
it('session CRUD、owner 隔离、分页和幂等归档', async () => {
  ...
  try {
    ...
    expect((await requestJson(app, 'POST', '/api/ai/sessions', '', {})).status).toBe(401)

    const created = await requestJson(app, 'POST', '/api/ai/sessions', a.cookie, {
      title: '  首轮会话  ',
    })
    ...
    // 主库与 Pi DB 使用同一 id
    expect(await runtime.agentSessionStore.listSessions()).toContain(body.data.id)
    ...
    expect((await readFailure(crossRead)).error.code).toBe(ApiErrorCodes.COMMON_NOT_FOUND)
    ...
    // 更新 title
    const updated = await requestJson(app, 'PATCH', `/api/ai/sessions/${body.data.id}`, a.cookie, { title: '改名' })
    expect(updated.status).toBe(200)
    expect((await readSuccess<{ title: string }>(updated)).data.title).toBe('改名')

    // 归档
    const archived = await requestJson(app, 'DELETE', `/api/ai/sessions/${body.data.id}`, a.cookie)
    expect(archived.status).toBe(200)
    expect((await readSuccess<{ archivedAt: string | null }>(archived)).data.archivedAt).not.toBeNull()

    // 归档幂等：再次 DELETE 成功
    const archivedAgain = await requestJson(app, 'DELETE', `/api/ai/sessions/${body.data.id}`, a.cookie)
    ...
    expect(
      (await requestJson(app, 'PATCH', `/api/ai/sessions/${body.data.id}`, a.cookie, { title: '再改' })).status,
    ).toBe(404)
    // 归档 session 即使携带无效 defaultAgentId 也先返回 404（先资源后输入）
    expect(
      (await requestJson(app, 'PATCH', `/api/ai/sessions/${body.data.id}`, a.cookie, { defaultAgentId: generateId() }))
        .status,
    ).toBe(404)
    expect(
      (
...
it('transcript 投影、过滤、内部字段与 cursor/limit', async () => {
  ...
  try {
    ...
    const response = await app.request(`/api/ai/sessions/${sessionId}/transcript`, { headers: { cookie: user.cookie } })
    ...
    expect(types).toEqual(['user_message', 'user_message', 'assistant_message', 'tool_activity', 'system'])
    // 升序 sequence
    expect(body.data.items.map((item) => Number(item.sequence))).toEqual([1, 2, 3, 4, 5].map((n) => n))
    ...
    expect(assistant.toolCalls).toEqual([{ toolCallId: 'tc-1', name: 'read_skill' }])
    ...
    const page1 = await readSuccess<{
      ...
      await app.request(`/api/ai/sessions/${sessionId}/transcript?limit=2&direction=forward`, {
    ...
    expect(page2.data.items.map((item) => Number(item.sequence))).toEqual([3, 4])
    ...
    const exactPage = await readSuccess<{
      ...
      await app.request(`/api/ai/sessions/${sessionId}/transcript?limit=7&direction=forward`, {
        headers: { cookie: user.cookie },
      }),
    )
    ...
    // 非法 lane 被契约拦截
    const badLane = await app.request(`/api/ai/sessions/${sessionId}/transcript?lane=..%2Fevil`, {
      headers: { cookie: user.cookie },
    })
...
it('transcript backward 默认取最新一页，游标往更早翻且 items 保持时间正序', async () => {
  ...
  try {
    ...
    expect(latest.data.items.map((item) => item.content)).toEqual(['消息 4', '消息 5'])
    // nextCursor 指向本页最早一条，用它继续往更早翻
    expect(latest.data.nextCursor).toBe(4)

    const earlier = await readSuccess<Page>(
      await app.request(`/api/ai/sessions/${sessionId}/transcript?limit=2&cursor=${latest.data.nextCursor}`, {
        headers: { cookie: user.cookie },
      }),
    )
    ...
    const oldest = await readSuccess<Page>(
      await app.request(`/api/ai/sessions/${sessionId}/transcript?limit=2&cursor=${earlier.data.nextCursor}`, {
        headers: { cookie: user.cookie },
      }),
    )
...
it('assistant item 的 blocks 保留 text 与 thinking 的原始顺序，content 只拼 text', async () => {
  ...
  try {
    ...
    expect(assistant?.toolCalls).toEqual([{ toolCallId: 'tc-blocks', name: 'read_skill' }])
    // 工具入参不进协议
...
it('只读一致性检查报告两类 orphan，不修改数据', async () => {
  ...
  try {
    ...
    // 检查不删除数据
    expect(await runtime.agentSessionStore.listSessions()).toEqual(expect.arrayContaining([keepId, orphanPiId]))
  } finally {
    cleanup()
  }
})
...
it('主库写入失败补偿删除 Pi Session；补偿失败记录日志', async () => {
  ...
  try {
    ...
    await expect(service.create({}, starterRuntimeAccess(generateId()), 'req-1')).rejects.toMatchObject({
    ...
    const failingDeleteStore = {
      ...
      deleteSession: vi.fn().mockRejectedValue(new Error('delete failed')) as never,
    } as unknown as AgentSessionStore
    const logger2 = {
      error: vi.fn(),
      warn: vi.fn(),
    ...
    await expect(service2.create({}, starterRuntimeAccess(generateId()))).rejects.toMatchObject({
      status: 500,
    })
...
it('defaultAgentId 必须引用存在的 enabled Agent', async () => {
  ...
  try {
    ...
    // 引用不存在的 agent
    const missing = await requestJson(app, 'POST', '/api/ai/sessions', user.cookie, { defaultAgentId: generateId() })
    expect(missing.status).toBe(400)
    expect((await readFailure(missing)).error.code).toBe(ApiErrorCodes.COMMON_INVALID_REQUEST)

    // 引用 draft agent → 409
    const draft = await requestJson(app, 'POST', '/api/ai/sessions', user.cookie, { defaultAgentId: draftId })
    expect(draft.status).toBe(409)
    expect((await readFailure(draft)).error.code).toBe(ApiErrorCodes.AI_AGENT_NOT_ENABLED)
