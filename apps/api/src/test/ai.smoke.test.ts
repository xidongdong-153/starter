...
import { AiGatewayError, createAiCrypto, createAiRuntime } from '@api/infra/ai/index.js'
import { aiEnabledModels, aiModelCalls, aiProviderConfigs, aiSettings } from '@api/infra/db/schema/index.js'
...
const fakeGateway: AiGateway = {
  async *stream(input) {
    ...
    const firstBlock = firstMessage?.role === 'user' ? firstMessage.content[0] : undefined
...
it('拒绝非规范 base64 的 AI 凭据主密钥', () => {
  expect(() =>
    parseEnv({
      ...
      AI_CREDENTIAL_ENCRYPTION_KEY: 'MDEyMzQ1Njc4OWFiY2RlZjAxMjM0NTY3ODlhYmNkZWY=ignored',
    }),
...
it('custom Provider check 将认证失败和上游失败映射为不同安全错误码', async () => {
  ...
  try {
    ...
    expect(
      createAuthorizationRepository(runtime.db).bootstrapAdminByEmail('custom-check-admin@example.com', systemContext)
        .kind,
    ).toBe('ok')
    ...
    const upstreamFailure = await app.request('/api/ai/admin/custom-providers/custom-check-provider/check', {
      method: 'POST',
      headers: { cookie: admin.cookie, 'Content-Type': 'application/json' },
      body: JSON.stringify({ expectedRevision: created.data.revision }),
    })
    expect(upstreamFailure.status).toBe(503)
    expect((await readFailure(upstreamFailure)).error.code).toBe(ApiErrorCodes.AI_CUSTOM_PROVIDER_CHECK_FAILED)

    status = 401
    const authFailure = await app.request('/api/ai/admin/custom-providers/custom-check-provider/check', {
      method: 'POST',
      headers: { cookie: admin.cookie, 'Content-Type': 'application/json' },
      body: JSON.stringify({ expectedRevision: created.data.revision }),
    })
    expect(authFailure.status).toBe(503)
    expect((await readFailure(authFailure)).error.code).toBe(ApiErrorCodes.AI_PROVIDER_AUTH_FAILED)
  } finally {
    cleanup()
...
async function startCheckServer(getStatus: () => number): Promise<{ url: string; close: () => Promise<void> }> {
...
it('固定 Provider registry 精确覆盖 pi-ai 0.84.1 的 40 个文本 Provider', () => {
  ...
  try {
    expect(runtime.ai.providers.map((provider) => provider.id).sort()).toEqual([...expectedProviderIds].sort())
  } finally {
    cleanup()
  }
})
...
it('自定义 Provider 创建失败时不保留 definition', async () => {
  ...
  try {
    ...
    expect((await readFailure(response)).error.code).toBe(ApiErrorCodes.AI_CREDENTIAL_KEY_UNAVAILABLE)

    const read = await app.request('/api/ai/admin/custom-providers/rollback-provider', {
...
it('缺少主密钥时已有密文 Provider 进入 error 并被停用', async () => {
  const { cleanup, runtime } = createTestApp()
  try {
    const crypto = createAiCrypto('MDEyMzQ1Njc4OWFiY2RlZjAxMjM0NTY3ODlhYmNkZWY=')
    ...
    const lockedRuntime = createAiRuntime(runtime.db, createAiCrypto(undefined))
    await lockedRuntime.ensureReady()
    expect(
      runtime.db.select().from(aiProviderConfigs).where(eq(aiProviderConfigs.providerId, 'openai')).get(),
    ).toMatchObject({
      enabled: false,
      authStatus: 'error',
    ...
    expect(runtime.db.select().from(aiSettings).where(eq(aiSettings.id, 'global')).get()).toMatchObject({
...
it('credential 限制会过滤 GitHub Copilot 的可用模型', async () => {
  ...
  try {
    ...
    expect(filteredRuntime.listModels('github-copilot').length).toBe(catalog.length)
...
it('custom Provider 模型测试复用统一 SSE 和 model_test 审计', async () => {
  const { app, cleanup, runtime } = createTestApp({}, { aiGateway: fakeGateway })
  try {
    ...
    runtime.db
      ...
      .where(eq(aiProviderConfigs.providerId, providerId))
      .run()
    runtime.db.insert(aiEnabledModels).values({ providerId, modelId, enabledAt: new Date() }).run()
...
it('配置、模型白名单、用户偏好和 SSE 使用同一套 AI 服务端规则', async () => {
  ...
  try {
    ...
    expect(authorization.bootstrapAdminByEmail('ai-admin@example.com', systemContext).kind).toBe('ok')
    ...
    const deniedConfig = await app.request('/api/ai/admin/providers/openai/config', {
      method: 'PUT',
      headers: { cookie: user.cookie, 'Content-Type': 'application/json' },
    ...
    expect((await readFailure(noDefault)).error.code).toBe(ApiErrorCodes.AI_NO_AVAILABLE_MODEL)

    const providersResponse = await app.request('/api/ai/admin/providers', {
      headers: { cookie: admin.cookie },
    })
    expect(providersResponse.status).toBe(200)
    const providers = (await readSuccess<AdminAiProvider[]>(providersResponse)).data
    expect(providers).toHaveLength(40)
    expect(providers.map((provider) => provider.providerId)).toContain('openai')
    expect(providers.map((provider) => provider.providerId)).not.toContain('openrouter-images')
    const vertexProvider = providers.find((provider) => provider.providerId === 'google-vertex')
    expect(vertexProvider?.configFields.map((field) => field.key)).toEqual([
      'GOOGLE_CLOUD_PROJECT',
      'GOOGLE_CLOUD_LOCATION',
    ])

    const invalidAzureConfig = await app.request('/api/ai/admin/providers/azure-openai-responses/config', {
      method: 'PUT',
      headers: { cookie: admin.cookie, 'Content-Type': 'application/json' },
      body: JSON.stringify({
        apiKey: 'azure-secret',
    ...
    expect((await readFailure(invalidAzureConfig)).error.code).toBe(ApiErrorCodes.AI_CONFIG_INVALID)
    ...
    const vertexConfigResponse = await app.request('/api/ai/admin/providers/google-vertex/config', {
      method: 'PUT',
      headers: { cookie: admin.cookie, 'Content-Type': 'application/json' },
      body: JSON.stringify({ apiKey: vertexKey, settings: vertexSettings }),
    })
    expect(vertexConfigResponse.status).toBe(200)
    const vertexConfig = (await readSuccess<AdminAiProvider>(vertexConfigResponse)).data
    expect(vertexConfig.configuredSettings).toEqual(vertexSettings)
    expect(JSON.stringify(vertexConfig)).not.toContain(vertexKey)
    expect(runtime.ai.getProviderRequestEnv('google-vertex')).toEqual(vertexSettings)

    const fakeKey = 'sk-test-plain-secret-value'
    const configResponse = await app.request('/api/ai/admin/providers/openai/config', {
      method: 'PUT',
      headers: { cookie: admin.cookie, 'Content-Type': 'application/json' },
      body: JSON.stringify({ apiKey: fakeKey, settings: {} }),
    })
    expect(configResponse.status).toBe(200)
    const configured = (await readSuccess<AdminAiProvider>(configResponse)).data
    ...
    const stored = runtime.db.select().from(aiProviderConfigs).where(eq(aiProviderConfigs.providerId, 'openai')).get()!
    expect(stored.payloadCiphertext).not.toContain(fakeKey)

    const checkResponse = await app.request('/api/ai/admin/providers/openai/check', {
      method: 'POST',
      headers: { cookie: admin.cookie },
    })
    expect(checkResponse.status).toBe(200)
    expect((await readSuccess<AdminAiProvider>(checkResponse)).data).toMatchObject({
      authStatus: 'ready',
      authSource: 'stored_api_key',
      enabled: false,
    })

    const stateResponse = await app.request('/api/ai/admin/providers/openai/state', {
      method: 'PUT',
      headers: { cookie: admin.cookie, 'Content-Type': 'application/json' },
    ...
    const catalog = (await readSuccess<AdminAiModelsResponse>(catalogResponse)).data
    const model = catalog.items.find((item) => item.providerId === 'openai' && item.available)
    ...
    const userModels = (await readSuccess<AiUserModel[]>(userModelsResponse)).data
    ...
    expect((await readSuccess<AiUserPreference>(preferenceResponse)).data).toEqual({
      selectedModel: null,
    ...
    expect((await readSuccess<AiUserPreference>(updatePreferenceResponse)).data).toEqual({
    ...
    expect((await readFailure(invalidModelResponse)).error.code).toBe(ApiErrorCodes.AI_MODEL_NOT_ALLOWED)
    ...
    expect(streamEvents.map((event) => event.type)).toEqual(['start', 'text_delta', 'done'])
    ...
    const usageListResponse = await app.request('/api/ai/usage/calls?page=1&pageSize=20', {
      headers: { cookie: admin.cookie },
    })
    ...
    const successfulCall = usageList.items.find((item) => item.result === 'succeeded')
    ...
    const usageDetailResponse = await app.request(`/api/ai/usage/calls/${successfulCall?.id as string}`, {
      headers: { cookie: admin.cookie },
    })
    expect(usageDetailResponse.status).toBe(200)
    expect((await readSuccess<{ toolExecutions: unknown[] }>(usageDetailResponse)).data.toolExecutions).toEqual([])

    const recheckResponse = await app.request('/api/ai/admin/providers/openai/check', {
      method: 'POST',
      headers: { cookie: admin.cookie },
    })
    expect(recheckResponse.status).toBe(200)
    expect((await readSuccess<AdminAiProvider>(recheckResponse)).data.enabled).toBe(true)
    ...
    expect(timeoutEvents.map((event) => event.type)).toEqual(['start', 'error'])
    ...
    for (const [prompt, code] of [
      ...
      expect(errorEvents.map((event) => event.type)).toEqual(['start', 'error'])
    ...
    expect(unexpectedToolEvents.map((event) => event.type)).toEqual(['start', 'error'])
    ...
    const completedUsageResponse = await app.request('/api/ai/usage/calls?page=1&pageSize=100', {
      headers: { cookie: admin.cookie },
    })
    ...
    expect(completedUsage.map((item) => item.result)).toEqual(
      expect.arrayContaining(['succeeded', 'timed_out', 'auth_failed', 'upstream_failed', 'cancelled']),
    )
    expect(completedUsage.filter((item) => item.stopReason === 'tool_use')).toEqual([
    ...
    const exactFilter = new URLSearchParams({
      ...
      from: new Date(new Date(filterTarget.startedAt).getTime() - 1).toISOString(),
      to: new Date(new Date(filterTarget.startedAt).getTime() + 1).toISOString(),
    })
    const exactFilterResponse = await app.request(`/api/ai/usage/calls?${exactFilter.toString()}`, {
      headers: { cookie: admin.cookie },
    })
    expect(
      (await readSuccess<{ items: Array<{ id: string }> }>(exactFilterResponse)).data.items.map((item) => item.id),
    ).toEqual([filterTarget.id])

    const firstPageResponse = await app.request('/api/ai/usage/calls?page=1&pageSize=2', {
      headers: { cookie: admin.cookie },
    })
    const secondPageResponse = await app.request('/api/ai/usage/calls?page=2&pageSize=2', {
      headers: { cookie: admin.cookie },
    })
    const firstPageIds = (await readSuccess<{ items: Array<{ id: string }> }>(firstPageResponse)).data.items.map(
      (item) => item.id,
    )
    const secondPageIds = (await readSuccess<{ items: Array<{ id: string }> }>(secondPageResponse)).data.items.map(
      (item) => item.id,
    )
    ...
    const timedOutUsageResponse = await app.request('/api/ai/usage/calls?page=1&pageSize=20&result=timed_out', {
      headers: { cookie: admin.cookie },
    })
    expect(
      (await readSuccess<{ items: Array<{ result: string }> }>(timedOutUsageResponse)).data.items.every(
        (item) => item.result === 'timed_out',
      ),
    ).toBe(true)

    const replaceConfigResponse = await app.request('/api/ai/admin/providers/openai/config', {
      method: 'PUT',
      headers: { cookie: admin.cookie, 'Content-Type': 'application/json' },
      body: JSON.stringify({ apiKey: 'sk-replaced-secret', settings: {} }),
    })
    expect(replaceConfigResponse.status).toBe(200)
    expect((await readSuccess<AdminAiProvider>(replaceConfigResponse)).data).toMatchObject({
      enabled: false,
      authStatus: 'needs_check',
    })
    ...
    expect((await readSuccess<AiUserPreference>(fallbackAfterReplace)).data).toMatchObject({
      effectiveModel: null,
