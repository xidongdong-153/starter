...
import { runEventSchema, type AgentRun, type AgentTranscript, type RunEvent } from '@starter/contracts'
import { eq } from 'drizzle-orm'
import { createParser } from 'eventsource-parser'
import { expect, it } from 'vitest'

import { createPiAgentExecutor, createPiSessionStore } from '@api/infra/agent/index.js'
import {
  aiAgentDefinitions,
  aiEnabledModels,
  aiProviderConfigs,
...
it('产品后端通过公开 Bearer HTTP/SSE 完成 Run 并按 scope 恢复 Transcript', async () => {
  ...
  const executor = createPiAgentExecutor({
    ...
    streamFn: (_model: Model<Api>, _context: Context, _options?: SimpleStreamOptions) =>
  ...
  try {
    ...
    const terminal = events.filter((event) => ['run.completed', 'run.failed', 'run.aborted'].includes(event.type))
    ...
    const otherProjectSession = await otherProject.createSession('Other project')
    const unavailableToolRun = await otherProject.startRun(otherProjectSession.id, agentId)
    ...
    const disconnectedResponse = await client.startRun(disconnectedSession.id, agentId)
    const reader = disconnectedResponse.body?.getReader()
    if (!reader) throw new Error('SSE 缺少 response body')
    const firstChunk = await reader.read()
    expect(firstChunk.done).toBe(false)
    const firstEvents = parseSseByArbitraryChunks(new TextDecoder().decode(firstChunk.value), false)
    ...
    await reader.cancel()
    const recoveredRun = await waitForTerminalRun(client, disconnectedSession.id, disconnectedRunId)
    expect(recoveredRun.status).toBe('completed')
    const recoveredTranscript = await readSuccess<AgentTranscript>(await client.getTranscript(disconnectedSession.id))
    expect(JSON.stringify(recoveredTranscript.data)).toContain('cross product answer')
...
async function waitForTerminalRun(client: ProductClient, sessionId: string, runId: string): Promise<AgentRun> {
...
async function registerAiAdmin(
  ...
  const role = runtime.db.select({ id: roles.id }).from(roles).where(eq(roles.key, 'admin')).get()!
  ...
  runtime.db.update(userRoles).set({ roleId: role.id }).where(eq(userRoles.userId, admin.user.id)).run()
  return admin
}
...
function seedAgent(runtime: ReturnType<typeof createTestApp>['runtime'], systemPromptId: string): string {
