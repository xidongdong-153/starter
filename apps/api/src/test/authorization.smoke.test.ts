import type { AuthorizationRoleCatalog, AuthorizationUser, CurrentPermissions, FileItem } from '@starter/contracts'
...
it('授权 migration 写入系统目录并给已有用户回填 operator', () => {
  ...
  try {
    ...
    sqlite.exec(readFileSync(resolve(migrationsFolder, '0000_broken_komodo.sql'), 'utf8'))
    sqlite
      .prepare(
        `INSERT INTO user
    ...
    sqlite.exec(readFileSync(resolve(migrationsFolder, '0001_tidy_hellcat.sql'), 'utf8'))
    sqlite.exec(readFileSync(resolve(migrationsFolder, '0002_mean_iron_fist.sql'), 'utf8'))
    sqlite.exec(readFileSync(resolve(migrationsFolder, '0004_system_logs_read.sql'), 'utf8'))
    sqlite.exec(readFileSync(resolve(migrationsFolder, '0005_pale_madrox.sql'), 'utf8'))
    sqlite.exec(readFileSync(resolve(migrationsFolder, '0006_crazy_banshee.sql'), 'utf8'))
    sqlite.exec(readFileSync(resolve(migrationsFolder, '0007_clammy_shinobi_shaw.sql'), 'utf8'))
    ...
    expect(sqlite.prepare('SELECT key FROM permissions ORDER BY key').all()).toEqual(
...
it('bootstrap admin 命令返回可执行的配置和 migration 错误', () => {
  ...
  try {
    ...
    expect(runBootstrapAdmin({ ...baseEnv, AUTH_BOOTSTRAP_ADMIN_EMAIL: 'admin@example.com' }, output)).toBe(1)
    expect(errors.at(-1)).toContain('请先运行 pnpm --filter @starter/api db:migrate')
  } finally {
...
it('权限查询区分 401、403，默认 operator 和多角色权限并集可用', async () => {
  ...
  try {
    ...
    expect((await readFailure(unauthenticated)).error.code).toBe(ApiErrorCodes.AUTH_UNAUTHENTICATED)
    ...
    expect(runtime.db.select().from(userRoles).where(eq(userRoles.userId, admin.user.id)).all()).toHaveLength(1)

    const operatorPermissions = await app.request('/api/me/permissions', {
      headers: { cookie: viewer.cookie },
    })
    expect(operatorPermissions.status).toBe(200)
    const operatorData = (await readSuccess<CurrentPermissions>(operatorPermissions)).data
    ...
    expect((await readFailure(deniedManagement)).error.code).toBe(ApiErrorCodes.AUTH_FORBIDDEN)

    const adminPermissions = await app.request('/api/me/permissions', {
      headers: { cookie: admin.cookie },
    })
    expect((await readSuccess<CurrentPermissions>(adminPermissions)).data.permissions).toHaveLength(
      Object.values(PermissionKeys).length,
    )

    const union = runtime.db.select({ id: roles.id }).from(roles).where(eq(roles.key, RoleKeys.VIEWER)).get()
    ...
    const unionData = (await readSuccess<CurrentPermissions>(unionPermissions)).data
    ...
    const isolated = await app.request(`/api/me/permissions?userId=${admin.user.id}`, {
      headers: { cookie: viewer.cookie },
    })
    expect((await readSuccess<CurrentPermissions>(isolated)).data.roles).toEqual([RoleKeys.OPERATOR, RoleKeys.VIEWER])
  } finally {
    cleanup()
  }
})
...
it('权限表查询失败返回 500，不降级为 403', async () => {
  ...
  try {
    ...
    runtime.database.sqlite.exec('ALTER TABLE permissions RENAME TO permissions_unavailable')

    const response = await app.request('/api/files', {
      headers: { cookie: operator.cookie },
    })
    expect(response.status).toBe(500)
    expect((await readFailure(response)).error.code).toBe(ApiErrorCodes.SYSTEM_INTERNAL_ERROR)
  } finally {
    cleanup()
  }
})
...
it('管理接口使用事务替换角色，viewer 写操作拒绝且归档立即失效', async () => {
  ...
  try {
    ...
    expect(
      (await readSuccess<AuthorizationUser[]>(users)).data.find((item) => item.id === viewer.user.id)?.roleKeys,
    ).toEqual([RoleKeys.OPERATOR])

    const replaceViewer = await app.request(`/api/authorization/users/${viewer.user.id}/roles`, {
      method: 'PUT',
      headers: {
        cookie: admin.cookie,
    ...
    expect((await readSuccess<AuthorizationUser>(replaceViewer)).data.roleKeys).toEqual([RoleKeys.VIEWER])

    const rolesResponse = await app.request('/api/authorization/roles', {
      headers: { cookie: admin.cookie },
    })
    const catalog = (await readSuccess<AuthorizationRoleCatalog>(rolesResponse)).data
    expect(catalog.roles.find((role) => role.key === RoleKeys.ADMIN)).toMatchObject({
      permissionsEditable: false,
      permissionKeys: expect.arrayContaining(Object.values(PermissionKeys)),
    })

    const viewerBeforePermissionChange = await app.request('/api/me/permissions', {
      headers: { cookie: viewer.cookie },
    })
    const viewerBeforeData = (await readSuccess<CurrentPermissions>(viewerBeforePermissionChange)).data
    const replaceViewerPermissions = await app.request('/api/authorization/roles/viewer/permissions', {
      method: 'PUT',
      headers: {
        cookie: admin.cookie,
        'Content-Type': 'application/json',
    ...
    expect(
      (await readSuccess<AuthorizationRoleCatalog['roles'][number]>(replaceViewerPermissions)).data.permissionKeys,
    ).toEqual([PermissionKeys.FILE_LIST])

    const viewerAfterPermissionChange = await app.request('/api/me/permissions', { headers: { cookie: viewer.cookie } })
    const viewerAfterData = (await readSuccess<CurrentPermissions>(viewerAfterPermissionChange)).data
    ...
    const restoreViewerPermissions = await app.request('/api/authorization/roles/viewer/permissions', {
      method: 'PUT',
      headers: {
        cookie: admin.cookie,
        'Content-Type': 'application/json',
    ...
    const selfMutation = await app.request(`/api/authorization/users/${admin.user.id}/roles`, {
      method: 'PUT',
      headers: {
        cookie: admin.cookie,
        'Content-Type': 'application/json',
    ...
    expect((await readFailure(selfMutation)).error.code).toBe(ApiErrorCodes.AUTH_FORBIDDEN)

    const protectedAdminMutation = await app.request('/api/authorization/roles/admin/permissions', {
      method: 'PUT',
      headers: {
        cookie: admin.cookie,
        'Content-Type': 'application/json',
    ...
    expect((await readFailure(viewerUpload)).error.code).toBe(ApiErrorCodes.AUTH_FORBIDDEN)
    ...
    const otherOwnerContent = await app.request(`/api/files/${file.id}/content`, { headers: { cookie: viewer.cookie } })
    expect(otherOwnerContent.status).toBe(404)
    expect((await readFailure(otherOwnerContent)).error.code).toBe(ApiErrorCodes.COMMON_NOT_FOUND)

    const operatorRole = runtime.db.select({ id: roles.id }).from(roles).where(eq(roles.key, RoleKeys.OPERATOR)).get()
    ...
    expect((await readSuccess<CurrentPermissions>(archivedPermission)).data).toEqual({
      roles: [],
      permissions: [],
    ...
    runtime.db
      .update(permissions)
      .set({ archivedAt: new Date(), updatedAt: new Date() })
      .where(and(eq(permissions.key, PermissionKeys.FILE_UPLOAD), eq(permissions.isSystem, true)))
      .run()
    const archivedOperator = await app.request('/api/me/permissions', {
      headers: { cookie: admin.cookie },
    })
    expect((await readSuccess<CurrentPermissions>(archivedOperator)).data.permissions).not.toContain(
      PermissionKeys.FILE_UPLOAD,
    )
  } finally {
    cleanup()
...
function readUserRoleKeys(db: ReturnType<typeof createTestApp>['runtime']['db'], userId: string) {
  return db
...
function readRolePermissionKeys(db: ReturnType<typeof createTestApp>['runtime']['db'], roleKey: string) {
  return db
...
it('持有 authorization:manage 的非平台管理员不能写入授权关系', async () => {
  ...
  try {
    ...
    const repository = createAuthorizationRepository(runtime.db)
    expect(repository.bootstrapAdminByEmail('boundary-admin@example.com', systemContext).kind).toBe('ok')

    // 默认 seed 不给 operator 这个权限，必须显式构造才能走到 repository 的 actor 校验。
    grantPermissionToRole(runtime.db, RoleKeys.OPERATOR, PermissionKeys.AUTHORIZATION_MANAGE)
    const operatorPermissions = await app.request('/api/me/permissions', {
      headers: { cookie: operator.cookie },
    })
    expect((await readSuccess<CurrentPermissions>(operatorPermissions)).data.permissions).toContain(
      PermissionKeys.AUTHORIZATION_MANAGE,
    )

    const deniedUserRoles = await app.request(`/api/authorization/users/${target.user.id}/roles`, {
      method: 'PUT',
      headers: {
    ...
    expect(deniedUserRolesBody.error.message).toBe('只有平台管理员可以修改授权关系')
    expect(readUserRoleKeys(runtime.db, target.user.id)).toEqual([RoleKeys.OPERATOR])

    // 请求的权限集合和 viewer 当前值不同，被拒绝后关系必须保持原样。
    const viewerPermissionsBefore = readRolePermissionKeys(runtime.db, RoleKeys.VIEWER)
    expect(viewerPermissionsBefore).not.toEqual([PermissionKeys.FILE_UPLOAD])
    const deniedRolePermissions = await app.request('/api/authorization/roles/viewer/permissions', {
      method: 'PUT',
      headers: {
        cookie: operator.cookie,
        'Content-Type': 'application/json',
    ...
    expect((await readFailure(deniedRolePermissions)).error.message).toBe('只有平台管理员可以修改授权关系')
    expect(readRolePermissionKeys(runtime.db, RoleKeys.VIEWER)).toEqual(viewerPermissionsBefore)

    // 同样的两个写操作，平台管理员可以执行。
    const allowedUserRoles = await app.request(`/api/authorization/users/${target.user.id}/roles`, {
      method: 'PUT',
      headers: { cookie: admin.cookie, 'Content-Type': 'application/json' },
      body: JSON.stringify({ roleKeys: [RoleKeys.VIEWER] }),
    })
    expect(allowedUserRoles.status).toBe(200)
    expect((await readSuccess<AuthorizationUser>(allowedUserRoles)).data.roleKeys).toEqual([RoleKeys.VIEWER])

    const allowedRolePermissions = await app.request('/api/authorization/roles/viewer/permissions', {
      method: 'PUT',
      headers: { cookie: admin.cookie, 'Content-Type': 'application/json' },
      body: JSON.stringify({ permissionKeys: [PermissionKeys.FILE_UPLOAD] }),
    })
    expect(allowedRolePermissions.status).toBe(200)
    expect(readRolePermissionKeys(runtime.db, RoleKeys.VIEWER)).toEqual([PermissionKeys.FILE_UPLOAD])
  } finally {
    cleanup()
  }
})
...
it('提交相同集合时短路，不重写授权关系', async () => {
  ...
  try {
    ...
    const repository = createAuthorizationRepository(runtime.db)
    expect(repository.bootstrapAdminByEmail('idempotent-admin@example.com', systemContext).kind).toBe('ok')

    // 用哨兵时间戳而不是比较前后时钟：assigned_at 是 timestamp_ms，
    // 同一毫秒内的重写会产生相同的值，那样的断言证明不了短路。
    const sentinel = new Date(1700000000000)
    runtime.db.update(userRoles).set({ assignedAt: sentinel }).where(eq(userRoles.userId, target.user.id)).run()

    const sameRoles = await app.request(`/api/authorization/users/${target.user.id}/roles`, {
      method: 'PUT',
      headers: { cookie: admin.cookie, 'Content-Type': 'application/json' },
      body: JSON.stringify({ roleKeys: [RoleKeys.OPERATOR] }),
    })
    expect(sameRoles.status).toBe(200)
    expect((await readSuccess<AuthorizationUser>(sameRoles)).data.roleKeys).toEqual([RoleKeys.OPERATOR])
    ...
    const viewerRole = runtime.db.select({ id: roles.id }).from(roles).where(eq(roles.key, RoleKeys.VIEWER)).get()
    ...
    const samePermissions = await app.request('/api/authorization/roles/viewer/permissions', {
      method: 'PUT',
      headers: { cookie: admin.cookie, 'Content-Type': 'application/json' },
    ...
    runtime.db.update(userRoles).set({ assignedAt: sentinel }).where(eq(userRoles.userId, admin.user.id)).run()
    expect(repository.bootstrapAdminByEmail('idempotent-admin@example.com', systemContext).kind).toBe('ok')
...
it('撤销最后一个平台管理员被拒绝，关系不变', async () => {
  ...
  try {
    ...
    const repository = createAuthorizationRepository(runtime.db)
    expect(repository.bootstrapAdminByEmail('last-admin@example.com', systemContext).kind).toBe('ok')
    expect(readUserRoleKeys(runtime.db, admin.user.id)).toEqual([RoleKeys.ADMIN])

    // 走 repository 而不是 HTTP：HTTP 上 actor 必须是活动 admin 且不能改自己，
    // 撤销别人的 admin 之后 actor 自己还在，这条路径当前不可达。
    expect(repository.replaceUserRoles(admin.user.id, [RoleKeys.OPERATOR], systemContext)).toEqual({
    ...
    expect(repository.replaceUserRoles(admin.user.id, [RoleKeys.OPERATOR], systemContext).kind).toBe('ok')
    expect(readUserRoleKeys(runtime.db, admin.user.id)).toEqual([RoleKeys.OPERATOR])
  } finally {
    cleanup()
  }
})
...
it('互斥角色（SSD）：admin 独占，违反互斥组的角色分配被拒绝且关系不变', async () => {
  ...
  try {
    ...
    expect(conflictBody.error.message).toBe('角色分配违反职责分离约束：admin 与 operator 不能同时分配')
    expect(readUserRoleKeys(runtime.db, target.user.id)).toEqual([RoleKeys.OPERATOR])

    // 单独分配 [admin] 成功。
    const adminOnly = await putRoles([RoleKeys.ADMIN])
    expect(adminOnly.status).toBe(200)
    expect((await readSuccess<AuthorizationUser>(adminOnly)).data.roleKeys).toEqual([RoleKeys.ADMIN])
    ...
    expect(readUserRoleKeys(runtime.db, target.user.id)).toEqual([RoleKeys.OPERATOR, RoleKeys.VIEWER])

    // 存量违规（绕过校验直接写入 admin）不被扫描或自动修改；
    // 幂等提交相同集合（before == after）时放行，只有实际变更的写入才被拦截。
    const adminRole = runtime.db.select().from(roles).where(eq(roles.key, RoleKeys.ADMIN)).get()
    ...
    runtime.db
      ...
      .run()
    expect(readUserRoleKeys(runtime.db, target.user.id)).toEqual([RoleKeys.ADMIN, RoleKeys.OPERATOR, RoleKeys.VIEWER])

    const staleSubmit = await putRoles([RoleKeys.ADMIN, RoleKeys.OPERATOR, RoleKeys.VIEWER])
