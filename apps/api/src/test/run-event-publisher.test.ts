...
describe("runEvent Publisher", () => {
  it("写库后才入队，并为同一 Run 分配连续 sequence", async () => {
    ...
    try {
      ...
      expect(
        repository.listAfter(runId, 0, 10).map((event) => event.sequence),
      ).toEqual([1, 2]);
      expect(
        repository.findSequenceByEventId(runId, first?.eventId ?? ""),
      ).toBe(1);
    } finally {
      test.cleanup();
    }
  ...
  it("message delta 按 1KB 上限合并，非合并事件先刷缓冲", () => {
    ...
    try {
      ...
      const merged = published.filter(
        (event) => event.type === "message.delta",
      );
      expect(merged.length).toBe(
        Math.floor((chunk.length * chunkCount) / RUN_EVENT_MERGE_MAX_BYTES),
      );
      ...
      expect(published.map((event) => event.sequence)).toEqual(
        published.map((_event, index) => index + 1),
      );
      expect(
        published
      ...
      expect(repository.listAfter(runId, 0, 100)).toHaveLength(
        published.length,
      );
    } finally {
      test.cleanup();
  ...
  it("tool progress 合并成最新状态，切换合并键先刷旧缓冲", () => {
    ...
    try {
      ...
      const progress = published.filter(
        (event) => event.type === "tool.progress",
      );
      expect(
        progress.map((event) => [
          event.toolCallId,
          (event.data as { summary: string }).summary,
        ]),
      ).toEqual([
        ["tool-call-1", "step 3"],
  ...
  it("写库失败时不入队、不再发布后续事件，并上报存储失败", () => {
    ...
    try {
      ...
      expect(
        repository.listAfter(runId, 0, 100).map((e) => e.sequence),
      ).toEqual([1]);
    } finally {
      test.cleanup();
    }
