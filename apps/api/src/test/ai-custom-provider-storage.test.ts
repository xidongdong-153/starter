...
it('custom Provider contracts 严格校验协议、compat、URL、模型和 secret 边界', () => {
  ...
  expect(openAiCompletionsCompatSchema.safeParse({ unexpected: true }).success).toBe(false)
  expect(openAiResponsesCompatSchema.safeParse({ supportsToolSearch: true }).success).toBe(true)
  expect(anthropicMessagesCompatSchema.safeParse({ supportsStrictTools: true }).success).toBe(true)
  expect(
    customAiProviderDefinitionSchema.safeParse({
      ...definition(),
  ...
  expect(JSON.stringify(customAiProviderDefinitionSchema.parse(definition()))).not.toContain('secret')
})
...
it('custom Provider repository 支持 CRUD、CAS、内置 ID 冲突和删除引用清理', async () => {
  ...
  try {
    ...
    expect(repository.findById('local-gateway')?.definition.models).toEqual([model])
    expect(() => repository.create({ definition: definition(), actorId, now })).toThrow(AiCustomProviderExistsError)
    expect(() => repository.create({ definition: definition('openai'), actorId, now })).toThrow(
      AiCustomProviderIdConflictError,
    )
    expect(() =>
      repository.create({
    ...
    expect(
      runtime.db.select().from(aiProviderConfigs).where(eq(aiProviderConfigs.providerId, 'local-gateway')).all(),
    ).toEqual([])
    expect(
      runtime.db.select().from(aiModelCatalogs).where(eq(aiModelCatalogs.providerId, 'local-gateway')).all(),
    ).toEqual([])
    expect(
      runtime.db.select().from(aiEnabledModels).where(eq(aiEnabledModels.providerId, 'local-gateway')).all(),
    ).toEqual([])
    expect(runtime.db.select().from(userAiPreferences).where(eq(userAiPreferences.userId, actorId)).all()).toEqual([])
    ...
    expect(() => repository.findById('broken-provider')).toThrow(AiCustomProviderDefinitionInvalidError)
  } finally {
    cleanup()
  }
})
    ...
    expect(() =>
      repository.delete({
        ...
        assertNoAgentReferences(references) {
          expect(references).toEqual([{ id: 'referencing-agent', name: 'Referencing Agent' }])
