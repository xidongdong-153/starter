...
it("guard 兜底：session 存在但 user.status 为 suspended 时返回 401 AUTH.USER_SUSPENDED", async () => {
  ...
  try {
    ...
    expect((await readFailure(profileResponse)).error.code).toBe(
...
it("状态变更接口权限矩阵：未登录 401、viewer 403、admin 200、目标不存在 404", async () => {
  ...
  try {
    ...
    const unauthenticated = await app.request(
      `/api/users/${target.user.id}/status`,
      {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
    ...
    const viewerResponse = await patchStatus(
      app,
      viewer.cookie,
      target.user.id,
      "suspended",
    ...
    expect((await readFailure(viewerResponse)).error.code).toBe(
      ApiErrorCodes.AUTH_FORBIDDEN,
    );

    const missingResponse = await patchStatus(
      app,
      admin.cookie,
      "019c3e00-0010-7000-8000-000000000099",
      "suspended",
    );
    expect(missingResponse.status).toBe(404);
    expect((await readFailure(missingResponse)).error.code).toBe(
      ApiErrorCodes.COMMON_NOT_FOUND,
    );

    const adminOk = await patchStatus(
      app,
      admin.cookie,
      target.user.id,
      "suspended",
...
it("防呆：admin 不能禁用自己（400），幂等：重复提交目标状态返回 200", async () => {
  ...
  try {
    ...
    const selfResponse = await patchStatus(
      app,
      admin.cookie,
      admin.user.id,
      "suspended",
    ...
    expect((await readFailure(selfResponse)).error.code).toBe(
