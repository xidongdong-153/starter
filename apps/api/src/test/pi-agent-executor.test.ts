...
import { createAiTelemetryContext, openAiSpanScope } from '@api/infra/telemetry/index.js'
...
it('piAgentExecutor 使用 Pi Agent 完成多轮 Tool，并按 caller sequence 映射事件', async () => {
  ...
  let calls = 0
  const streamFn = (_requestModel: Model<Api>, context: Context, _options?: SimpleStreamOptions) => {
    calls += 1
    const last = context.messages.at(-1)
    if (last?.role === 'toolResult') {
      return streamResponse(assistantMessage([{ type: 'text', text: 'answer' }], 'stop'), 'stop')
    }
  ...
  const runScope = openAiSpanScope(createAiTelemetryContext(recorder), 'starter.ai.run', {
  ...
  const [events, terminal] = await Promise.all([collect(prepared.events), prepared.result])
  ...
  expect(transcript.filter((entry) => entry.type === 'message')).toHaveLength(4)
  expect(transcript.map((entry) => entry.type)).toEqual(['message', 'message', 'message', 'message'])
  ...
  expect(turnSpans.map((span) => span.parentId)).toEqual([runSpan?.id, runSpan?.id])
  expect(stepSpans.map((span) => span.parentId)).toEqual(turnSpans.map((span) => span.id))
  expect(toolSpans[0]?.parentId).toBe(stepSpans[0]?.id)
  expect(turnSpans.map((span) => span.attributes['starter.ai.turn.index'])).toEqual([1, 2])
  // Step span 的 ID 与事件关联字段一致
  const eventStepIds = new Set(events.map((event) => event.stepId).filter((value) => value !== null))
  expect(eventStepIds).toEqual(new Set(stepSpans.map((span) => span.attributes['starter.ai.step.id'])))
  expect(toolSpans[0]?.attributes['starter.ai.tool.call_id']).toBe('tool-call-1')
...
it('思考内容映射成 thinking 事件，message.completed 只带正文', async () => {
  ...
  const streamFn = (_requestModel: Model<Api>, _context: Context, _options?: SimpleStreamOptions) => {
    const stream = createAssistantMessageEventStream()
    const partial = assistantMessage([], 'pending')
    stream.push({ type: 'start', partial })
  ...
  expect(events.find((event) => event.type === 'thinking.started')?.data).toMatchObject({ display: true })
  expect(events.map((event) => (event.type === 'thinking.delta' ? event.data.delta : '')).join('')).toBe('先想一下')
  expect(events.map((event) => event.sequence ?? 0)).toEqual(
    events.map((event) => event.sequence ?? 0).sort((a, b) => a - b),
  )
...
it('工具上报进度时发布 tool.progress，只带脱敏摘要', async () => {
  ...
  registry.attach(lease, runId, prepared.controls)
  await prepared.start()
  const [events] = await Promise.all([collect(prepared.events), prepared.result])
...
it('工具超时后模型继续回复，Run 以 completed 结束', async () => {
  const directory = await mkdtemp(join(tmpdir(), 'starter-agent-tool-timeout-'))
  const store = createPiSessionStore({
    cwd: directory,
    databasePath: join(directory, 'agent-sessions.db'),
  })
  ...
  const streamFn = (_requestModel: Model<Api>, context: Context, _options?: SimpleStreamOptions) => {
    ...
    const last = context.messages.at(-1)
    if (last?.role === 'toolResult') {
      return streamResponse(assistantMessage([{ type: 'text', text: '工具超时了，换个思路。' }], 'stop'), 'stop')
    }
    return streamResponse(
      assistantMessage(
        [
...
it('pi JSON Schema 拒绝参数时仍生成安全 Tool 结果和一次审计', async () => {
  const directory = await mkdtemp(join(tmpdir(), 'starter-agent-invalid-tool-'))
  const store = createPiSessionStore({
    cwd: directory,
    databasePath: join(directory, 'agent-sessions.db'),
  })
  try {
    ...
    const streamFn = (_requestModel: Model<Api>, context: Context, _options?: SimpleStreamOptions) => {
      ...
      const last = context.messages.at(-1)
      if (last?.role === 'toolResult') {
        return streamResponse(assistantMessage([{ type: 'text', text: 'safe answer' }], 'stop'), 'stop')
      }
      return streamResponse(
        assistantMessage(
          [
...
it('pi compaction 成功后写入 entry，并用 retained context 继续运行', async () => {
  ...
  try {
    ...
    const streamFn = () => streamResponse(assistantMessage([{ type: 'text', text: 'answer' }], 'stop'), 'stop')
    const models = {
      getModel: vi.fn(() => compactModel),
      completeSimple: vi.fn(async () => assistantMessage([{ type: 'text', text: 'summary' }], 'stop')),
    ...
    const compactionModelCallId = modelAudit.beginModelCall.mock.calls[0]?.[0].id
    ...
    expect(transcript.filter((entry) => entry.type === 'message')).toHaveLength(3)

    // compaction 不再静默：发布 context.compacted 事件，携带 tokensBefore
    const compacted = compactionEvents.find((event) => event.type === 'context.compacted')
    expect(compacted).toBeDefined()
    expect(compacted?.data).toMatchObject({
      tokensBefore: expect.any(Number),
    })
    const compactionEntry = transcript.find((entry) => entry.type === 'compaction')
    expect((compacted?.data as { entryId: string } | undefined)?.entryId).toBe(compactionEntry?.id)
    expect((compacted?.data as { tokensBefore: number } | undefined)?.tokensBefore).toBeGreaterThan(0)
    ...
    expect(compactionStep?.attributes['starter.ai.step.outcome']).toBe('succeeded')
...
it('pi compaction 摘要失败时保留原 transcript 并返回失败结果', async () => {
  const directory = await mkdtemp(join(tmpdir(), 'starter-agent-compaction-failure-'))
  const store = createPiSessionStore({
    cwd: directory,
    databasePath: join(directory, 'agent-sessions.db'),
  })
  try {
    ...
    const executor = createPiAgentExecutor({
      ...
      models,
      resolveModel: () => compactModel,
      streamFn: () => streamResponse(assistantMessage([{ type: 'text', text: 'should not run' }], 'stop'), 'stop'),
      compaction: { reserveTokens: 10, keepRecentTokens: 0 },
    })
    const prepared = executor.prepare({
      execution: testRunExecution({
...
it('pi compaction entry 写入失败时保留原 transcript 并返回 Session storage 错误', async () => {
  const directory = await mkdtemp(join(tmpdir(), 'starter-agent-compaction-entry-failure-'))
  const store = createPiSessionStore({
    cwd: directory,
    databasePath: join(directory, 'agent-sessions.db'),
  })
  try {
    ...
    expect((await collect(prepared.events)).some((event) => event.type === 'context.compacted')).toBe(false)
    expect(models.completeSimple).toHaveBeenCalledOnce()
    const transcript = await store.readTranscript({ sessionId, lane: 'main' })
    expect(transcript.some((entry) => entry.type === 'compaction')).toBe(false)
    expect(
...
it('撞上 maxTurns 且仍在调工具时追加一轮无工具收尾', async () => {
  ...
  const streamFn = (_requestModel: Model<Api>, context: Context, _options?: SimpleStreamOptions) => {
    ...
    if ((context.tools ?? []).length === 0) {
      return streamResponse(assistantMessage([{ type: 'text', text: '根据已有结果给出结论。' }], 'stop'), 'stop')
    }
    return streamResponse(
      assistantMessage(
        [
  ...
  expect(JSON.stringify(modelContexts[2]?.messages)).toContain('Tools are no longer available')
...
it('撞上 maxTurns 时模型已给文字回答则不追加收尾轮', async () => {
  ...
  const streamFn = (_requestModel: Model<Api>, _context: Context, _options?: SimpleStreamOptions) => {
    ...
    return streamResponse(assistantMessage([{ type: 'text', text: '已经够了。' }], 'stop'), 'stop')
  }
  const tools = createAiToolRegistry([
    defineAiTool({
      sideEffect: 'read_only',
...
it('run 结束前用 listRunning 兜底关闭遗留的 Turn / Step', async () => {
  ...
  try {
    ...
    const executor = createPiAgentExecutor({
      sessionStore: store,
      resolveModel: () => model,
      streamFn: () => streamResponse(assistantMessage([{ type: 'text', text: 'done' }], 'stop'), 'stop'),
      lifecycle,
    })
    const prepared = executor.prepare({
      execution: testRunExecution({
