...
it("审计构造器只序列化 action 允许的 payload 字段", () => {
  ...
  try {
    ...
    expect(row!.beforeJson).toBe(
      JSON.stringify({ roleKeys: [RoleKeys.OPERATOR] }),
    );
    expect(row!.afterJson).toBe(
      JSON.stringify({ roleKeys: [RoleKeys.VIEWER] }),
    );
    expect(`${row!.beforeJson}${row!.afterJson}`).not.toContain(
      "must-not-be-stored",
    );
  } finally {
    cleanup();
...
it("用户角色替换按 admin 成员关系变化写三种 action", async () => {
  ...
  try {
    ...
    expect(
      repository.bootstrapAdminByEmail("audit-admin@example.com", systemContext)
        .kind,
    ).toBe("ok");
    ...
    expect(rows.slice(1).map((row) => [row.beforeJson, row.afterJson])).toEqual(
      [
        ...
        [
          JSON.stringify({ roleKeys: [RoleKeys.VIEWER] }),
          JSON.stringify({ roleKeys: [RoleKeys.ADMIN] }),
        ],
        [
          JSON.stringify({ roleKeys: [RoleKeys.ADMIN] }),
          JSON.stringify({ roleKeys: [RoleKeys.OPERATOR] }),
    ...
    expect(rows.at(-1)!.afterJson).toBe(
      JSON.stringify({ roleKeys: finalRoleKeys }),
    );
  } finally {
    cleanup();
  }
...
it("bootstrap 首次执行写事件，重复执行和幂等请求都不写", async () => {
  ...
  try {
    ...
    expect(
      repository.bootstrapAdminByEmail("audit-boot@example.com", systemContext)
        .kind,
    ).toBe("ok");
...
it("关系写入失败时不追加审计，关系删除也一并回滚", async () => {
  ...
  try {
    ...
    const target = await register(
      app,
      "audit-relation-rollback-target@example.com",
    );
    const repository = createAuthorizationRepository(runtime.db);
    expect(
      repository.bootstrapAdminByEmail(
        "audit-relation-rollback@example.com",
        systemContext,
      ).kind,
    ).toBe("ok");

    const beforeAuditCount = readAuditRowsFor(
      runtime.db,
      target.user.id,
    ).length;
    runtime.database.sqlite.exec(
    ...
    const roleKeys = runtime.db
      ...
      .map((role) => role.key);
    expect(roleKeys).toEqual([RoleKeys.OPERATOR]);
    expect(readAuditRowsFor(runtime.db, target.user.id)).toHaveLength(
      beforeAuditCount,
    );
  } finally {
    cleanup();
...
it("审计查询要求 authorization-audit:read，返回结构化 before/after", async () => {
  ...
  try {
    ...
    const initialized = page.items.find(
      (item) => item.action === AuditActions.USER_ROLES_INITIALIZED,
    );
...
it("审计查询支持过滤，并在相同 created_at 下按 id 稳定分页", async () => {
  ...
  try {
    ...
    const roleChange = await app.request(
      `/api/authorization/users/${other.user.id}/roles`,
      {
        method: "PUT",
        headers: { cookie: admin.cookie, "Content-Type": "application/json" },
    ...
    const permissionChange = await app.request(
      "/api/authorization/roles/viewer/permissions",
      {
        method: "PUT",
        headers: { cookie: admin.cookie, "Content-Type": "application/json" },
    ...
    runtime.db
      .update(authorizationAuditEvents)
    ...
    for (let page = 1; page <= total; page += 1) {
      const response = await app.request(
        `/api/authorization/audit-events?page=${page}&pageSize=1`,
        { headers: { cookie: admin.cookie } },
      );
      expect(response.status).toBe(200);
      const body = (await readSuccess<AuthorizationAuditEventPage>(response))
        .data;
    ...
    const filteredPage = (
      await readSuccess<AuthorizationAuditEventPage>(filtered)
    ).data;
    ...
    const actorFiltered = await app.request(
      `/api/authorization/audit-events?actorId=${admin.user.id}`,
      { headers: { cookie: admin.cookie } },
    );
    const actorPage = (
      await readSuccess<AuthorizationAuditEventPage>(actorFiltered)
    ).data;
    expect(actorPage.items.length).toBeGreaterThan(0);
    expect(
      actorPage.items.every((item) => item.actorId === admin.user.id),
    ).toBe(true);

    const targetFiltered = await app.request(
      `/api/authorization/audit-events?targetId=${other.user.id}`,
      { headers: { cookie: admin.cookie } },
    );
    const targetPage = (
      await readSuccess<AuthorizationAuditEventPage>(targetFiltered)
    ).data;
    expect(targetPage.items.length).toBeGreaterThan(0);
    expect(
      targetPage.items.every((item) => item.targetId === other.user.id),
    ).toBe(true);
    ...
    const timePage = (
      await readSuccess<AuthorizationAuditEventPage>(timeFiltered)
    ).data;
    expect(timePage.total).toBe(total);

    const afterSameInstant = encodeURIComponent(
      new Date(sameInstant.getTime() + 1).toISOString(),
    );
    const outsideRange = await app.request(
      `/api/authorization/audit-events?from=${afterSameInstant}`,
      { headers: { cookie: admin.cookie } },
    );
    expect(
      (await readSuccess<AuthorizationAuditEventPage>(outsideRange)).data.total,
    ).toBe(0);

    const unknownAction = await app.request(
      "/api/authorization/audit-events?action=not-a-real-action",
      { headers: { cookie: admin.cookie } },
    );
...
it("损坏的审计 JSON、payload 或 target type 返回 500，不泄漏原始值", async () => {
  ...
  try {
    ...
    expect((await readFailure(mismatched)).error.code).toBe(
    ...
    const mismatchedTarget = await app.request(
      "/api/authorization/audit-events",
      { headers: { cookie: admin.cookie } },
    );
    expect(mismatchedTarget.status).toBe(500);
    expect((await readFailure(mismatchedTarget)).error.code).toBe(
      ApiErrorCodes.SYSTEM_INTERNAL_ERROR,
    );
  } finally {
    cleanup();
...
it("审计事件类型在 contracts 中按 action 判别 payload", () => {
  ...
  expect(permissionEvent.before.permissionKeys).toEqual([
