...
import type { AgentTranscript, ApiSuccess, AiAttachment, AiUsage } from '@starter/contracts'
import { agentTranscriptUserMessageSchema, aiAttachmentSchema, ApiErrorCodes } from '@starter/contracts'
...
function textStream(text: string): ReturnType<typeof createAssistantMessageEventStream> {
...
function gatedTextStream(gate: Promise<void>, text: string): ReturnType<typeof createAssistantMessageEventStream> {
  const stream = createAssistantMessageEventStream()
  void gate.then(() => {
...
async function uploadAndRead(
  ...
  return aiAttachmentSchema.parse((await readSuccess<AiAttachment>(response)).data)
}

async function createSession(app: TestAppInstance, cookie: string): Promise<string> {
...
function seedEnabledModel(runtime: TestApp['runtime'], modelId: string): { providerId: string; modelId: string } {
  const now = new Date()
  runtime.db
...
function seedAgentWithModel(runtime: TestApp['runtime'], modelId: string): string {
  const id = generateId()
...
async function waitForRunCompleted(
  ...
  await vi.waitFor(
    async () => {
      const detail = await app.request(`/api/ai/sessions/${sessionId}/runs/${runId}`, { headers: { cookie } })
...
it('白名单内四种 MIME 上传成功，附件行按 starter_user 归属落库', async () => {
  ...
  try {
    ...
    for (const mimeType of ['image/png', 'image/jpeg', 'image/webp', 'image/gif']) {
      ...
      expect(response.status).toBe(201)
      const item = aiAttachmentSchema.parse((await readSuccess<AiAttachment>(response)).data)
...
it('超 5MB、白名单外 MIME、空文件与非法 session 归属的上传被拒绝', async () => {
  ...
  try {
    ...
    expect((await readFailure(tooLarge)).error.code).toBe(ApiErrorCodes.AI_ATTACHMENT_TOO_LARGE)
    ...
    for (const mimeType of ['image/bmp', 'text/plain']) {
      ...
      expect((await readFailure(rejected)).error.code).toBe(ApiErrorCodes.AI_ATTACHMENT_TYPE_NOT_ALLOWED)
    }
    ...
    expect((await readFailure(empty)).error.code).toBe(ApiErrorCodes.COMMON_INVALID_REQUEST)
    ...
    expect((await readFailure(missingSession)).error.code).toBe(ApiErrorCodes.COMMON_NOT_FOUND)
    ...
    expect((await readFailure(foreignSession)).error.code).toBe(ApiErrorCodes.COMMON_NOT_FOUND)
...
it('startRun 带 attachmentIds 时 user message 含 image 块与顶层 attachmentIds，纯文本请求不带 image 块', async () => {
  ...
  try {
    ...
    expect((message as { attachmentIds?: unknown }).attachmentIds).toEqual([first.id, second.id])
    ...
    expect((plainMessage as { content: unknown }).content).toEqual([{ type: 'text', text: '纯文本问题' }])
    expect((plainMessage as { attachmentIds?: unknown }).attachmentIds).toBeUndefined()
  } finally {
    test.cleanup()
...
it('模型不支持图片时 startRun 报 AI_IMAGE_NOT_SUPPORTED，不建 Run 也不消费幂等键', async () => {
  ...
  try {
    ...
    const rejected = await startRunWithAttachments(test.app, user.cookie, sessionId, agentId, [attachment.id])
    expect(rejected.status).toBe(400)
    expect((await readFailure(rejected)).error.code).toBe(ApiErrorCodes.AI_IMAGE_NOT_SUPPORTED)

    // 带上幂等键重发一次，验证两次都被拒且都不建 Run
    const rejectedWithKey = await test.app.request(`/api/ai/sessions/${sessionId}/runs`, {
      method: 'POST',
      headers: {
        cookie: user.cookie,
        'Content-Type': 'application/json',
    ...
    expect((await readFailure(rejectedWithKey)).error.code).toBe(ApiErrorCodes.AI_IMAGE_NOT_SUPPORTED)
    expect(test.runtime.db.select().from(aiAgentRuns).all()).toHaveLength(0)

    // 同一幂等键去掉附件重试能成功启动，说明失败请求没有消费 key
    const retried = await test.app.request(`/api/ai/sessions/${sessionId}/runs`, {
      method: 'POST',
      headers: { cookie: user.cookie, 'Content-Type': 'application/json' },
      body: JSON.stringify({
        agentId,
...
it('模型不支持图片时 steer 与 followUp 带附件同样报 AI_IMAGE_NOT_SUPPORTED', async () => {
  ...
  try {
    ...
    const steer = await test.app.request(`/api/ai/sessions/${sessionId}/runs/${runId}/steer`, {
      method: 'POST',
      headers: { cookie: user.cookie, 'Content-Type': 'application/json' },
      body: JSON.stringify({
        text: '再看这张图',
    ...
    expect((await readFailure(steer)).error.code).toBe(ApiErrorCodes.AI_IMAGE_NOT_SUPPORTED)

    const followUp = await test.app.request(`/api/ai/sessions/${sessionId}/runs/${runId}/follow-ups`, {
      method: 'POST',
      headers: { cookie: user.cookie, 'Content-Type': 'application/json' },
      body: JSON.stringify({
        text: '追问带图',
    ...
    expect((await readFailure(followUp)).error.code).toBe(ApiErrorCodes.AI_IMAGE_NOT_SUPPORTED)
...
it('模型不支持图片时 completion 带附件报 AI_IMAGE_NOT_SUPPORTED 且不写模型调用审计', async () => {
  ...
  try {
    ...
    expect((await readFailure(response)).error.code).toBe(ApiErrorCodes.AI_IMAGE_NOT_SUPPORTED)
...
it('越权引用：他人附件与挂错 session 的附件返回 AI_ATTACHMENT_NOT_FOUND 且不建 Run', async () => {
  ...
  try {
    ...
    const deniedForeign = await startRunWithAttachments(test.app, owner.cookie, sessionA, agentId, [foreign.id])
    expect(deniedForeign.status).toBe(404)
    expect((await readFailure(deniedForeign)).error.code).toBe(ApiErrorCodes.AI_ATTACHMENT_NOT_FOUND)
    ...
    const deniedScope = await startRunWithAttachments(test.app, owner.cookie, sessionB, agentId, [scoped.id])
    expect(deniedScope.status).toBe(404)
    expect((await readFailure(deniedScope)).error.code).toBe(ApiErrorCodes.AI_ATTACHMENT_NOT_FOUND)
...
it('attachmentIds 超过 4 张在请求 schema 层被拒绝', async () => {
  ...
  try {
    ...
    expect((await readFailure(start)).error.code).toBe(ApiErrorCodes.COMMON_INVALID_REQUEST)
    ...
    expect((await readFailure(completion)).error.code).toBe(ApiErrorCodes.COMMON_INVALID_REQUEST)
  } finally {
    cleanup()
  }
})
...
it('带图 Run 完成后 transcript 投影 images 引用，base64 不出边界', async () => {
  ...
  try {
    ...
    const transcriptResponse = await test.app.request(`/api/ai/sessions/${sessionId}/transcript`, {
    ...
    const userItem = transcript.items.find((item) => item.type === 'user_message')
    ...
    expect(transcript.items.find((item) => item.type === 'assistant_message')).toBeDefined()
...
it('附件下载返回原始字节与 Content-Type，他人下载 404', async () => {
  ...
  try {
    ...
    const content = await app.request(`/api/ai/attachments/${attachment.id}/content`, {
      headers: { cookie: user.cookie },
    })
    expect(content.status).toBe(200)
    expect(content.headers.get('content-type')).toBe('image/png')
    expect(content.headers.get('content-length')).toBe(String(PNG_BYTES.length))
    expect(content.headers.get('cross-origin-resource-policy')).toBe('cross-origin')
    expect(new Uint8Array(await content.arrayBuffer())).toEqual(PNG_BYTES)

    const denied = await app.request(`/api/ai/attachments/${attachment.id}/content`, {
...
it('steer 带附件时插入的 user message 含 image 块并送达模型', async () => {
  ...
  const streamSimple: Models['streamSimple'] = (_model, context) => {
    ...
    contexts.push(context)
    signalFirstCall()
    return calls === 1 ? gatedTextStream(gate, '第一轮回复') : textStream('第二轮回复')
  }
  const test = createAttachmentRunApp({
    store,
    modelId: IMAGE_MODEL_ID,
  ...
  try {
    ...
    expect((steered as { attachmentIds?: unknown }).attachmentIds).toEqual([attachment.id])
  } finally {
    test.cleanup()
    await rm(directory, { recursive: true, force: true })
  }
...
it('携带 sessionId 上传的附件行随 session 删除级联清理', async () => {
  ...
  try {
    ...
    runtime.db.delete(aiAgentSessions).where(eq(aiAgentSessions.id, sessionId)).run()
    expect(runtime.db.select().from(aiAttachments).all()).toHaveLength(0)
  } finally {
    cleanup()
  }
