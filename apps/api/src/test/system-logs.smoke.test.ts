...
it('日志接口权限：未认证 401，非 admin 403，admin 可查且按时间倒序', async () => {
  ...
  try {
    ...
    const data = (await readSuccess<{ items: LogEntry[]; total: number }>(ok)).data
    // 倒序（最新在前）、损坏行跳过、跨文件（旧文件 500 排最后）
    expect(timesOf(data.items)).toEqual([6000, 5000, 4000, 3000, 2000, 1000, 500])
...
it('日志接口过滤：level、query 过滤且返回 total', async () => {
  ...
  const { app, cleanup, runtime } = createTestApp({ LOGS_DIR: logsDir })
  try {
    const admin = await bootstrapAdmin(app, runtime, 'logs-filter-admin@example.com')
    const get = (search: string) =>
      app.request(`/api/system/logs${search}`, {
        headers: { cookie: admin.cookie },
      })

    const levelError = await readSuccess<{ items: LogEntry[]; total: number }>(await get('?level=error'))
    expect(timesOf(levelError.data.items)).toEqual([6000, 2000])
    expect(levelError.data.total).toBe(2)

    const queryUpload = await readSuccess<{ items: LogEntry[]; total: number }>(await get('?query=files.upload'))
...
it('日志接口分页：page/pageSize 切片且 total 为全部匹配数', async () => {
  ...
  const { app, cleanup, runtime } = createTestApp({ LOGS_DIR: logsDir })
  try {
    const admin = await bootstrapAdmin(app, runtime, 'logs-paged-admin@example.com')
    const get = (search: string) =>
      app.request(`/api/system/logs${search}`, {
        headers: { cookie: admin.cookie },
      })

    const page2 = await readSuccess<{ items: LogEntry[]; total: number }>(await get('?page=2&pageSize=3'))
    expect(timesOf(page2.data.items)).toEqual([3000, 2000, 1000])
    expect(page2.data.total).toBe(7)

    const page3 = await readSuccess<{ items: LogEntry[]; total: number }>(await get('?page=3&pageSize=3'))
    ...
    const defaultPage = await readSuccess<{ items: LogEntry[]; total: number }>(await get(''))
...
it('日志接口 requestId 链路：精确过滤并按时间正序', async () => {
  ...
  const { app, cleanup, runtime } = createTestApp({ LOGS_DIR: logsDir })
  try {
    const admin = await bootstrapAdmin(app, runtime, 'logs-link-admin@example.com')
    const response = await app.request('/api/system/logs?requestId=req-1', {
      headers: { cookie: admin.cookie },
    })
    expect(response.status).toBe(200)
    const data = (await readSuccess<{ items: LogEntry[]; total: number }>(response)).data
...
it('日志接口未配置 LOGS_DIR 时返回 400', async () => {
  ...
  try {
    const admin = await bootstrapAdmin(app, runtime, 'logs-nodir-admin@example.com')
