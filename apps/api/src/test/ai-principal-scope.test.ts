...
import { createAppSecret, hashAppSecret } from '@api/modules/ai/application/application.crypto.js'
...
it('product app guard 从 credential 派生 scope，并拒绝缺失或不完整 subject', async () => {
  ...
  const app = new Hono()
  app.onError((error, context) => context.text('error', error instanceof AppError ? error.status : 500))
...
function adapterOptions(
  ...
  return {
    ...
    hasPermission: vi.fn(async (_userId: string, _permission: Permission) => true),
  }
}
...
describe('product_app tool permission boundary', () => {
  it('product_app 调用带要求的 Tool 直接 forbidden，伪造 Starter 用户 external ID 也不查 user_roles', async () => {
    ...
    const adapter = createPiToolAdapter(registerProtectedTool().list(), options)
    const tool = adapter.tools[0]
    if (!tool) throw new Error('tool missing')

    await expect(tool.execute('product-tool-call', {}, new AbortController().signal)).rejects.toBeInstanceOf(
  ...
  it('starter_user 权限查询抛错时按 forbidden 处理，不降级允许', async () => {
    ...
    await expect(tool.execute('starter-tool-call', {}, new AbortController().signal)).rejects.toBeInstanceOf(
      PiToolExecutionError,
    )
    const override = await adapter.afterToolCall({
      toolCall: {
  ...
  it('超过 16000 字符或不可序列化的参数按 invalid_arguments 处理', async () => {
    ...
    for (const args of [bigArgs, circularArgs]) {
      ...
      const tool = adapter.tools[0]
      if (!tool) throw new Error('tool missing')
      await expect(tool.execute('oversize-tool-call', args, new AbortController().signal)).rejects.toBeInstanceOf(
        PiToolExecutionError,
      )
      const override = await adapter.afterToolCall({
        toolCall: {
