...
import { aiAgentRuns, permissions, rolePermissions, roles, userRoles } from '@api/infra/db/schema/index.js'
...
import { readFailure, readSuccess, register, type createTestApp } from './helpers.js'
...
interface RunAppContext {
  ...
  startRunJson: (sessionId: string, body: Record<string, unknown>) => Promise<Response>
  startRunSse: (sessionId: string, body: Record<string, unknown>) => Promise<Response>
  runRows: () => RunRow[]
  cleanup: () => Promise<void>
}

function doneStream(): ReturnType<typeof createAssistantMessageEventStream> {
  return streamAssistant(buildAssistantMessage([{ type: 'text', text: 'done' }], 'stop'), 'stop')
}
  ...
  return () => {
    ...
    void gate.then(() => {
      stream.push({
        type: 'done',
        reason: 'stop',
        message: buildAssistantMessage([{ type: 'text', text: 'done' }], 'stop'),
      })
    })
    return stream
  }
...
async function setupRunApp(streamFn: Models['streamSimple']): Promise<RunAppContext> {
...
async function pollRunTerminal(
  ...
  for (let attempt = 0; attempt < 100; attempt += 1) {
    const response = await app.request(`/api/ai/sessions/${sessionId}/runs/${runId}`, { headers })
...
it('同 key 同 session 两次启动返回同一 runId，只创建一条 Run row', async () => {
  ...
  try {
    ...
    await pollRunTerminal(ctx.app, { cookie: ctx.user.cookie }, sessionId, runId)
...
it('同 key 不同 session 返回 409 AI_IDEMPOTENCY_KEY_CONFLICT', async () => {
  ...
  try {
    ...
    expect((await readFailure(conflict)).error.code).toBe(ApiErrorCodes.AI_IDEMPOTENCY_KEY_CONFLICT)

    // 等第一个 Run 终态后再断言行数，避免后台事务在 cleanup 后落库。
    const replay = await readSuccess<{ runId: string }>(await ctx.startRunJson(sessionA, { idempotencyKey: key }))
    const runId = replay.data.runId
    await pollRunTerminal(ctx.app, { cookie: ctx.user.cookie }, sessionA, runId)
    expect(ctx.runRows()).toHaveLength(1)
  } finally {
    await ctx.cleanup()
  }
...
it('lane busy 不消费 key：占端结束后同 key 请求创建新 Run', async () => {
  ...
  try {
    ...
    expect((await readFailure(busy)).error.code).toBe(ApiErrorCodes.AI_SESSION_BUSY)

    release()
    await pollRunTerminal(ctx.app, { cookie: ctx.user.cookie }, sessionId, runA)

    const next = await ctx.startRunJson(sessionId, { idempotencyKey: key })
    const runB = await readRunId(next)
    expect(runB).not.toBe(runA)
    await pollRunTerminal(ctx.app, { cookie: ctx.user.cookie }, sessionId, runB)
...
it('两个 product_app 凭据使用相同 key 各自独立成 Run', async () => {
  ...
  try {
    ...
    const statusA = await pollRunTerminal(app, bearerHeaders(appA.secret), runA.sessionId, runA.runId)
    const statusB = await pollRunTerminal(app, bearerHeaders(appB.secret), runB.sessionId, runB.runId)
...
it('failed Run 同 key 重试返回既有 runId，不新建也不重跑', async () => {
  ...
  try {
    ...
    const first = await ctx.startRunJson(sessionId, { idempotencyKey: key })
    const runId = await readRunId(first)
    const terminal = await pollRunTerminal(ctx.app, { cookie: ctx.user.cookie }, sessionId, runId)
    ...
    const after = await pollRunTerminal(ctx.app, { cookie: ctx.user.cookie }, sessionId, retryRunId)
...
async function registerAdmin(app: RunAppContext['app'], runtime: RunAppContext['runtime']) {
