...
it("product_app 发现 Agent、JSON 启动 Run、读取结构化输出并回放 transcript", async () => {
  ...
  try {
    ...
    const enabledAgent = await setupAgent(
      app,
      runtime,
      admin,
      "third-party-enabled",
    );
    const disabledAgent = await setupAgent(
      app,
      runtime,
      admin,
    ...
    const disabled = await patchJson(
      app,
      `/api/ai/admin/agents/${disabledAgent.agentId}/status`,
      admin.cookie,
      { status: "disabled" },
    );
    expect(disabled.status).toBe(200);
    const productAgent = await setupAgent(
      app,
      runtime,
      admin,
      "third-party-product",
    ...
    const adminContractAgent = await setupAgent(
      app,
      runtime,
      admin,
      "third-party-admin-contract",
      8,
      [],
      adminContract,
    );
    ...
    expect(list.data.items.every((item) => item.status === "enabled")).toBe(
      true,
    );

    const detailResponse = await client.getAgent(enabledAgent.agentId);
    expect(detailResponse.status).toBe(200);
    const detail = await readSuccess<{ id: string; status: string }>(
      detailResponse,
    );
    ...
    expect((await readFailure(invalidBearer)).error.code).toBe(
    ...
    const jsonStart = await client.startRunJson(
      session1.id,
      productAgent.agentId,
    );
    ...
    const sseStart = await client.startRunSse(
      sseSession.id,
      productAgent.agentId,
    );
    ...
    const outputsResponse = await client.getStructuredOutputs(
      session1.id,
      run1Id,
    );
    ...
    expect(
      uuidSchema.safeParse(outputs.data.items[0]?.referenceId).success,
    ).toBe(true);

    const session2 = await client.createSession("Admin 可见性 contract");
    const adminContractStart = await client.startRunJson(
      session2.id,
      adminContractAgent.agentId,
    );
    expect(adminContractStart.status).toBe(200);
    const adminContractStarted = await readSuccess<{ runId: string }>(
      adminContractStart,
    );
    ...
    const maskedResponse = await client.getStructuredOutputs(
      session2.id,
      run2Id,
    );
    ...
    const adminRouteRun1 = await app.request(
      `/api/ai/admin/runs/${run1Id}/structured-outputs`,
      { headers: { Cookie: admin.cookie } },
    );
    expect(adminRouteRun1.status).toBe(200);
    const adminRun1Outputs =
      await readSuccess<StructuredOutputList>(adminRouteRun1);
    expect(adminRun1Outputs.data.items[0]?.value).toEqual({
      result: "approved",
    });

    const adminRouteRun2 = await app.request(
      `/api/ai/admin/runs/${run2Id}/structured-outputs`,
      { headers: { Cookie: admin.cookie } },
    );
    expect(adminRouteRun2.status).toBe(200);
    const adminRouteOutputs =
      await readSuccess<StructuredOutputList>(adminRouteRun2);
    ...
    expect(
      (await otherClient.getStructuredOutputs(session1.id, run1Id)).status,
    ).toBe(404);

    // Admin 路由 runId 不存在：404。
    const missingRun = await app.request(
      `/api/ai/admin/runs/${generateId()}/structured-outputs`,
      { headers: { Cookie: admin.cookie } },
    );
    expect(missingRun.status).toBe(404);
    expect((await readFailure(missingRun)).error.code).toBe(
    ...
    expect(emit1?.structuredOutput?.referenceId).toBe(
      outputs.data.items[0]?.referenceId,
    );
...
function createProductClient(
  ...
  return {
    ...
    getAgent: async (agentId) =>
      app.request(`/api/ai/agents/${agentId}`, { headers }),
    ...
    getStructuredOutputs: async (sessionId, runId) =>
      app.request(
        `/api/ai/sessions/${sessionId}/runs/${runId}/structured-outputs`,
        { headers },
      ),
...
async function registerAdmin(
  ...
  const admin = await register(
    app,
    `third-party-admin-${Date.now()}@example.com`,
  );
