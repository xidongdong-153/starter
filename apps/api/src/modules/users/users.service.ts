...
export function createUsersService(repository: UsersRepository) {
  async function listUsers(query: UserManagementQuery): Promise<UserManagementUserPage> {
    ...
    return {
      items: result.users.map((u) => toUserManagementUser(u, roleKeysByUser.get(u.id) ?? [])),
      total: result.total,
      page: query.page,
  ...
  async function getUserDetail(userId: string): Promise<UserManagementUserDetail> {
  ...
  async function updateUserStatus(
    ...
    const result = repository.updateUserStatus(actorId, targetUserId, status, requestId)
    switch (result.kind) {
      case 'user-not-found':
        throw new AppError(ApiErrorCodes.COMMON_NOT_FOUND, '用户不存在', 404)
      case 'self-suspend':
        throw new AppError(ApiErrorCodes.COMMON_INVALID_REQUEST, '不能禁用自己', 400)
      default:
