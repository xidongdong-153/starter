...
const listUsersRoute = createRoute({
  ...
  responses: {
    200: apiSuccessResponse(userManagementUserPageSchema, '分页用户列表', 'UserManagementUserPageResponse'),
...
const getUserDetailRoute = createRoute({
  ...
  responses: {
    200: apiSuccessResponse(userManagementUserDetailSchema, '用户详情', 'UserManagementUserDetailResponse'),
    401: unauthorizedResponse,
    403: forbiddenResponse,
...
const updateUserStatusRoute = createRoute({
  ...
  responses: {
    200: apiSuccessResponse(updateUserStatusResponseSchema, '更新后的用户状态', 'UpdateUserStatusResponse'),
    400: invalidRequestResponse,
    401: unauthorizedResponse,
    403: forbiddenResponse,
    404: notFoundResponse,
...
export function createUsersRoute(runtime: AppRuntime) {
  ...
  const requireUsersRead = createRequirePermission(runtime.db, PermissionKeys.AUTHORIZATION_READ)
  const requireUsersManage = createRequirePermission(runtime.db, PermissionKeys.AUTHORIZATION_MANAGE)
  ...
  const app = new OpenAPIHono<HonoEnv>()
    .openapi(
      ...
      async (c) => c.json(createSuccessResponse(await service.listUsers(c.req.valid('query')), c.var.requestId), 200),
    )
    .openapi(
      {
        ...getUserDetailRoute,
      ...
      async (c) =>
        c.json(createSuccessResponse(await service.getUserDetail(c.req.valid('param').userId), c.var.requestId), 200),
    )
    .openapi(
      {
      ...
      async (c) => {
        ...
        const data = await service.updateUserStatus(c.var.currentUserId, userId, status, c.var.requestId)
        c.var.logger.info(
          {
