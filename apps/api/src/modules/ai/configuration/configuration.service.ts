...
import type { AiGateway, AiRuntime, AiStoredPayloadColumns } from '@api/infra/ai/index.js'
import { AiGatewayError, AiRuntimeError } from '@api/infra/ai/index.js'
import { AiCredentialDecryptError, AiCredentialKeyUnavailableError } from '@api/infra/ai/ai-crypto.js'
...
import { toAdminAiModel, toAdminAiProvider, toAiUserModel, toMissingAdminAiModel } from './configuration.presenter.js'

export function createAiService(
  repository: ReturnType<typeof import('./configuration.repository.js').createAiRepository>,
  runtime: AiRuntime,
  gateway: AiGateway,
  invocationRunner?: AiInvocationRunner,
  requestTimeoutMs = 120_000,
  customProviders?: ReturnType<typeof import('./custom-provider.repository.js').createAiCustomProviderRepository>,
) {
  async function createCustomProvider(input: CreateCustomAiProviderInput, actorId: string): Promise<CustomAiProvider> {
    requireCustomRepository()
    await runtime.validateCustomProviderUrl(input.baseUrl).catch(() => {
      throw new AppError(ApiErrorCodes.AI_CUSTOM_PROVIDER_URL_INVALID, 'Provider URL 不允许访问', 400)
    })
    ...
    try {
      ...
      if (apiKey !== undefined) {
        const payload = runtime.prepareProviderConfig(definition.providerId, undefined, apiKey, {})
        repository.saveProviderConfig(definition.providerId, payload, actorId, null)
      } else {
  ...
  async function updateCustomProvider(
    ...
    if (requireCustomRecord(providerId).revision !== expectedRevision) {
      throw new AppError(ApiErrorCodes.AI_CUSTOM_PROVIDER_CONFLICT, 'Custom Provider 已被其他操作更新', 409)
    }
    ...
    try {
      if (apiKey !== undefined) {
        ...
        repository.saveProviderConfig(providerId, payload, actorId, config?.rowVersion ?? null)
      } else {
  ...
  async function updateCustomCredential(
    ...
    try {
      ...
      repository.saveProviderConfig(providerId, payload, actorId, current?.rowVersion ?? null)
      runtime.reloadProvider(providerId)
      return getCustomProvider(providerId)
    } catch (error) {
      throwCustomProviderError(error)
  ...
  async function deleteCustomProvider(
    ...
    const config = repository.findProviderConfig(providerId)
    if (config?.enabled) {
      throw new AppError(ApiErrorCodes.AI_PROVIDER_DISABLED, '请先停用 Provider', 409)
    }
    try {
      const deleted = customProviders!.delete({
        ...
        assertNoAgentReferences(references) {
          if (references.length > 0) {
            throw new AppError(ApiErrorCodes.AI_CUSTOM_PROVIDER_IN_USE, 'Provider 仍被 Agent 引用', 409)
          }
        },
      })
  ...
  async function clearCustomCredential(providerId: string, actorId: string): Promise<CustomAiProvider> {
    requireCustomRecord(providerId)
  ...
  async function getCustomProvider(providerId: string): Promise<CustomAiProvider> {
    requireCustomRepository()
    await runtime.ensureReady()
  ...
  async function listProviders(): Promise<AdminAiProvider[]> {
    await runtime.ensureReady()
    const configs = new Map(repository.listProviderConfigs().map((config) => [config.providerId, config]))
  ...
  async function clearProviderCredential(providerId: string, actorId: string): Promise<AdminAiProvider> {
    await runtime.ensureReady()
    requireProvider(providerId)
    const current = repository.findProviderConfig(providerId)
    try {
      const payload = current ? runtime.prepareCredentialClear(toStoredColumns(current)) : null
      repository.clearProviderCredential(providerId, payload, actorId, current?.rowVersion ?? null)
      runtime.reloadProvider(providerId)
      return await findProviderDto(providerId)
    } catch (error) {
      throwRuntimeError(error)
  ...
  async function checkProvider(providerId: string, actorId: string): Promise<AdminAiProvider> {
    await runtime.ensureReady()
    const definition = requireProvider(providerId)
  ...
  async function setProviderState(providerId: string, enabled: boolean, actorId: string): Promise<AdminAiProvider> {
    await runtime.ensureReady()
    requireProvider(providerId)
    const config = repository.findProviderConfig(providerId)
    if (!config) {
      throw new AppError(ApiErrorCodes.AI_PROVIDER_NOT_CONFIGURED, 'Provider 尚未完成认证检查', 409)
    }
    if (enabled && (config.authStatus !== 'ready' || config.checkedConfigRevision !== config.configRevision)) {
      throw new AppError(ApiErrorCodes.AI_PROVIDER_NOT_CONFIGURED, 'Provider 配置需要重新检查后才能启用', 409)
    }
  ...
  async function refreshProviderModels(providerId: string, actorId: string): Promise<AdminAiModelsResponse> {
    await runtime.ensureReady()
    const definition = requireProvider(providerId)
    if (!definition.supportsModelRefresh) {
      throw new AppError(ApiErrorCodes.AI_CONFIG_INVALID, '这个 Provider 不支持刷新模型目录', 400)
    }
    const config = repository.findProviderConfig(providerId)
    if (!isCheckedReady(config)) {
      throw new AppError(ApiErrorCodes.AI_PROVIDER_NOT_CONFIGURED, 'Provider 尚未完成认证检查', 503)
    }
    ...
    try {
      ...
      if (error instanceof DOMException && error.name === 'AbortError') {
        throw new AppError(ApiErrorCodes.AI_REQUEST_ABORTED, '模型目录刷新已取消', 503)
      }
      throw new AppError(ApiErrorCodes.AI_CATALOG_REFRESH_FAILED, '模型目录刷新失败，可以稍后重试', 503)
    }
  }
  ...
  async function listAdminModels(): Promise<AdminAiModelsResponse> {
    await runtime.ensureReady()
    const configs = new Map(repository.listProviderConfigs().map((config) => [config.providerId, config]))
    const definitions = new Map(runtime.providers.map((definition) => [definition.id, definition]))
    ...
    const items: AdminAiModel[] = catalog.map((model) => {
      ...
      const available = Boolean(config?.enabled && ready && availableKeys.has(refKey(model)))
      return toAdminAiModel({
        model,
        providerName: definitions.get(model.providerId)?.name ?? model.providerId,
    ...
    for (const ref of enabledRefs) {
      if (!catalogKeys.has(refKey(ref))) {
        items.push(toMissingAdminAiModel(ref, definitions.get(ref.providerId)?.name ?? ref.providerId))
      }
    }
  ...
  async function replaceEnabledModels(
    ...
    await runtime.ensureReady()
    const catalog = new Map(runtime.listAvailableModels().map((model) => [refKey(model), model]))
    const configs = new Map(repository.listProviderConfigs().map((config) => [config.providerId, config]))
    for (const ref of input.models) {
      if (!catalog.has(refKey(ref))) {
        throw new AppError(ApiErrorCodes.AI_MODEL_NOT_FOUND, '模型不在当前目录中', 404)
      }
      if (!isCheckedReady(configs.get(ref.providerId))) {
        throw new AppError(ApiErrorCodes.AI_PROVIDER_NOT_CONFIGURED, '模型所属 Provider 尚未完成认证检查', 409)
      }
    }
  ...
  async function setGlobalDefault(model: AiModelRef | null, actorId: string): Promise<AdminAiModelsResponse> {
    await runtime.ensureReady()
    if (model && !isModelAllowed(model)) {
      throw new AppError(ApiErrorCodes.AI_MODEL_NOT_ALLOWED, '只能把已启用的可用模型设为全局默认', 403)
    }
  ...
  async function listUserModels(): Promise<AiUserModel[]> {
    ...
    return runtime
      .listAvailableModels()
      .filter((model) => allowed.has(refKey(model)) && isProviderAvailable(model.providerId))
      .map((model) => toAiUserModel(model, definitions.get(model.providerId)?.name ?? model.providerId))
      .sort(compareModels)
  }

  async function getPreference(userId: string): Promise<AiUserPreference> {
    await runtime.ensureReady()
    const selectedModel = toPreferenceRef(repository.findUserPreference(userId))
  ...
  async function setPreference(userId: string, model: AiModelRef | null): Promise<AiUserPreference> {
    await runtime.ensureReady()
    if (model && !isModelAllowed(model)) {
      throw new AppError(ApiErrorCodes.AI_MODEL_NOT_ALLOWED, '这个模型当前不可用', 403)
    }
  ...
  async function prepareTest(userId: string, input: AiTestInput) {
    await runtime.ensureReady()
    const model = input.model ? requireExplicitModel(input.model) : selectModelForUser(userId)
    return {
      model,
  ...
  function selectModelForUser(userId: string): AiModelRef {
    ...
    if (!effective) {
      throw new AppError(ApiErrorCodes.AI_NO_AVAILABLE_MODEL, '当前没有可用模型', 503)
    }
  ...
  function selectDefaultModel(userModel: AiModelRef | null) {
    if (userModel && isModelAllowed(userModel)) return { model: userModel, source: 'user' as const }
    const globalModel = repository.findGlobalDefault()
    if (globalModel && isModelAllowed(globalModel)) return { model: globalModel, source: 'global' as const }
    return null
  }

  function isModelAllowed(model: AiModelRef): boolean {
    const enabled = repository.listEnabledModels().some((item) => refKey(item) === refKey(model))
    if (!enabled || !isProviderAvailable(model.providerId)) return false
    return runtime.listAvailableModels(model.providerId).some((item) => item.modelId === model.modelId)
  }
  ...
  function readSettings(config: AiProviderConfigRecord | undefined): Record<string, string> {
    if (!config) return {}
    try {
  ...
  function requireProvider(providerId: string) {
    const definition = runtime.providers.find((provider) => provider.id === providerId)
  ...
  function requireCustomRepository(): void {
    if (!customProviders) {
      throw new AppError(ApiErrorCodes.AI_CONFIG_INVALID, 'Custom Provider 存储未装配', 500)
    }
  }
  ...
  function toCustomProvider(
    record: NonNullable<ReturnType<NonNullable<typeof customProviders>['findById']>>,
  ): CustomAiProvider {
    ...
    return {
      ...
      authStatus:
        config?.authStatus === 'needs_check' || config?.authStatus === 'ready' || config?.authStatus === 'error'
          ? config.authStatus
          : 'not_configured',
  ...
  function throwCustomProviderError(error: unknown): never {
    ...
    if (error instanceof AiCustomProviderExistsError) {
      throw new AppError(ApiErrorCodes.AI_CUSTOM_PROVIDER_EXISTS, 'Custom Provider 已存在', 409)
    }
    if (error instanceof AiCustomProviderIdConflictError) {
      throw new AppError(ApiErrorCodes.AI_CUSTOM_PROVIDER_ID_CONFLICT, 'Provider ID 与内置 Provider 冲突', 409)
    }
    if (error instanceof AiCustomProviderRevisionConflictError) {
      throw new AppError(ApiErrorCodes.AI_CUSTOM_PROVIDER_CONFLICT, 'Custom Provider 已被其他操作更新', 409)
    }
    if (error instanceof AiCustomProviderDefinitionInvalidError) {
      throw new AppError(ApiErrorCodes.AI_CONFIG_INVALID, 'Custom Provider 配置无效', 400)
    }
    if (
...
function validateProviderConfig(definition: AiRuntime['providers'][number], input: UpdateAiProviderConfigInput): void {
  if (input.apiKey !== undefined && !definition.acceptsAdminApiKey) {
    throw new AppError(ApiErrorCodes.AI_CONFIG_INVALID, '这个 Provider 不接受 Admin 保存 API Key', 400)
  }
  const fields = new Map(definition.configFields.map((field) => [field.key, field]))
  const unknownKeys = Object.keys(input.settings).filter((key) => !fields.has(key))
  if (unknownKeys.length > 0) {
    throw new AppError(ApiErrorCodes.AI_CONFIG_INVALID, 'Provider 配置包含未知字段', 400, { keys: unknownKeys })
  }
  if (input.apiKey !== undefined) {
    const missing = definition.configFields
      ...
      .map((field) => field.key)
    if (missing.length > 0) {
      throw new AppError(ApiErrorCodes.AI_CONFIG_INVALID, 'Provider 配置缺少必填字段', 400, { keys: missing })
    }
  }
  for (const configField of definition.configFields) {
    ...
    if (value && configField.type === 'url' && !isUrl(value)) {
      throw new AppError(ApiErrorCodes.AI_CONFIG_INVALID, `${configField.label} 不是有效 URL`, 400)
    }
  }
  if (definition.id === 'azure-openai-responses') {
    if (
      ...
      throw new AppError(ApiErrorCodes.AI_CONFIG_INVALID, 'Azure OpenAI 需要 Base URL 或 Resource Name', 400)
    }
...
function throwProviderNotFound(): never {
  throw new AppError(ApiErrorCodes.AI_PROVIDER_NOT_FOUND, 'Provider 不存在', 404)
}
...
function throwCustomCheckError(error: AiRuntimeError): never {
  ...
  throw new AppError(ApiErrorCodes.AI_CUSTOM_PROVIDER_CHECK_FAILED, 'Provider 检查失败，可以稍后重试', 503)
}
...
function throwRuntimeError(error: unknown): never {
  ...
  if (normalized.kind === 'credential_key_unavailable') {
    throw new AppError(ApiErrorCodes.AI_CREDENTIAL_KEY_UNAVAILABLE, 'API 未配置 AI 凭据加密密钥', 503)
  }
  if (normalized.kind === 'conflict') {
    throw new AppError(ApiErrorCodes.AI_CREDENTIAL_CONFLICT, 'Provider 凭据已被其他操作更新，请重试', 409)
  }
  throw new AppError(ApiErrorCodes.AI_PROVIDER_AUTH_FAILED, 'Provider 认证失败，请检查配置后重试', 503)
}

function normalizeRuntimeError(error: unknown): AiRuntimeError {
  if (error instanceof AiProviderConfigConflictError) return new AiRuntimeError('conflict')
...
function isCheckedReady(config: AiProviderConfigRecord | undefined): boolean {
  return Boolean(config?.authStatus === 'ready' && config.checkedConfigRevision === config.configRevision)
}

function toStoredColumns(config: AiProviderConfigRecord): AiStoredPayloadColumns {
  return {
...
function countByProvider(items: readonly { providerId: string }[]): Map<string, number> {
  const counts = new Map<string, number>()
  for (const item of items) counts.set(item.providerId, (counts.get(item.providerId) ?? 0) + 1)
  return counts
}
...
function compareModels(
  ...
  return left.providerId.localeCompare(right.providerId) || left.modelId.localeCompare(right.modelId)
}
...
function isStringMapJson(value: string): boolean {
  try {
    ...
    return (
      ...
      Object.values(parsed).every((item) => typeof item === 'string' && item.trim().length > 0)
    )
  } catch {
    return false
  }
