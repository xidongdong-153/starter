import type { AiAuthSource, AiAuthStatus, AiModelRef } from '@starter/contracts'
import type { AppDatabase } from '@api/infra/db/client.js'
import type { AiPreparedProviderPayload } from '@api/infra/ai/index.js'
import { and, eq } from 'drizzle-orm'

import { aiEnabledModels, aiProviderConfigs, aiSettings, userAiPreferences } from '../ai.schema.js'
...
export function createAiRepository(db: AppDatabase) {
  ...
  function findProviderConfig(providerId: string): AiProviderConfigRecord | undefined {
    return db.select().from(aiProviderConfigs).where(eq(aiProviderConfigs.providerId, providerId)).get()
  }
  ...
  function saveProviderConfig(
    ...
    return db.transaction((tx) => {
      const current = tx.select().from(aiProviderConfigs).where(eq(aiProviderConfigs.providerId, providerId)).get()
      if ((current?.rowVersion ?? null) !== expectedRowVersion) throw new AiProviderConfigConflictError()
      const now = new Date()
      ...
      return tx.select().from(aiProviderConfigs).where(eq(aiProviderConfigs.providerId, providerId)).get()!
    })
  }
  ...
  function recordAuthCheck(input: {
    ...
    return db.transaction((tx) => {
      const current = tx
        ...
        .get()
      if ((current?.configRevision ?? null) !== input.expectedConfigRevision) throw new AiProviderConfigConflictError()
      const now = new Date()
      const values = {
        ...
        checkedConfigRevision: input.status === 'ready' ? (current?.configRevision ?? 0) : null,
      ...
      tx.insert(aiProviderConfigs)
        ...
        .run()
      if (input.status !== 'ready') clearGlobalDefaultForProvider(tx, input.providerId, input.actorId, now)
      return tx.select().from(aiProviderConfigs).where(eq(aiProviderConfigs.providerId, input.providerId)).get()!
    })
  }
  ...
  function setProviderEnabled(
    ...
    return db.transaction((tx) => {
      ...
      if (
        !current ||
        (enabled && (current.authStatus !== 'ready' || current.checkedConfigRevision !== current.configRevision))
      ) {
        return undefined
      }
      const now = new Date()
  ...
  function replaceEnabledModels(models: readonly AiModelRef[], actorId: string): void {
    db.transaction((tx) => {
      ...
      if (global && !hasRef(models, global)) setGlobalDefaultWith(tx, null, actorId, now)
    })
  }

  function pruneProviderModels(providerId: string, validModelIds: readonly string[], actorId: string): void {
    db.transaction((tx) => {
      const providerRows = tx.select().from(aiEnabledModels).where(eq(aiEnabledModels.providerId, providerId)).all()
      for (const row of providerRows) {
        if (!validModelIds.includes(row.modelId)) {
          tx.delete(aiEnabledModels)
            .where(and(eq(aiEnabledModels.providerId, providerId), eq(aiEnabledModels.modelId, row.modelId)))
            .run()
        }
      }
      const global = findGlobalDefaultWith(tx)
      if (global?.providerId === providerId && !validModelIds.includes(global.modelId)) {
  ...
  function findUserPreference(userId: string): AiUserPreferenceRecord | undefined {
    return db.select().from(userAiPreferences).where(eq(userAiPreferences.userId, userId)).get()
  }

  function setUserPreference(userId: string, model: AiModelRef | null): void {
    if (!model) {
      db.delete(userAiPreferences).where(eq(userAiPreferences.userId, userId)).run()
      return
    }
    const values = {
      userId,
...
function hasRef(models: readonly AiModelRef[], target: AiModelRef): boolean {
  return models.some((model) => model.providerId === target.providerId && model.modelId === target.modelId)
}
...
function findGlobalDefaultWith(db: DbLike): AiModelRef | null {
  const row = db.select().from(aiSettings).where(eq(aiSettings.id, 'global')).get()
  return row?.globalProviderId && row.globalModelId
    ? { providerId: row.globalProviderId, modelId: row.globalModelId }
    : null
}

function setGlobalDefaultWith(db: DbLike, model: AiModelRef | null, actorId: string | null, now: Date): void {
  const values = {
    id: 'global',
...
function clearGlobalDefaultForProvider(db: DbLike, providerId: string, actorId: string | null, now: Date): void {
  const current = findGlobalDefaultWith(db)
  if (current?.providerId === providerId) setGlobalDefaultWith(db, null, actorId, now)
}
