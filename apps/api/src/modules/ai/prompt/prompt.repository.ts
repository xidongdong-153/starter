...
import { aiAgentDefinitions, aiPromptTemplates, aiSettings, aiSystemPrompts } from '@api/modules/ai/ai.schema.js'
  ...
  return {
    ...
    updateSystemPrompt(input) {
      db.update(aiSystemPrompts)
        ...
        .run()
      return db.select().from(aiSystemPrompts).where(eq(aiSystemPrompts.id, input.id)).get() ?? null
    },
