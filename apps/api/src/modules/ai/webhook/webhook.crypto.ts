 ...
 */
export function createWebhookCrypto(encodedKey: string | undefined): WebhookCrypto {
  ...
  return {
    ...
    decryptSecret(payload: string): string {
      ...
      if (parts.length !== 4 || parts[0] !== ENCRYPTION_VERSION) throw new WebhookCryptoDecryptError()
      try {
