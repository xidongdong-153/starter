...
import { createWebhookSigningSecret, type WebhookCrypto } from './webhook.crypto.js'
import type { AiWebhookDeliveryRecord, AiWebhookEndpointRecord, AiWebhookRepository } from './webhook.repository.js'
...
function endpointNotFound(): AppError {
  return new AppError(ApiErrorCodes.AI_WEBHOOK_ENDPOINT_NOT_FOUND, 'Webhook 端点不存在', 404)
}
...
export function toWebhookEndpoint(record: AiWebhookEndpointRecord): AiWebhookEndpoint {
  return {
...
function toWebhookDelivery(record: AiWebhookDeliveryRecord): AiWebhookDeliveryList['items'][number] {
  return {
    id: record.id,
    ...
    eventProtocolVersion: record.eventProtocolVersion as AiWebhookDeliveryList['items'][number]['eventProtocolVersion'],
...
export function createAiWebhookService(input: {
  ...
  async function assertAllowedUrl(url: string): Promise<string> {
    try {
      ...
      if (error instanceof AiUrlGuardError) {
        throw new AppError(ApiErrorCodes.AI_CONFIG_INVALID, `Webhook URL 不允许（${error.reason}）`, 400)
      }
      throw error
    }
  }
  ...
  async function createEndpoint(data: CreateAiWebhookEndpointInput, actorId: string): Promise<AiWebhookEndpointSecret> {
    ...
    logger.info({ endpointId: record.id, appId: data.appId }, 'Webhook 端点已创建')
  ...
  async function updateEndpoint(
    ...
    const url = data.url !== undefined ? await assertAllowedUrl(data.url) : undefined
  ...
  function rotateEndpoint(endpointId: string, actorId: string): AiWebhookEndpointSecret {
    requireEndpoint(endpointId)
    if (!crypto.available) throw credentialKeyUnavailable()
    const signingSecret = createWebhookSigningSecret()
  ...
  async function testEndpoint(endpointId: string): Promise<AiWebhookTestResult> {
    const record = requireEndpoint(endpointId)
    let secret: string
    try {
  ...
  function listDeliveries(query: AiWebhookDeliveryQuery): AiWebhookDeliveryList {
