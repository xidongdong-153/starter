import type { AgentDefinitionConfig, AgentDefinitionDetail, AgentDefinitionSummary } from '@starter/contracts'
...
export function toAgentDefinitionSummary(record: AiAgentDefinitionRecord): AgentDefinitionSummary {
  return {
    id: record.id,
    name: record.name,
...
export function toAgentDefinitionDetail(record: AiAgentDefinitionRecord): AgentDefinitionDetail {
  return {
...
export function parseAgentDefinitionConfig(configJson: string): AgentDefinitionConfig {
...
function invalidStoredConfig(): AppError {
  return new AppError(ApiErrorCodes.SYSTEM_INTERNAL_ERROR, 'Agent 配置数据无效', 500)
}
