...
export function createAiAttachmentRepository(db: AppDatabase): AiAttachmentRepository {
  ...
  function findById(id: string): AiAttachmentRecord | undefined {
    return db.select().from(aiAttachments).where(eq(aiAttachments.id, id)).get()
  }

  function findByIds(ids: string[]): AiAttachmentRecord[] {
    if (ids.length === 0) return []
    return db.select().from(aiAttachments).where(inArray(aiAttachments.id, ids)).all()
  }

  function deleteById(id: string): AiAttachmentRecord | undefined {
    return db.delete(aiAttachments).where(eq(aiAttachments.id, id)).returning().get()
  }
