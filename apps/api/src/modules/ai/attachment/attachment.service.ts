...
import type { AiAttachmentRecord, AiAttachmentRepository } from './attachment.repository.js'
...
export function createAiAttachmentService(deps: {
  ...
  function attachmentNotFound(): AppError {
    return new AppError(ApiErrorCodes.AI_ATTACHMENT_NOT_FOUND, '附件不存在', 404)
  }
  ...
  async function upload(input: {
    ...
    if (!mimeType.success) {
      throw new AppError(ApiErrorCodes.AI_ATTACHMENT_TYPE_NOT_ALLOWED, '只支持 JPEG、PNG、WebP、GIF 格式的图片', 400)
    }
    if (file.size <= 0) {
      throw new AppError(ApiErrorCodes.COMMON_INVALID_REQUEST, '图片内容为空', 400)
    }
    if (file.size > MAX_ATTACHMENT_SIZE_BYTES) {
      throw new AppError(ApiErrorCodes.AI_ATTACHMENT_TOO_LARGE, '图片不能超过 5MB', 400)
    }
    if (sessionId !== null) {
    ...
    const stored = await deps.storage.write(principal.principalId, file.name, new Uint8Array(await file.arrayBuffer()))
    try {
      const record = await deps.repository.create({
        id: stored.fileId,
        ownerUserId: principal.kind === 'starter_user' ? principal.principalId : null,
  ...
  async function readContent(access: RuntimeAccessContext, attachmentId: string): Promise<AiAttachmentContent> {
    const record = await deps.repository.findById(attachmentId)
    if (!record || !isAiAttachmentOwnedBy(record, access)) throw attachmentNotFound()
  ...
  async function remove(access: RuntimeAccessContext, attachmentId: string): Promise<{ ok: true }> {
    const record = await deps.repository.findById(attachmentId)
    if (!record || !isAiAttachmentOwnedBy(record, access)) throw attachmentNotFound()
...
export function isAiAttachmentOwnedBy(record: AiAttachmentRecord, access: RuntimeAccessContext): boolean {
  if (access.principal.kind === 'starter_user') {
    return record.principalKind === 'starter_user' && record.ownerUserId === access.principal.principalId
  }
  return record.principalKind === 'product_app' && record.appId !== null && record.appId === access.principal.appId
}
