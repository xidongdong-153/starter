...
import { getAiUsageAuditRoute, listAiUsageAuditRoute } from './usage-audit.openapi.js'
...
export function createAiUsageAuditRoute(deps: {
  ...
  return new OpenAPIHono<HonoEnv>()
    .openapi(
      ...
      (c) => c.json(createSuccessResponse(service.listModelCalls(c.req.valid('query')), c.var.requestId), 200),
    )
    .openapi(
      {
      ...
      (c) => {
        ...
        if (!item) {
          throw new AppError(ApiErrorCodes.COMMON_NOT_FOUND, '未找到这条模型调用记录', 404)
        }
