...
type MergeableDraft = Extract<RunEventDraft, { type: 'message.delta' | 'tool.progress' }>
...
function mergeBytes(draft: MergeableDraft): number {
  return Buffer.byteLength(draft.type === 'message.delta' ? draft.data.delta : draft.data.summary, 'utf8')
}
...
function mergeDraft(pending: MergeableDraft, next: MergeableDraft): MergeableDraft {
