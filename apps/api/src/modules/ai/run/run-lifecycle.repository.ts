...
export function createAiRunLifecycleRepository(db: AppDatabase): AiRunLifecycleRepository {
  ...
  function completeTurn(id: string, outcome: Exclude<RunTurnOutcome, 'running'>, finishedAt: Date): void {
  ...
  function listRunning(runId: string) {
    return {
      turns: db
        ...
        .from(aiRunTurns)
        .where(and(eq(aiRunTurns.runId, runId), eq(aiRunTurns.outcome, 'running')))
        .all()
        .map((row) => row.id),
      steps: db
        .select({ id: aiRunSteps.id })
        .from(aiRunSteps)
        .where(and(eq(aiRunSteps.runId, runId), eq(aiRunSteps.outcome, 'running')))
        .all()
        .map((row) => row.id),
    }
  }
  return {
    ...
    listRunning,
    listTurns: (runId) =>
      db.select().from(aiRunTurns).where(eq(aiRunTurns.runId, runId)).orderBy(asc(aiRunTurns.turnIndex)).all(),
    listSteps: (runId) =>
      db
        .select()
        .from(aiRunSteps)
