...
export const listAiApplicationsRoute = createRoute({
  ...
  security,
  responses: {
    200: apiSuccessResponse(z.array(aiApplicationSchema), 'AI 产品应用列表', 'AiApplicationListResponse'),
...
export const createAiApplicationRoute = createRoute({
  ...
  responses: {
    200: apiSuccessResponse(aiApplicationSecretSchema, '新建应用及一次性 secret', 'CreatedAiApplicationResponse'),
...
export const rotateAiApplicationRoute = createRoute({
  ...
  tags,
  security,
  description: '轮换 secret，不改变 tenantId/projectId。secret 只在本次响应返回。',
  request: { params: aiApplicationParamsSchema },
  responses: {
    200: apiSuccessResponse(aiApplicationSecretSchema, '应用及一次性新 secret', 'RotatedAiApplicationResponse'),
    401: unauthorizedResponse,
    403: forbiddenResponse,
