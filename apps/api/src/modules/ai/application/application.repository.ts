...
export function createAiApplicationRepository(db: AppDatabase) {
  return {
    createWithAudit(
      input: typeof aiAppCredentials.$inferInsert,
      requestId: string | null,
    ): AiAppCredentialRecord {
      return db.transaction((tx) => {
        const record = tx
          .insert(aiAppCredentials)
          .values(input)
          .returning()
          .get();
    ...
    findById(id: string): AiAppCredentialRecord | undefined {
      return db
        .select()
        .from(aiAppCredentials)
        .where(eq(aiAppCredentials.id, id))
        .get();
    },
    findActiveByPrefix(prefix: string): AiAppCredentialRecord[] {
      return db
        ...
        .where(
          and(
            eq(aiAppCredentials.secretPrefix, prefix),
            eq(aiAppCredentials.status, "active"),
          ),
        )
        .all();
    },
