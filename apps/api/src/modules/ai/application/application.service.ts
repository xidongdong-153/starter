...
export function createAiApplicationService(input: {
  repository: ReturnType<
    typeof import("./application.repository.js").createAiApplicationRepository
  >;
  logger: Logger;
}) {
  ...
  function rotate(
    ...
    if (current.status !== "active") {
      throw new AppError(
        ApiErrorCodes.AI_APP_CREDENTIAL_REVOKED,
        "应用凭据已撤销",
        409,
      );
    }
    const generated = createAppSecret();
    const now = new Date();
    const record = repository.replaceSecret(
      appId,
      actorId,
      generated.hash,
      generated.prefix,
