...
import { ApiErrorCodes, aiAttachmentMimeTypeSchema, uuidSchema } from '@starter/contracts'

import type { AiAgentSessionRecord } from './session.repository.js'

type AssistantItem = Extract<AgentTranscriptItem, { type: 'assistant_message' }>
 ...
 */
export function collectStructuredOutputIds(entries: readonly Entry[]): string[] {
...
function projectMessage(
  ...
  if (message.role === 'toolResult') {
    ...
    const status = details?.status ?? (message.isError ? 'failed' : 'succeeded')
    ...
    const structuredOutput = structuredOutputId ? structuredOutputs?.get(structuredOutputId) : undefined
    const item = {
...
function readAttachmentIds(message: AgentMessage): string[] {
  ...
  return value.filter((id): id is string => typeof id === 'string' && uuidSchema.safeParse(id).success)
}
...
function userContentToString(content: string | (TextContent | ImageContent)[]): string {
  if (typeof content === 'string') return content
  return textBlocks(content.filter((block): block is { type: 'text'; text: string } => block.type === 'text'))
}

/**
 * 拼接 assistant message 的 text 块；thinking / toolCall 块不进正文。
 */
export function assistantContentToString(content: Extract<AgentMessage, { role: 'assistant' }>['content']): string {
  return textBlocks(content.filter((block): block is { type: 'text'; text: string } => block.type === 'text'))
}
 ...
 */
function assistantBlocks(content: Extract<AgentMessage, { role: 'assistant' }>['content']): AssistantItem['blocks'] {
...
function assistantStatus(message: Extract<AgentMessage, { role: 'assistant' }>): AssistantItem['status'] | null {
