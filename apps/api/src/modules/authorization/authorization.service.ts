...
import type { AuthorizationRepository, AuthorizationWriteContext } from './authorization.repository.js'
...
export function createAuthorizationService(repository: AuthorizationRepository) {
  async function getCurrent(userId: string): Promise<CurrentPermissions> {
    ...
    return toCurrentPermissions(authorization.roleKeys, authorization.permissionKeys)
  }
  ...
  async function listRoles(status: RoleCatalogStatus = 'active'): Promise<AuthorizationRoleCatalog> {
    const result = await repository.listRoleCatalog(status)
    return toAuthorizationRoleCatalog(result.activeRoles, result.activePermissions, result.permissionAssignments)
  }

  function throwActorNotPlatformAdmin(): never {
    throw new AppError(ApiErrorCodes.AUTH_FORBIDDEN, '只有平台管理员可以修改授权关系', 403)
  }

  function createRole(context: AuthorizationWriteContext, input: CreateRoleInput): AuthorizationRole {
    const result = repository.createRole(input, context)
    if (result.kind === 'invalid-permission-keys') {
      throw new AppError(ApiErrorCodes.COMMON_INVALID_REQUEST, '权限不存在或已归档', 400, {
    ...
    if (result.kind === 'role-key-conflict') {
      throw new AppError(ApiErrorCodes.AUTH_ROLE_KEY_CONFLICT, '角色 key 已被占用', 409)
    }
    return toAuthorizationRole(result.role, result.permissionKeys)
  }

  function updateRole(context: AuthorizationWriteContext, roleKey: string, input: UpdateRoleInput): AuthorizationRole {
  ...
  function archiveRole(context: AuthorizationWriteContext, roleKey: string): AuthorizationRole {
    ...
    if (result.kind === 'role-in-use') {
      throw new AppError(ApiErrorCodes.AUTH_ROLE_IN_USE, '角色仍有用户分配，不能归档', 409, {
  ...
  function restoreRole(context: AuthorizationWriteContext, roleKey: string): AuthorizationRole {
  ...
  async function getRoleImpact(roleKey: string): Promise<AuthorizationRoleImpact> {
  ...
  async function getPermissionImpact(permissionKey: Permission): Promise<AuthorizationPermissionImpact> {
    ...
    return toAuthorizationPermissionImpact(permissionKey, result.roleKeys, result.affectedUserCount)
  }
  ...
  function replaceUserRoles(
    ...
    if (context.actorId === targetUserId) {
      throw new AppError(ApiErrorCodes.AUTH_FORBIDDEN, '不能修改自己的角色', 403)
    }
    if (input.roleKeys.length === 0) {
      throw new AppError(ApiErrorCodes.COMMON_INVALID_REQUEST, '用户至少需要一个活动角色', 400)
    }

    const result = repository.replaceUserRoles(targetUserId, input.roleKeys, context)
    if (result.kind === 'user-not-found') {
      throw new AppError(ApiErrorCodes.COMMON_NOT_FOUND, '用户不存在', 404)
    }
    if (result.kind === 'invalid-role-keys') {
      throw new AppError(ApiErrorCodes.COMMON_INVALID_REQUEST, '角色不存在或已归档', 400, {
        invalidKeys: result.invalidKeys,
      })
    }
    if (result.kind === 'actor-not-platform-admin') {
    ...
    if (result.kind === 'last-platform-admin') {
      throw new AppError(ApiErrorCodes.AUTH_LAST_PLATFORM_ADMIN, '至少需要保留一个平台管理员', 409)
    }
  ...
  function replaceRolePermissions(
    ...
    if (roleKey === RoleKeys.ADMIN) {
      throw new AppError(ApiErrorCodes.AUTH_FORBIDDEN, '不能修改 admin 角色的权限', 403)
    }

    const result = repository.replaceRolePermissions(roleKey, input.permissionKeys, context)
    if (result.kind === 'role-not-found') {
      throw new AppError(ApiErrorCodes.COMMON_NOT_FOUND, '角色不存在', 404)
    }
    if (result.kind === 'invalid-permission-keys') {
  ...
  async function listAuditEvents(query: AuthorizationAuditQuery): Promise<AuthorizationAuditEventPage> {
