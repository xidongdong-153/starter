...
const currentPermissionsRoute = createRoute({
  ...
  security: [{ cookieAuth: [] }],
  responses: {
    200: apiSuccessResponse(currentPermissionsSchema, '当前用户的活动角色和权限', 'CurrentPermissionsResponse'),
...
const listAuthorizationUsersRoute = createRoute({
  ...
  security: [{ cookieAuth: [] }],
  responses: {
    200: apiSuccessResponse(authorizationUsersSchema, '用户及其活动角色', 'AuthorizationUsersResponse'),
    401: unauthorizedResponse,
...
const replaceUserRolesRoute = createRoute({
  ...
  responses: {
    200: apiSuccessResponse(authorizationUsersSchema.element, '更新后的用户角色', 'AuthorizationUserResponse'),
...
const createRoleRoute = createRoute({
  ...
  responses: {
    200: apiSuccessResponse(authorizationRoleSchema, '创建的自定义角色', 'CreateRoleResponse'),
    400: invalidRequestResponse,
    401: unauthorizedResponse,
    403: forbiddenResponse,
    409: conflictResponse,
...
const updateRoleRoute = createRoute({
  ...
  responses: {
    200: apiSuccessResponse(authorizationRoleSchema, '更新后的角色', 'UpdateRoleResponse'),
    400: invalidRequestResponse,
    401: unauthorizedResponse,
    403: forbiddenResponse,
    404: notFoundResponse,
...
const archiveRoleRoute = createRoute({
  ...
  responses: {
    200: apiSuccessResponse(authorizationRoleSchema, '归档后的角色', 'ArchiveRoleResponse'),
    400: invalidRequestResponse,
    401: unauthorizedResponse,
    403: forbiddenResponse,
    404: notFoundResponse,
...
const restoreRoleRoute = createRoute({
  ...
  responses: {
    200: apiSuccessResponse(authorizationRoleSchema, '恢复后的角色', 'RestoreRoleResponse'),
    400: invalidRequestResponse,
    401: unauthorizedResponse,
    403: forbiddenResponse,
    404: notFoundResponse,
...
const roleImpactRoute = createRoute({
  ...
  responses: {
    200: apiSuccessResponse(authorizationRoleImpactSchema, '角色当前分配用户数', 'AuthorizationRoleImpactResponse'),
    400: invalidRequestResponse,
    401: unauthorizedResponse,
    403: forbiddenResponse,
    404: notFoundResponse,
...
const replaceRolePermissionsRoute = createRoute({
  ...
  responses: {
    200: apiSuccessResponse(authorizationRoleSchema, '更新后的角色权限', 'AuthorizationRoleResponse'),
    400: invalidRequestResponse,
    401: unauthorizedResponse,
    403: forbiddenResponse,
    404: notFoundResponse,
...
export function createAuthorizationRoute(runtime: AppRuntime) {
  ...
  const requireAuthorizationRead = createRequirePermission(runtime.db, PermissionKeys.AUTHORIZATION_READ)
  const requireAuthorizationManage = createRequirePermission(runtime.db, PermissionKeys.AUTHORIZATION_MANAGE)
  const requireAuditRead = createRequirePermission(runtime.db, PermissionKeys.AUTHORIZATION_AUDIT_READ)
  const service = createAuthorizationService(createAuthorizationRepository(runtime.db))
  const app = new OpenAPIHono<HonoEnv>()

    .openapi({ ...currentPermissionsRoute, middleware: requireAuth }, async (c) =>
      c.json(createSuccessResponse(await service.getCurrent(c.var.currentUserId), c.var.requestId), 200),
    )
    ...
    .openapi(
      ...
      async (c) => c.json(createSuccessResponse(await service.listUsers(), c.var.requestId), 200),
    )
    ...
    .openapi(
      ...
      async (c) =>
        c.json(createSuccessResponse(await service.listRoles(c.req.valid('query').status), c.var.requestId), 200),
    )
    ...
    .openapi(
      ...
      async (c) =>
        c.json(createSuccessResponse(await service.getRoleImpact(c.req.valid('param').roleKey), c.var.requestId), 200),
    )
    ...
    .openapi(
      ...
      async (c) =>
        c.json(
          createSuccessResponse(await service.getPermissionImpact(c.req.valid('param').permissionKey), c.var.requestId),
          200,
        ),
    )
    ...
    .openapi(
      ...
      async (c) =>
        c.json(createSuccessResponse(await service.listAuditEvents(c.req.valid('query')), c.var.requestId), 200),
    )
