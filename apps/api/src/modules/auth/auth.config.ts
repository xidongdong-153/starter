...
import { buildResetPasswordEmail, buildVerificationEmail } from './auth.mail.js'
import { profiles, roles, user, userRoles } from '@api/infra/db/schema/index.js'
...
export function createAuth(db: AppDatabase, env: AppEnv, logger: AppLogger, mailer: Mailer) {
  return betterAuth({
    appName: 'Starter',
    basePath: '/api/auth',
    ...
    logger: {
      ...
      log(level: AuthLogLevel, message: string, ...args: unknown[]) {
        ...
        logger[level]({ ...(err && { err }), event: 'better_auth.log' }, `Better Auth: ${message}`)
      },
    },
      ...
      session: {
        create: {
          before: async (newSession) => {
            const target = db.select({ status: user.status }).from(user).where(eq(user.id, newSession.userId)).get()
          ...
          after: async (newUser) => {
            ...
            db.transaction((tx) => {
              const defaultRole = tx
                .select({ id: roles.id })
                .from(roles)
                .where(and(eq(roles.key, RoleKeys.OPERATOR), isNull(roles.archivedAt)))
                .get()
              if (!defaultRole) {
                throw new Error('默认角色 operator 不存在，请先执行数据库 migration')
              }

              tx.insert(profiles).values({ userId: newUser.id, createdAt: now, updatedAt: now }).run()
              tx.insert(userRoles)
                .values({
                  userId: newUser.id,
