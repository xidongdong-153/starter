...
import { apiSuccessResponse, unauthorizedResponse } from '@api/openapi/responses.js'
import { authConfigSchema, currentSessionSchema, registerAuthOpenApiComponents } from './auth.openapi.js'
...
const authConfigRoute = createRoute({
  ...
  tags: ['Auth'],
  responses: {
    200: apiSuccessResponse(authConfigSchema, '可用的登录方式', 'AuthConfigResponse'),
  },
})
...
const meRoute = createRoute({
  ...
  security: [{ cookieAuth: [] }],
  responses: {
    200: apiSuccessResponse(currentSessionSchema, '当前登录用户和 session', 'CurrentSessionResponse'),
...
export function createAuthRoute(runtime: AppRuntime) {
  ...
  const app = new OpenAPIHono<HonoEnv>()
    .openapi(authConfigRoute, (c) => {
      const data: AuthConfig = {
        providers: {
          email: true,
          github: Boolean(runtime.env.GITHUB_CLIENT_ID && runtime.env.GITHUB_CLIENT_SECRET),
          google: Boolean(runtime.env.GOOGLE_CLIENT_ID && runtime.env.GOOGLE_CLIENT_SECRET),
        },
      }
        ...
        createSuccessResponse(
          {
            user: {
              ...
              status: session.user.status === 'suspended' ? 'suspended' : 'active',
  ...
  app.on(['GET', 'POST'], '/api/auth/*', (c) => runtime.auth.handler(c.req.raw))
