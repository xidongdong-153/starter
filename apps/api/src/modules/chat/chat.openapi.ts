...
export const listChatAgentsRoute = createRoute({
  ...
  responses: {
    200: genericSuccessResponse('已启用的 Agent 列表；data 与 GET /api/ai/agents 同构'),
...
export const listChatSessionsRoute = createRoute({
  ...
  responses: {
    200: genericSuccessResponse('Agent Session 列表；data 与 GET /api/ai/sessions 同构'),
    401: unauthorizedResponse,
  },
})
...
export const createChatSessionRoute = createRoute({
  ...
  responses: {
    200: genericSuccessResponse('创建的 Agent Session；data 与 POST /api/ai/sessions 同构'),
...
export const getChatSessionRoute = createRoute({
  ...
  responses: {
    200: genericSuccessResponse('Agent Session 详情；data 与 GET /api/ai/sessions/{sessionId} 同构'),
    401: unauthorizedResponse,
    404: notFoundResponse,
  },
})
...
export const updateChatSessionRoute = createRoute({
  ...
  responses: {
    200: genericSuccessResponse('更新的 Agent Session；data 与 PATCH /api/ai/sessions/{sessionId} 同构'),
    400: invalidRequestResponse,
    401: unauthorizedResponse,
    404: notFoundResponse,
    409: conflictResponse,
...
export const deleteChatSessionRoute = createRoute({
  ...
  request: { params: sessionParams },
  responses: {
    200: genericSuccessResponse('归档后的 Agent Session；data 与 DELETE /api/ai/sessions/{sessionId} 同构'),
    401: unauthorizedResponse,
    404: notFoundResponse,
  },
})
...
export const getChatRunRoute = createRoute({
  ...
  responses: {
    200: genericSuccessResponse('Agent Run；data 与 GET /api/ai/sessions/{sessionId}/runs/{runId} 同构'),
    400: invalidRequestResponse,
    401: unauthorizedResponse,
    404: notFoundResponse,
  },
...
export const abortChatRunRoute = createRoute({
  ...
  request: { params: runParams },
  responses: {
    200: genericSuccessResponse('Agent Run；data 与 POST /api/ai/sessions/{sessionId}/runs/{runId}/abort 同构'),
    401: unauthorizedResponse,
    404: notFoundResponse,
    409: conflictResponse,
  },
  ...
  responses: {
    200: {
      content: {
        ...
        'application/json': {
          schema: apiSuccessSchema(startAgentRunJsonSchema, 'AgentRunStartJsonResponse'),
        },
      },
      description: [
        'Agent Run RunEvent SSE 流。SSE id 对应 eventId，event 对应 type，data 是完整 RunEvent JSON。',
...
export const uploadChatAttachmentRoute = createRoute({
  ...
  responses: {
    201: genericSuccessResponse('上传成功的附件；data 与 POST /api/ai/attachments 同构'),
    400: invalidRequestResponse,
    401: unauthorizedResponse,
    404: notFoundResponse,
  },
