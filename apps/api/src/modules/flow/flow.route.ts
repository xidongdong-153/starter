...
export function createFlowRoute(runtime: AppRuntime, services: AiServices) {
  ...
  return new OpenAPIHono<HonoEnv>()
    ...
    .openapi(
      { ...createFlowSessionRoute, middleware: requireAuth },
      async (c) =>
        c.json(
          createSuccessResponse(
    ...
    .openapi(
      { ...getFlowSessionTranscriptRoute, middleware: requireAuth },
      async (c) =>
        c.json(
          createSuccessResponse(
    ...
    .openapi(
      { ...getFlowRunStructuredOutputsRoute, middleware: requireAuth },
      (c) =>
        c.json(
          createSuccessResponse(
            services.runService.structuredOutputs(
              access(c),
              c.req.valid("param").sessionId,
              c.req.valid("param").runId,
            ),
