...
export function createProfileRepository(db: AppDatabase) {
  ...
  function setAvatar(userId: string, fileId: string | null): void {
    db.update(profiles).set({ avatarFileId: fileId, updatedAt: new Date() }).where(eq(profiles.userId, userId)).run()
  }
...
export type ProfileRecord = NonNullable<Awaited<ReturnType<ProfileRepository['findByUserId']>>>
