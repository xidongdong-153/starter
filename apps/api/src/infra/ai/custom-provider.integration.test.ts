...
describe('custom Provider real protocol streams', () => {
  ...
  it('custom model 复用 Pi native stream 的 agent_run 审计', async () => {
    ...
    try {
      const { model, models } = createGateway('openai-completions', upstream.url)
      const audit = {
  ...
  it.each(protocols)('%s auth failure is exposed as a safe error event', async (protocol) => {
    const upstream = await startFakeUpstream(protocol, 'auth')
    try {
      const { gateway, model } = createGateway(protocol, upstream.url)
      await expect(collect(gateway.stream(gatewayInput(model)))).rejects.toMatchObject({
        kind: 'auth',
      })
    } finally {
      await upstream.close()
  ...
  it.each(protocols)('%s timeout failure is surfaced without hanging', async (protocol) => {
    const upstream = await startFakeUpstream(protocol, 'timeout')
    try {
      const { gateway, model } = createGateway(protocol, upstream.url)
      const failure = collect(gateway.stream({ ...gatewayInput(model), timeoutMs: 30 }))
  ...
  it.each(protocols)('%s upstream failure is surfaced without raw payload', async (protocol) => {
...
async function startFakeUpstream(protocol: Protocol, mode: Mode): Promise<{ url: string; close: () => Promise<void> }> {
