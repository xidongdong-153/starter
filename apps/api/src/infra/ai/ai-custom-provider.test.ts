...
describe("custom Provider runtime", () => {
  it.each([
    ...
    "openai-responses",
    "anthropic-messages",
  ] as const)("把 %s 固定映射到同名 pi-ai API", (protocol) => {
...
describe("ai URL guard", () => {
  it("允许显式 CIDR 中的生产私网 HTTP，并始终拒绝回环、link-local 和 metadata", async () => {
    ...
    await expect(guard.assertAllowed("http://10.2.3.4/v1")).resolves.toEqual(
      new URL("http://10.2.3.4/v1"),
    );
    await expect(
      guard.assertAllowed("http://93.184.216.34/v1"),
    ).rejects.toMatchObject({ reason: "scheme" });
    await expect(
      guard.assertAllowed("https://127.0.0.1/v1"),
    ).rejects.toMatchObject({ reason: "private" });
    await expect(
      guard.assertAllowed("https://169.254.170.2/v1"),
    ).rejects.toMatchObject({ reason: "private" });
    await expect(
      guard.assertAllowed("https://169.254.169.254/v1"),
    ).rejects.toMatchObject({ reason: "private" });
  });
