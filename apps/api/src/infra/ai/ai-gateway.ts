...
export function createAiGateway(
  ...
  return {
    async *stream(input) {
      const model = models.getModel(input.model.providerId, input.model.modelId)
      ...
      const signal = AbortSignal.any(input.signal ? [input.signal, timeoutSignal] : [timeoutSignal])
      ...
      try {
        ...
        const stream = models.streamSimple(model, createSdkContext(input, model), {
          signal,
          env,
        ...
        for await (const event of stream) {
          ...
          if (abortState.cause) {
            throw new AiGatewayError(abortState.cause, {
              usage,
              cost,
              stopReason: event.reason === 'deferred' ? 'deferred' : normalizeStopReason(event.reason),
            })
          }
          ...
          const assistantMessage = toModelAssistantMessage(event.message, input.turnIndex)
          const stopReason = normalizeStopReason(event.reason)
          const completedToolCalls = validateCompletedToolCalls(assistantMessage, toolCalls, stopReason, usage, cost)
...
function createSdkContext(input: AiGatewayInput, model: Model<string>): Context {
  return {
...
function toModelAssistantMessage(message: AssistantMessage, turnIndex: number): AiModelAssistantMessage {
...
function validateCompletedToolCalls(
  ...
  const finalCalls = assistantMessage.blocks.filter((block): block is AiModelToolCall => block.type === 'tool_call')
...
function contentMetadata(turnIndex: number, contentIndex: number): AiModelContentMetadata {
  return {
    turnIndex,
    contentIndex,
...
function errorFromStreamEvent(
  ...
  if (abortCause === 'aborted' || (!abortCause && inputSignal?.aborted) || reason === 'aborted') {
    return new AiGatewayError('aborted', details)
  }
  if (isAuthStatusMessage(errorMessage)) return new AiGatewayError('auth', details)
...
function isAbortError(error: unknown): boolean {
  return (
    (error instanceof DOMException && error.name === 'AbortError') || (isRecord(error) && error.name === 'AbortError')
  )
}
