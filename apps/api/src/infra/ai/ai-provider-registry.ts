...
import type { AiAuthMode, AiProviderConfigField, CustomAiProviderDefinition } from '@starter/contracts'
...
const overrides: Record<string, ProviderOverride> = {
  ...
  'azure-openai-responses': {
    configFields: [
      ...
      field('AZURE_OPENAI_API_VERSION', 'API Version', '可选的 Azure OpenAI API version。', 'text', false),
      field(
  ...
  'cloudflare-ai-gateway': {
    configFields: [
      ...
      field('CLOUDFLARE_GATEWAY_ID', 'Gateway ID', 'Cloudflare AI Gateway ID。'),
    ],
  },
  'cloudflare-workers-ai': {
    configFields: [field('CLOUDFLARE_ACCOUNT_ID', 'Account ID', 'Cloudflare account ID。')],
  },
  'google-vertex': {
    configFields: [
      field('GOOGLE_CLOUD_PROJECT', 'Project ID', 'Google Cloud project ID。使用 API Key、ADC 或服务账号时都需要。'),
      field('GOOGLE_CLOUD_LOCATION', 'Location', 'Google Cloud location，例如 us-central1。'),
    ],
    setupInstructions: [
  ...
  radius: {
    configFields: [
      field('RADIUS_GATEWAY_URL', 'Gateway URL', 'Radius gateway 地址。修改后需要重新检查和刷新模型。', 'url', false),
    ],
    setupInstructions: [
...
export function createCustomAiProviderDefinition(
  ...
  return {
    ...
    configFields: [],
    setupInstructions: ['可选保存 API Key；未保存凭据时按 keyless Provider 处理。'],
    supportsModelRefresh: false,
  }
}
export function findAiProviderDefinition(providerId: string): AiProviderDefinition | undefined {
  return createAiProviderRegistry().find((provider) => provider.id === providerId)
}
...
function toDefinition(provider: Provider): AiProviderDefinition {
  ...
  const setupInstructions = override?.setupInstructions ?? defaultInstructions(provider)
  return {
    id: provider.id,
    name: provider.name,
    kind: 'built_in',
