...
it("runtime 启动恢复、热加载、卸载和新实例恢复使用同一数据库 definition", async () => {
  ...
  try {
    ...
    expect(runtime.ai.listModels("runtime-custom")).toEqual([
      expect.objectContaining({ modelId: "first-model" }),
    ]);
    ...
    runtime.database.sqlite
      ...
      .run(
        JSON.stringify(definition(baseUrl, "second-model")),
        now.getTime() + 1,
        "runtime-custom",
      );
    runtime.ai.reloadProvider("runtime-custom");
    expect(runtime.ai.listModels("runtime-custom")).toEqual([
      expect.objectContaining({ modelId: "second-model" }),
    ]);
    expect(runtime.ai.providers).toContainEqual(
      expect.objectContaining({ id: "runtime-custom", revision: 2 }),
    );

    runtime.ai.unloadProvider("runtime-custom");
    expect(runtime.ai.listModels("runtime-custom")).toEqual([]);
    expect(runtime.ai.providers).not.toContainEqual(
      expect.objectContaining({ id: "runtime-custom" }),
    );

    const restarted = createAiRuntime(
      runtime.db,
      createAiCrypto("MDEyMzQ1Njc4OWFiY2RlZjAxMjM0NTY3ODlhYmNkZWY="),
      { appEnv: "test" },
    );
    await restarted.ensureReady();
    expect(restarted.listModels("runtime-custom")).toEqual([
      expect.objectContaining({ modelId: "second-model" }),
    ]);
  } finally {
    cleanup();
...
it("启动时隔离单条坏 definition，不阻断其他 custom Provider", async () => {
  ...
  try {
    ...
    await runtime.ai.ensureReady();
    expect(runtime.ai.listModels("runtime-good")).toEqual([
      expect.objectContaining({ providerId: "runtime-good" }),
    ]);
    ...
    expect(
      runtime.db
        ...
        .where(eq(aiProviderConfigs.providerId, "runtime-broken"))
        .get(),
    ).toMatchObject({
      enabled: false,
      authStatus: "error",
...
it("runtime probe 按真实 HTTP status 分类 auth、timeout 和 upstream", async () => {
  ...
  try {
    ...
    runtime.db
      ...
      .run();
    const encrypted = createAiCrypto(
    ...
    for (const item of [
      ...
      status = item.status;
      await expect(
        runtime.ai.checkAuth("runtime-custom"),
      ).rejects.toMatchObject(new AiRuntimeError(item.kind));
    }
  } finally {
    cleanup();
...
it("bounded JSON 拒绝超过最大深度的 definition", () => {
  ...
  expect(() => parseBoundedJson(JSON.stringify(value))).toThrow(
    "JSON exceeds maximum depth",
  );
  expect(() => parseBoundedJson(JSON.stringify({ a: { b: 1 } }), 2)).toThrow(
    "JSON exceeds maximum depth",
  );
