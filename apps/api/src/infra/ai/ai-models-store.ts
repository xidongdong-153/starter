...
export class AiModelsStore implements ModelsStore {
  ...
  async read(providerId: string, options?: ModelsStoreOperationOptions): Promise<ModelsStoreEntry | undefined> {
    ...
    const row = this.db.select().from(aiModelCatalogs).where(eq(aiModelCatalogs.providerId, providerId)).get()
    throwIfAborted(options?.signal)
  ...
  async delete(providerId: string, options?: ModelsStoreOperationOptions): Promise<void> {
    throwIfAborted(options?.signal)
    this.db.delete(aiModelCatalogs).where(eq(aiModelCatalogs.providerId, providerId)).run()
    throwIfAborted(options?.signal)
  }
}
...
function parseModels(json: string): readonly Model<Api>[] {
  ...
  if (!Array.isArray(value) || !value.every(isModel)) throw new Error('AI model catalog is invalid')
  return value
}
