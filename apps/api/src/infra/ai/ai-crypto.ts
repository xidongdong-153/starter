...
export function createAiCrypto(encodedKey: string | undefined): AiCrypto {
  ...
  return {
    ...
    encrypt(payload) {
      if (!key || key.byteLength !== 32) throw new AiCredentialKeyUnavailableError()

      const iv = randomBytes(12)
      const cipher = createCipheriv('aes-256-gcm', key, iv)
      const plaintext = Buffer.from(JSON.stringify(projectPayload(payload)), 'utf8')
    ...
    decrypt(columns) {
      if (!key || key.byteLength !== 32) throw new AiCredentialKeyUnavailableError()
      if (columns.encryptionVersion !== ENCRYPTION_VERSION) throw new AiCredentialDecryptError()

      try {
        const decipher = createDecipheriv('aes-256-gcm', key, Buffer.from(columns.payloadIv, 'base64'))
...
export function createCredentialHint(credential: Credential | undefined): string | null {
...
function parsePayload(value: unknown): AiEncryptedPayload {
  if (!isRecord(value) || !isStringRecord(value.runtimeSettings)) throw new AiCredentialDecryptError()

  const credential = value.credential
  if (credential !== undefined && !isCredential(credential)) throw new AiCredentialDecryptError()
