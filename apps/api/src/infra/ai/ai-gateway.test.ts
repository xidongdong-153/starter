...
async function collect(
  input: AsyncGenerator<AiGatewayEvent>,
): Promise<AiGatewayEvent[]> {
...
describe("ai gateway message mapper", () => {
  ...
  it("跨 block 的 text delta 保留 SDK 到达顺序，completed 保留 final block 顺序", async () => {
    ...
    const message = fauxAssistantMessage([
      fauxText("first block"),
      fauxText("second block"),
    ]);
    ...
    const events = await collect(
      gateway.stream({ model: modelRef, messages: [], turnIndex: 2 }),
    );
    expect(
      events
        .filter((event) => event.type === "text_delta")
        .map((event) => [event.contentIndex, event.text]),
    ).toEqual([
      [1, "second delta"],
      [0, "first delta"],
    ]);
    ...
    if (completed?.type === "completed") {
      expect(
        completed.assistantMessage.blocks.map((block) => block.contentIndex),
      ).toEqual([0, 1]);
    }
  });
  ...
  it.each([
    ...
    "toolcall_end 后 $stopReason 不发送工具完成事件",
    async ({ stopReason, expectedKind }) => {
      const { faux, gateway, modelRef } = createFixture();
      faux.setResponses([
      ...
      expect(events.some((event) => event.type === "tool_call_completed")).toBe(
        false,
      );
