import type { Api, ApiKeyCredential, Model, Provider, ProviderStreams } from '@earendil-works/pi-ai'
import { createProvider } from '@earendil-works/pi-ai'
import { anthropicMessagesApi } from '@earendil-works/pi-ai/api/anthropic-messages.lazy'
import { openAICompletionsApi } from '@earendil-works/pi-ai/api/openai-completions.lazy'
import { openAIResponsesApi } from '@earendil-works/pi-ai/api/openai-responses.lazy'

import type { CustomAiProviderDefinition, CustomAiProviderModel } from '@starter/contracts'
...
export function createCustomAiProvider(
  ...
  switch (definition.protocol) {
    case 'openai-completions':
      return createTypedProvider(definition, 'openai-completions', openAICompletionsApi(), guard.fetch)
    case 'openai-responses':
      return createTypedProvider(definition, 'openai-responses', openAIResponsesApi(), guard.fetch)
    case 'anthropic-messages':
      return createTypedProvider(definition, 'anthropic-messages', anthropicMessagesApi(), guard.fetch)
    default:
    ...
    auth: {
      apiKey: {
        ...
        resolve: async ({ credential, signal }) => {
          ...
          return key ? { auth: { apiKey: key }, source: 'stored credential' } : { auth: {}, source: 'keyless' }
        },
      },
    },
...
function withGuardedFetch(streams: ProviderStreams, fetch: typeof globalThis.fetch): ProviderStreams {
  return {
