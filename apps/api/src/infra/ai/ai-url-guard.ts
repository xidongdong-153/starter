...
export class AiUrlGuardError extends Error {
  constructor(readonly reason: 'scheme' | 'host' | 'private' | 'redirect' | 'timeout' | 'response_size') {
...
export function createAiUrlGuard(options: AiUrlGuardOptions = {}) {
  ...
  const allowedPrivateCidrs = (options.allowedPrivateCidrs ?? []).map(parseCidr)
  ...
  async function resolveAllowedAddress(input: string | URL): Promise<{
    ...
    if (url.protocol !== 'http:' && url.protocol !== 'https:') throw new AiUrlGuardError('scheme')
    if (url.username || url.password || url.search || url.hash) throw new AiUrlGuardError('host')
    ...
    const address = addresses.find(
      (item) => isAllowedAddress(item, allowedPrivateCidrs) || (appEnv !== 'production' && isLoopbackAddress(item)),
    )
    if (!address) throw new AiUrlGuardError('private')
    if (appEnv === 'production' && url.protocol !== 'https:' && isPublicAddress(address)) {
  ...
  async function fetch(input: string | URL | Request, init?: RequestInit): Promise<Response> {
    ...
    const source: string | URL = request ? request.url : (input as string | URL)
    ...
    const body = init?.body ?? (request ? await request.arrayBuffer() : undefined)
    ...
    try {
      ...
      if (response.status >= 300 && response.status < 400) throw new AiUrlGuardError('redirect')
      const length = response.headers.get('content-length')
      if (length && Number(length) > maxResponseBytes) throw new AiUrlGuardError('response_size')
      if (!response.body) return response
      bodyOwnsTimeout = true
      return limitResponseBody(response, maxResponseBytes, timeout, controller, signal)
    } catch (error) {
      if (controller.signal.aborted && !signal.aborted) throw new AiUrlGuardError('timeout')
      throw error
    } finally {
      ...
      async pull(streamController) {
        try {
          ...
          streamController.error(
            controller.signal.aborted && !callerSignal.aborted ? new AiUrlGuardError('timeout') : error,
          )
        }
      },
      async cancel(reason) {
...
async function requestWithPinnedAddress(input: {
  ...
  const body = input.body === undefined ? undefined : await bodyBytes(input.body)
  const requestHeaders = Object.fromEntries(input.headers.entries())
  if (body && !input.headers.has('content-length')) requestHeaders['content-length'] = String(body.byteLength)
  ...
  return new Promise<Response>((resolve, reject) => {
    const request = transport.request(
      ...
      (response) => {
        ...
        const bodyStream = ReadableStream.from(response as unknown as AsyncIterable<Uint8Array>)
        resolve(
...
function parseCidr(value: string): Cidr {
  ...
  if (!net.isIP(host) || !Number.isInteger(prefix) || prefix < 0 || prefix > bits) {
...
function isLoopbackAddress(address: string): boolean {
  if (net.isIP(address) === 4) return inCidr(ipToBigInt(address), ipToBigInt('127.0.0.0'), 8, 32)
...
function isAllowedAddress(address: string, allowed: readonly Cidr[]): boolean {
  ...
  return allowed.some(
    (item) => item.bits === (net.isIP(address) === 6 ? 128 : 32) && (value & item.mask) === item.network,
  )
}
...
function isForbiddenAddress(address: string): boolean {
  ...
  return ranges.some(([network, prefix, bits]) => inCidr(value, ipToBigInt(network), prefix, bits))
}
...
function isPublicAddress(address: string): boolean {
  ...
  if (net.isIP(address) === 4) {
    ...
    return !privateRanges.some(([network, prefix]) => inCidr(value, ipToBigInt(network), prefix, 32))
  }
  const privateRanges: readonly (readonly [string, number])[] = [
    ['::1', 128],
    ['fc00::', 7],
  ...
  return !privateRanges.some(([network, prefix]) => inCidr(value, ipToBigInt(network), prefix, 128))
}
...
function ipToBigInt(address: string): bigint {
  ...
  const normalized = address.includes('::') ? expandIpv6(address) : address.split(':')
  return normalized.reduce((result, part) => (result << 16n) | BigInt(Number.parseInt(part || '0', 16)), 0n)
}
...
function expandIpv6(address: string): string[] {
  ...
  const zeros: string[] = []
  for (let index = zeros.length; index < 8 - leftParts.length - rightParts.length; index += 1) zeros.push('0')
