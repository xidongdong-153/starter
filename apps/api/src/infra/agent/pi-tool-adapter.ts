...
export interface PiToolAdapterOptions {
  ...
  logger?: AppLogger
  onTerminalFailure?: (reason: 'timed_out' | 'cancelled' | 'storage_failed') => void
}
...
export function createPiToolAdapter(tools: readonly RegisteredAiTool[], options: PiToolAdapterOptions): PiToolAdapter {
  ...
  const agentTools = tools.map((tool) =>
    createAgentTool(tool, options, pendingFailures, pendingAudits, executionAttempts),
  )
  ...
  return {
    ...
    onToolExecutionStart(input) {
      ...
      const timeoutMs = effectiveTimeoutMs(tool?.timeoutMs ?? 5000, options.getRemainingRunMs?.())
    ...
    onToolExecutionEnd(input) {
      ...
      if (pending.finalized) {
        return (
          details ?? toolResultDetails(pending.status ?? 'failed', pending.errorCode ?? ApiErrorCodes.AI_TOOL_FAILED)
        )
      }

      const outcome = preflightToolFailure(pending)
      const outcomeStatus = outcome.status as Exclude<AgentToolStatus, 'interrupted'>
      pending.finalized = true
      pending.status = outcomeStatus
      pending.errorCode = outcome.errorCode
      finalizeToolAudit(options.audit, pending.handle, outcomeStatus, outcome.errorCode)
...
function createAgentTool(
  ...
  const runExecution = async (
    ...
    if (safeSerializeArguments(params) === null) {
      finalizeAudit('invalid_arguments', ApiErrorCodes.AI_TOOL_INVALID_ARGUMENTS)
      return failWithoutAudit(
        toolCallId,
        pendingFailures,
        'invalid_arguments',
    ...
    if (tool.requiredPermission) {
      ...
      try {
        ...
        if (execution.principal.kind === 'starter_user') {
          allowed = await options.hasPermission(execution.principal.principalId, tool.requiredPermission)
        }
      } catch {
        // 权限查询异常按拒绝处理，不降级允许。
        allowed = false
    ...
    try {
      ...
      if (
        ...
        (result.safeSummary !== null && (typeof result.safeSummary !== 'string' || result.safeSummary.length > 1000))
      ) {
      ...
      return {
        ...
        details: {
          ...
          ...(result.structuredOutputId ? { structuredOutputId: result.structuredOutputId } : {}),
        },
        terminate: result.terminate === true,
...
function preflightToolFailure(pending: PendingToolAudit): PiToolResultDetails {
  if (pending.signal?.aborted) {
    return toolResultDetails('cancelled', ApiErrorCodes.AI_TOOL_CANCELLED, 'The tool was cancelled.')
  }
  if (!pending.tool) {
    return toolResultDetails('not_found', ApiErrorCodes.AI_TOOL_NOT_FOUND, 'The requested tool is not available.')
  }
  ...
  return toolResultDetails('failed', ApiErrorCodes.AI_TOOL_FAILED, 'The tool failed.')
}
...
function readToolResultDetails(value: unknown): PiToolResultDetails | null {
  ...
  const errorCode =
    typeof details.errorCode === 'string' && Object.values(ApiErrorCodes).includes(details.errorCode as ApiErrorCode)
      ? (details.errorCode as ApiErrorCode)
      : null
  return {
    status,
    errorCode,
    safeSummary: typeof details.safeSummary === 'string' ? details.safeSummary.slice(0, 1000) : null,
    ...
    /^[0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/iu.test(details.structuredOutputId)
...
function effectiveTimeoutMs(toolTimeoutMs: number, remainingRunMs: number | undefined): number {
  return Math.max(1, Math.min(toolTimeoutMs, remainingRunMs ?? toolTimeoutMs))
}

function executeWithAbort<T>(promise: Promise<T>, signal: AbortSignal): Promise<T> {
