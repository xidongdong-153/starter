import type { AgentMessage, CompactionEntry, CustomEntry, Entry, Session } from '@earendil-works/pi-agent-core'
...
export interface AgentSessionHandle {
  ...
  appendMessage: (lane: string, message: AgentMessage, id?: string) => Promise<Extract<Entry, { type: 'message' }>>
  appendCompaction: (
    lane: string,
    entry: Omit<CompactionEntry, 'parentId' | 'seq' | 'timestamp'>,
  ) => Promise<CompactionEntry>
  ...
  findRunTerminalEntries: (options: { lane?: string; runId: string }) => Promise<CustomEntry[]>
}
...
export function createPiSessionStore(options: PiSessionStoreOptions): AgentSessionStore {
  ...
  async function metadataFor(sessionId: string): Promise<SqliteSessionMetadata> {
    const metadata = (await repository.list({ cwd: options.cwd })).find((item) => item.id === sessionId)
    if (!metadata) {
  ...
  async function open(sessionId: string): Promise<Session<SqliteSessionMetadata>> {
    return repository.open(await metadataFor(sessionId))
  }

  function createHandle(session: Session<SqliteSessionMetadata>, metadata: SqliteSessionMetadata): AgentSessionHandle {
    return {
      id: metadata.id,
      metadata,
      readTranscript: (readOptions = {}) => readTranscriptFromSession(session, readOptions),
      appendMessage: async (lane, message, id) => appendMessageToSession(session, lane, message, id),
      appendCompaction: async (lane, entry) => appendCompactionToSession(session, lane, entry),
      appendRunTerminalEntry: async (lane, data) => appendTerminalToSession(session, lane, data),
      createLane: (lane, at = null) => session.createLane(lane, at),
      findRunTerminalEntries: (findOptions) => findTerminalEntriesFromSession(session, findOptions),
    }
  }
  ...
  return {
    ...
    async listSessions() {
      return (await repository.list({ cwd: options.cwd })).map((item) => item.id)
    },
...
async function readTranscriptFromSession(
  ...
  return lane.findEntriesOnBranch({
    ...
    ...(options.cursor === undefined ? {} : { cursor: { afterSeq: options.cursor } }),
...
async function findTerminalEntriesFromSession(
  ...
  return entries.filter(
    (entry): entry is CustomEntry =>
      entry.type === 'custom' && isRecord(entry.data) && entry.data.runId === options.runId,
  )
}
