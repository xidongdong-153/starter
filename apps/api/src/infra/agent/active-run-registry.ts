...
export class ActiveRunRegistry {
  ...
  reserve(inputOrSessionId: ActiveRunReserveInput | string, lane?: string): ActiveRunLease {
    const input =
      typeof inputOrSessionId === 'string' ? { sessionId: inputOrSessionId, lane: lane ?? 'main' } : inputOrSessionId
  ...
  attach(lease: ActiveRunLease, runId: string, controls: AttachableActiveRunControls): ActiveRunHandle {
    ...
    const currentLease = this.bySessionLane.get(laneKey(lease.sessionId, lease.lane))
  ...
  release(runIdOrLease: string | ActiveRunLease): void {
    if (typeof runIdOrLease === 'string') {
      ...
      const lease = this.bySessionLane.get(laneKey(handle.sessionId, handle.lane))
      if (lease) {
        this.bySessionLane.delete(laneKey(handle.sessionId, handle.lane))
        this.handlesBySessionLane.delete(laneKey(handle.sessionId, handle.lane))
    ...
    this.bySessionLane.delete(laneKey(runIdOrLease.sessionId, runIdOrLease.lane))
    this.handlesBySessionLane.delete(laneKey(runIdOrLease.sessionId, runIdOrLease.lane))
    for (const [runId, handle] of this.byRunId) {
      if (handle.sessionId === runIdOrLease.sessionId && handle.lane === runIdOrLease.lane) {
