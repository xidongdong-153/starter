...
export type AiTelemetryOperation =
  "start_span" | "set_attributes" | "set_status" | "add_event";
...
export interface AiSpanScope<Name extends AiSpanName> {
  ...
  readonly close: (input?: {
    attributes?: AiSpanEndAttributes<Name>;
    status?: SpanStatus;
  }) => void;
}
...
export function createAiTelemetryContext(
  ...
  return {
    startSpan: (spanOptions, callback) =>
      safeStartSpan(context, spanOptions, callback, report),
  };
}
...
function safeStartSpan<T>(
  ...
  const invoke = (span: TelemetrySpan | undefined): Promise<T> => {
    ...
    business ??= (async () =>
      callback(span ? wrapSpan(span, options.name, report) : inertSpan()))();
    return business;
  };
  try {
...
function wrapSpan(
  ...
  return {
    startSpan: (options, callback) =>
      safeStartSpan(span, options, callback, report),
...
export function startAiSpan<Name extends AiSpanName, T>(
  ...
  return parent.startSpan(
    { name, attributes: attributes as SpanAttributes },
    (span) => callback(span as AiSpan<Name>),
  );
}
