...
export function runBootstrapAdmin(
  input: NodeJS.ProcessEnv = process.env,
  output: BootstrapOutput = console,
): 0 | 1 {
  ...
  try {
    ...
    if (!email) {
      throw new Error(
        "未配置 AUTH_BOOTSTRAP_ADMIN_EMAIL，请在 apps/api/.env.development 中填写已存在账号的邮箱",
      );
    }
    ...
    try {
      ...
      if (error instanceof Error && /no such table:/i.test(error.message)) {
        throw new Error(
          "授权表不存在，请先运行 pnpm --filter @starter/api db:migrate",
        );
      }
      throw error;
    }
