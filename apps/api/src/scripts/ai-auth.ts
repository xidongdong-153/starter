...
export async function runAiAuth(
  ...
  try {
    ...
    if (!command.logout && !crypto.available) {
      throw new Error('未配置 AI_CREDENTIAL_ENCRYPTION_KEY，不能保存 OAuth 凭据')
    }

    database = createDatabase(env.DATABASE_PATH)
    const runtime = createAiRuntime(database.db, crypto)
    await runtime.ensureReady()
    const provider = runtime.providers.find((item) => item.id === command.providerId)
    if (!provider) throw new Error('Provider 不存在')
    if (!provider.supportedAuthModes.includes('oauth')) throw new Error('这个 Provider 不支持 OAuth')

    const repository = createAiRepository(database.db)
    if (command.logout && repository.findProviderConfig(command.providerId)?.credentialType !== 'oauth') {
    ...
    if (error instanceof Error && /no such table:/iu.test(error.message)) {
      output.error('AI 配置表不存在，请先运行 pnpm --filter @starter/api db:migrate')
    } else {
      output.error(error instanceof Error ? error.message : 'AI 认证命令执行失败')
    }
    return 1
  } finally {
...
function parseCommand(args: readonly string[]) {
  ...
  const unknownOptions = args.filter((arg) => arg.startsWith('--') && arg !== '--logout')
  if (unknownOptions.length > 0 || positional.length !== 1) {
    throw new Error('用法：pnpm --filter @starter/api ai:auth -- <providerId> [--logout]')
  }
  ...
  return {
    async prompt(prompt) {
      if (prompt.type === 'select') {
        for (const option of prompt.options) {
          process.stdout.write(`${option.id}: ${option.label}${option.description ? ` - ${option.description}` : ''}\n`)
        }
      }
      const answer = await askTerminal(prompt)
      if (prompt.type === 'select' && !prompt.options.some((option) => option.id === answer)) {
    ...
    notify(event) {
      if (event.type === 'auth_url') {
        process.stdout.write(`${event.instructions ?? '打开以下地址完成认证'}\n${event.url}\n`)
      } else if (event.type === 'device_code') {
        process.stdout.write(`打开 ${event.verificationUri}，输入设备码 ${event.userCode}\n`)
      } else if (event.type === 'info') {
        process.stdout.write(`${event.message}\n`)
        for (const link of event.links ?? []) process.stdout.write(`${link.label ?? '参考'}: ${link.url}\n`)
      } else {
        process.stdout.write(`${event.message}\n`)
      }
    },
...
if (entryPath && import.meta.url === pathToFileURL(resolve(entryPath)).href) {
  process.exitCode = await runAiAuth(process.argv.slice(2), createTerminalInteraction())
}
