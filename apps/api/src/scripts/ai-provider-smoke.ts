...
const smokeEnvSchema = z.object({
  ...
  AI_SMOKE_TIMEOUT_MS: z.coerce.number().int().min(1_000).max(300_000).optional(),
})
...
export async function runAiProviderSmoke(
  ...
  try {
    ...
    const provider = runtime.ai.providers.find((item) => item.id === smoke.AI_SMOKE_PROVIDER_ID)
    if (!provider) throw new Error('Provider 不在 registry 中')
    output.log(`provider=${provider.id} model=${smoke.AI_SMOKE_MODEL_ID} event=start`)
    ...
    if (smoke.AI_SMOKE_CHECK_AUTH) {
      ...
      output.log(`provider=${provider.id} auth_source=${auth.source} credential_type=${auth.credentialType ?? 'null'}`)
    }
    ...
    const model = runtime.ai.listModels(provider.id).find((item) => item.modelId === smoke.AI_SMOKE_MODEL_ID)
    ...
    try {
      ...
      output.log(`provider=${provider.id} model=${model.modelId} event=error error=${category}`)
      return 1
    }
    if (!stopReason) {
      output.log(`provider=${provider.id} model=${model.modelId} event=error error=upstream_failed`)
      return 1
    }

    const durationMs = Date.now() - startedAt
    output.log(`provider=${provider.id} model=${model.modelId} event=text_delta count=${textDeltas}`)
    output.log(
    ...
    } else if (error instanceof Error && /no such table:/iu.test(error.message)) {
      output.error('AI 配置表不存在，请先运行 pnpm --filter @starter/api db:migrate')
    } else {
      output.error(error instanceof Error ? error.message : 'AI smoke 执行失败')
    }
    return 1
  } finally {
